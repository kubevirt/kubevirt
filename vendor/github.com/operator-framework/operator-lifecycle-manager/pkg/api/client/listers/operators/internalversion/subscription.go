...
type SubscriptionLister interface {
	...
	List(selector labels.Selector) (ret []*operators.Subscription, err error)
...
// List lists all Subscriptions in the indexer.
func (s *subscriptionLister) List(selector labels.Selector) (ret []*operators.Subscription, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*operators.Subscription))
	})
	return ret, err
}
...
type SubscriptionNamespaceLister interface {
	...
	Get(name string) (*operators.Subscription, error)
...
// List lists all Subscriptions in the indexer for a given namespace.
func (s subscriptionNamespaceLister) List(selector labels.Selector) (ret []*operators.Subscription, err error) {
...
// Get retrieves the Subscription from the indexer for a given namespace and name.
func (s subscriptionNamespaceLister) Get(name string) (*operators.Subscription, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(operators.Resource("subscription"), name)
	}
	return obj.(*operators.Subscription), nil
}
