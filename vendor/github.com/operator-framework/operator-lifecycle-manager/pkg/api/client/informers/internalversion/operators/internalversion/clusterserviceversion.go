...
type ClusterServiceVersionInformer interface {
	...
	Lister() internalversion.ClusterServiceVersionLister
}
...
func NewClusterServiceVersionInformer(client clientsetinternalversion.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredClusterServiceVersionInformer(client clientsetinternalversion.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.Operators().ClusterServiceVersions(namespace).List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.Operators().ClusterServiceVersions(namespace).Watch(options)
			},
		},
...
func (f *clusterServiceVersionInformer) defaultInformer(client clientsetinternalversion.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
...
func (f *clusterServiceVersionInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&operators.ClusterServiceVersion{}, f.defaultInformer)
}

func (f *clusterServiceVersionInformer) Lister() internalversion.ClusterServiceVersionLister {
	return internalversion.NewClusterServiceVersionLister(f.Informer().GetIndexer())
}
