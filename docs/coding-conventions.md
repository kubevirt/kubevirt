...
KubeVirt follows commit message guidelines based on the [Kubernetes Pull Request Process guide](https://www.kubernetes.dev/docs/guide/pull-requests/#commit-message-guidelines). These guidelines help maintain a clear and consistent commit history.
...
- **GitHub keywords**: Do not use GitHub keywords (like "Fixes #xxxx") or (@)mentions within your commit message

### DCO compliance

All commits must include a `Signed-off-by` line to certify compliance with the [Developer Certificate of Origin](https://developercertificate.org/). This can be done by adding `--signoff` to your git commit command.
...
```
network: Add support for SR-IOV interfaces
...
- Body uses GitHub keywords ("Fixes #123")
