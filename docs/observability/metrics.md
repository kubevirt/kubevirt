...
| vmi:kubevirt_vmi_gpu_gr_engine_active:sum | Recording rule | Gauge | Ratio (0-1) of the time the graphics engine was active on the GPU passed through to a virtual machine instance. |
...
| vmi:kubevirt_vmi_gpu_util:sum | Recording rule | Gauge | Utilization (ratio, 0-1) of the time the GPU passed through to a virtual machine instance was busy. |
