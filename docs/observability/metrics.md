...
| cluster:kubevirt_api_request_deprecated_total:sum | Recording rule | Counter | The total number of requests to deprecated KubeVirt APIs, by API verb (e.g., GET, POST, PUT, PATCH, DELETE, LIST, WATCH). |
...
| kubevirt_vmi_guest_vcpu_queue | Recording rule | Gauge | [Deprecated] Replaced by vmi:kubevirt_vmi_guest_vcpu_queue:count. |
...
| vmi:kubevirt_vmi_guest_vcpu_queue:count | Recording rule | Gauge | Guest queue length. |
