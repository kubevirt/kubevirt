...
| `nodePlacement` | Scheduling configuration including nodeSelector for targeting specific nodes. | Yes |
...
1. If the VMI's `spec.nodeSelector` contains all key-value pairs from an
   additional handler's `nodePlacement.nodeSelector`, that handler's
...
- All key-value pairs in the handler's `nodePlacement.nodeSelector` must
