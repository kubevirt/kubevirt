...
A test is eligible for de-quarantining once it demonstrates a failure rate of 0.1% or less. This can be verified in two
ways:

* Triggering the `pull-kubevirt-check-dequarantine-test` job by commenting
  `/test pull-kubevirt-check-dequarantine-test`[^1].
* Observing the periodic test runs to confirm the failure rate meets the criteria.
