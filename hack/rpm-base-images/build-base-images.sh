...
if [[ "${BUILD_ARCH}" != "s390x" ]]; then
    BASE_IMAGES+=("pr-helper")
fi

# libguestfs-tools only for x86_64 and s390x
if [[ "${BUILD_ARCH}" != "arm64" && "${BUILD_ARCH}" != "aarch64" ]]; then
    BASE_IMAGES+=("libguestfs-tools")
fi

# Allow override via BASE_IMAGE_TARGETS
if [[ -n "${BASE_IMAGE_TARGETS:-}" ]]; then
    IFS=',' read -ra BASE_IMAGES <<< "${BASE_IMAGE_TARGETS}"
fi
