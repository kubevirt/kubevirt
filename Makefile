...
conformance:
	hack/dockerized "export KUBEVIRT_PROVIDER=${KUBEVIRT_PROVIDER} PRIMARY_NETWORK_BINDING_PLUGIN=${PRIMARY_NETWORK_BINDING_PLUGIN} SKIP_OUTSIDE_CONN_TESTS=${SKIP_OUTSIDE_CONN_TESTS} RUN_ON_ARM64_INFRA=${RUN_ON_ARM64_INFRA} SKIP_BLOCK_STORAGE_TESTS=${SKIP_BLOCK_STORAGE_TESTS} SKIP_SNAPSHOT_STORAGE_TESTS=${SKIP_SNAPSHOT_STORAGE_TESTS} KUBEVIRT_E2E_FOCUS=${KUBEVIRT_E2E_FOCUS} DOCKER_PREFIX=${DOCKER_PREFIX} DOCKER_TAG=${DOCKER_TAG} && hack/conformance.sh"
...
rpm-deps:
	SYNC_VENDOR=true hack/dockerized "KUBEVIRT_CENTOS_STREAM_VERSION=${KUBEVIRT_CENTOS_STREAM_VERSION} CUSTOM_REPO=${CUSTOM_REPO} SINGLE_ARCH=${SINGLE_ARCH} BASESYSTEM=${BASESYSTEM} LIBVIRT_VERSION=${LIBVIRT_VERSION} QEMU_VERSION=${QEMU_VERSION} SEABIOS_VERSION=${SEABIOS_VERSION} EDK2_VERSION=${EDK2_VERSION} LIBGUESTFS_VERSION=${LIBGUESTFS_VERSION} GUESTFSTOOLS_VERSION=${GUESTFSTOOLS_VERSION} PASST_VERSION=${PASST_VERSION} VIRTIOFSD_VERSION=${VIRTIOFSD_VERSION} SWTPM_VERSION=${SWTPM_VERSION} LIBNBD_VERSION=${LIBNBD_VERSION} GLIB2_VERSION=${GLIB2_VERSION} GNUTLS_VERSION=${GNUTLS_VERSION} SYSTEMD_LIBS_VERSION=${SYSTEMD_LIBS_VERSION} LIBCAP_NG_VERSION=${LIBCAP_NG_VERSION} CENTOS_STREAM_COMPOSE=${CENTOS_STREAM_COMPOSE} CENTOS_STREAM_9_COMPOSE=${CENTOS_STREAM_9_COMPOSE} CENTOS_STREAM_10_COMPOSE=${CENTOS_STREAM_10_COMPOSE} CENTOS_STREAM_COMPOSE_BASEURL=${CENTOS_STREAM_COMPOSE_BASEURL} BAZELDNF_NOBEST=${BAZELDNF_NOBEST} KUBEVIRT_CROSS_ARCH_EMULATION=${KUBEVIRT_CROSS_ARCH_EMULATION} ./hack/rpm-deps.sh"
	$(MAKE) generate
