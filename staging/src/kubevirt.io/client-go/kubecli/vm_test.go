...
var _ = Describe("Kubevirt VirtualMachine Client", func() {
	...
	DescribeTable("should create a VirtualMachine", func(proxyPath string) {
		...
		createdVM, err := client.VirtualMachine(k8sv1.NamespaceDefault).Create(context.Background(), vm, k8smetav1.CreateOptions{})
	...
	DescribeTable("should update a VirtualMachine", func(proxyPath string) {
		...
		updatedVM, err := client.VirtualMachine(k8sv1.NamespaceDefault).Update(context.Background(), vm, metav1.UpdateOptions{})
	...
	DescribeTable("should evacuate cancel a VirtualMachine", func(proxyPath string) {
		client, err := GetKubevirtClientFromFlags(server.URL()+proxyPath, "")
		Expect(err).ToNot(HaveOccurred())

		server.AppendHandlers(ghttp.CombineHandlers(
			ghttp.VerifyRequest("PUT", path.Join(proxyPath, subVMPath, "evacuatecancel")),
			ghttp.RespondWithJSONEncoded(http.StatusOK, nil),
		))
