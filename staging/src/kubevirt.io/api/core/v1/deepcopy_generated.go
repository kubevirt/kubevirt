...
func (in *ComponentVersions) DeepCopyInto(out *ComponentVersions) {
	*out = *in
	if in.VirtController != nil {
		in, out := &in.VirtController, &out.VirtController
		*out = make(map[string]string, len(*in))
		for key, val := range *in {
			(*out)[key] = val
		}
	}
	if in.VirtHandler != nil {
		in, out := &in.VirtHandler, &out.VirtHandler
		*out = make(map[string]string, len(*in))
		for key, val := range *in {
			(*out)[key] = val
		}
...
func (in *KubeVirtStatus) DeepCopyInto(out *KubeVirtStatus) {
	*out = *in
	...
	in.ComponentVersions.DeepCopyInto(&out.ComponentVersions)
	return
}
