...
func NewPasstNetworkConfigurator(ifaces []vmschema.Interface, networks []vmschema.Network, opts NetworkConfiguratorOptions) (*PasstNetworkConfigurator, error) {
	...
	if iface.Binding == nil && iface.Passt == nil {
		return nil, fmt.Errorf("interface %q is not set with Passt network binding plugin or Passt binding method", network.Name)
	}
	if iface.Binding != nil && iface.Binding.Name != PasstPluginName {
...
func (p PasstNetworkConfigurator) generateInterface() (*domainschema.Interface, error) {
	...
	if p.options.UseLaunchSecurity && ifaceModel == vmschema.VirtIO {
		driver = &domainschema.InterfaceDriver{Name: "vhost", IOMMU: "on"}
	}

	var rom *domainschema.Rom
	if p.options.UseLaunchSecurity {
