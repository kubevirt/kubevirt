...
func (app *virtHandlerApp) runServer(errCh chan error, consoleHandler *rest.ConsoleHandler, lifecycleHandler *rest.LifecycleHandler) {
	...
	ws.Route(ws.PUT("/v1/namespaces/{namespace}/virtualmachineinstances/{name}/guestexec").To(lifecycleHandler.GuestExec).Produces(restful.MIME_JSON).Consumes(restful.MIME_JSON).Reads(v1.GuestExecOptions{}).Returns(http.StatusOK, "OK", v1.GuestExecResult{}))
