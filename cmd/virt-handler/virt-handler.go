...
func (app *virtHandlerApp) Run() {
	...
	if err != nil {
		logger.Critical(fmt.Sprintf("Error searching for namespace: %v", err))
	}
	...
	if err := app.prepareCertManager(); err != nil {
		logger.Critical(fmt.Sprintf("Error preparing the certificate manager: %v", err))
	}
	...
	if err := app.setupTLS(factory); err != nil {
		logger.Critical(fmt.Sprintf("Error constructing migration tls config: %v", err))
	}
