...
| `imagePullPolicy` | `string` | Yes | Image pull policy for the sidecar container. Must be one of: `IfNotPresent`, `Always`, or `Never`. | `"imagePullPolicy": "IfNotPresent"` |
...
| `configMap.name` | `string` | No | Name of the ConfigMap in the same namespace containing a script to execute. | `"name": "my-config-map"` |
| `configMap.key` | `string` | No | Key in the ConfigMap that contains the script. | `"key": "my_script.sh"` |
| `configMap.hookPath` | `string` | No | Path where the script will be mounted. Must be either `/usr/bin/onDefineDomain` or `/usr/bin/preCloudInitIso`. | `"hookPath": "/usr/bin/onDefineDomain"` |
...
| `pvc.name` | `string` | No | Name of the PVC in the same namespace to mount in the sidecar container. | `"name": "my-pvc"` |
| `pvc.volumePath` | `string` | No | Mount path in the sidecar container. | `"volumePath": "/debug"` |
