...
var _ = SIGDescribe("VirtualMachineSnapshot Tests", func() {
	...
	Context("With simple VM", func() {
		...
		It("[test_id:4609]should successfully create a snapshot", decorators.Conformance, func() {
			createAndVerifyVMSnapshot(vm)
		})

		It("[test_id:4610]create a snapshot when VM is running should succeed", decorators.Conformance, func() {
	...
	Context("[storage-req]", decorators.StorageReq, decorators.RequiresSnapshotStorageClass, func() {
		...
		Context("With online vm snapshot", func() {
			...
			It("[test_id:6767]with volumes and guest agent available", decorators.Conformance, func() {
			...
			It("[test_id:6768]with volumes and no guest agent available", decorators.Conformance, func() {
				quantity, err := resource.ParseQuantity("1Gi")
				Expect(err).ToNot(HaveOccurred())
				vmi := libvmifact.NewFedora(libnet.WithMasqueradeNetworking())
				vmi.Namespace = testsuite.GetTestNamespace(nil)
		...
		Context("With more complicated VM", func() {
			...
			It("[test_id:4611] should successfully create a snapshot", decorators.Conformance, func() {
				snapshot = libstorage.NewSnapshot(vm.Name, vm.Namespace)
		...
		Context("With VM using instancetype and preferences", func() {
			...
			DescribeTable("Bug #8435 - should create a snapshot successfully", decorators.Conformance, func(toRunSourceVM bool) {
				if !toRunSourceVM {
					By("Stoping the VM")
					vm = libvmops.StopVirtualMachine(vm)
				}
