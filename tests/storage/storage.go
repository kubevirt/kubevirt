	...
	Describe("Starting a VirtualMachineInstance", func() {
		...
		Context("VirtIO-FS with an empty PVC", func() {
			...
			It("should be successfully started and virtiofs could be accessed", func() {
				...
				podVirtioFsFileExist, _, err := exec.ExecuteCommandOnPod(
					virtClient,
					pod,
					"compute",
		...
		Context("[rfe_id:2298][crit:medium][vendor:cnv-qe@redhat.com][level:component] With HostDisk and PVC initialization", func() {
			...
			Context("With smaller than requested PVCs", func() {
				...
				BeforeEach(func() {
					...
					diskSizeStr, _, err := exec.ExecuteCommandOnPod(virtClient, pod, pod.Spec.Containers[0].Name, []string{tests.BinBash, "-c", fmt.Sprintf("df %s | tail -n 1 | awk '{print $4}'", mountDir)})
					Expect(err).ToNot(HaveOccurred())
		...
		Context("disk shareable tunable", func() {
			...
			BeforeEach(func() {
				dv = libstorage.NewRandomDataVolumeWithRegistryImport(cd.DataVolumeImportUrlForContainerDisk(cd.ContainerDiskCirros), util.NamespaceTestDefault, k8sv1.ReadWriteOnce)
