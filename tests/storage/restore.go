...
var _ = SIGDescribe("[Serial]VirtualMachineRestore Tests", func() {
	...
	Context("rook-ceph", func() {
		Context("With a more complicated VM", func() {
			...
			It("[QUARANTINE][test_id:5259]should restore a vm multiple from the same snapshot", func() {
			...
			It("[QUARANTINE][test_id:5260]should restore a vm that boots from a datavolumetemplate", func() {
				vm, vmi = createAndStartVM(tests.NewRandomVMWithDataVolumeAndUserDataInStorageClass(
					tests.GetUrl(tests.CirrosHttpUrl),
					tests.NamespaceTestDefault,
					"#!/bin/bash\necho 'hello'\n",
