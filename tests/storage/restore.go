	...
	Context("[storage-req]", decorators.StorageReq, func() {
		Context("With a more complicated VM", func() {
			...
			It("[test_id:5259]should restore a vm multiple from the same snapshot", func() {
				vm, vmi = createAndStartVM(newVMWithDataVolumeForRestore(cd.ContainerDiskCirros, snapshotStorageClass))
			...
			DescribeTable("should restore a vm with restore size bigger then PVC size", func(restoreToNewVM bool) {
				vm = newVMWithDataVolumeForRestore(cd.ContainerDiskCirros, snapshotStorageClass)
			...
			DescribeTable("should restore an online vm snapshot that boots from a datavolumetemplate with guest agent", func(restoreToNewVM bool) {
				vm, vmi = createAndStartVM(newVMWithDataVolumeForRestore(cd.ContainerDiskFedoraTestTooling, snapshotStorageClass))
				Eventually(matcher.ThisVMI(vmi), 12*time.Minute, 2*time.Second).Should(matcher.HaveConditionTrue(v1.VirtualMachineInstanceAgentConnected))
...
func newVMWithDataVolumeForRestore(containerDisk cd.ContainerDisk, storageClass string) *v1.VirtualMachine {
