...
var _ = SIGDescribe("VirtualMachineRestore Tests", func() {
	...
	Context("With simple VM", func() {
		...
		Context("and good snapshot exists", func() {
			...
			Context("restore to a new VM that does not exist", func() {
				...
				It("with changed name and MAC address", decorators.Conformance, func() {
		...
		Context("with instancetype and preferences", func() {
			...
			DescribeTable("should use existing ControllerRevisions for an existing VM restore", decorators.Conformance, Label("instancetype", "preference", "restore"), func(runStrategy v1.VirtualMachineRunStrategy) {
			...
			DescribeTable("should create new ControllerRevisions for newly restored VM", decorators.Conformance, Label("instancetype", "preference", "restore"), func(runStrategy v1.VirtualMachineRunStrategy) {
				libvmi.WithRunStrategy(runStrategy)(vm)
				vm, err := virtClient.VirtualMachine(testsuite.GetTestNamespace(vm)).Create(context.Background(), vm, metav1.CreateOptions{})
				Expect(err).ToNot(HaveOccurred())
	...
	Context("[storage-req]", decorators.StorageReq, func() {
		Context("With a more complicated VM", func() {
			...
			DescribeTable("should restore a vm that boots from a datavolumetemplate", decorators.Conformance, func(restoreToNewVM bool) {
			...
			DescribeTable("should restore a vm from an online snapshot", decorators.Conformance, func(restoreToNewVM bool) {
				vm = createVMWithCloudInit(cd.ContainerDiskCirros, snapshotStorageClass)
			...
			DescribeTable("should restore an online vm snapshot that boots from a datavolumetemplate with guest agent", decorators.Conformance, func(restoreToNewVM bool) {
			...
			Context("with cross namespace clone ability", func() {
				...
				DescribeTable("should restore a vm that boots from a network cloned datavolumetemplate", func(restoreToNewVM, deleteSourcePVC, deleteSourceNamespace bool) {
					...
					Entry("to the same VM", decorators.Conformance, false, false, false),
					Entry("to a new VM", decorators.Conformance, true, false, false),
