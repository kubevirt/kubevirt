...
var _ = SIGDescribe("VirtualMachineRestore Tests", func() {
	...
	Context("With simple VM", func() {
		...
		Context("with run strategy and snapshot", func() {
			...
			It("should successfully restore", func() {
				...
				Expect(err).ToNot(HaveOccurred())
				Expect(restoredVM.Spec.RunStrategy).To(HaveValue(Equal(v1.RunStrategyRerunOnFailure)))
			})
		})
	...
	Context("[storage-req]", decorators.StorageReq, func() {
		Context("With a more complicated VM", func() {
			...
			createSnapshotAndRestore := func(device string, login console.LoginToFunction, onlineSnapshot bool, tpm bool, targetVMName string) {
			...
			doRestore := func(device string, login console.LoginToFunction, onlineSnapshot bool, targetVMName string) {
				createSnapshotAndRestore(device, login, onlineSnapshot, false, targetVMName)
			...
			DescribeTable("Should restore a vm with backend storage", Serial, func(onlineSnapshot bool) {
				...
				createSnapshotAndRestore("", loginFunc, onlineSnapshot, true, vm.Name)
			...
			Context("with memory dump", func() {
				...
				DescribeTable("should not restore memory dump volume", func(restoreToNewVM bool) {
					...
					createSnapshotAndRestore("", console.LoginToFedora, onlineSnapshot, false, getTargetVMName(restoreToNewVM, newVmName))
					Expect(restore.Status.Restores).To(HaveLen(1))
