	...
	Describe("Hotplug volumes", func() {
		...
		Context("should be able to volume migrate a VM", func() {
			...
			createFileOnHotpluggedVol := func(vmi *v1.VirtualMachineInstance, volName string) {
				...
				Expect(console.RunCommand(vmi, "sudo mkfs.ext3 /dev/sda", 60*time.Second)).To(Succeed())
				Expect(console.RunCommand(vmi, fmt.Sprintf("sudo mount -t ext3 /dev/%s /mnt", device), 60*time.Second)).To(Succeed())
				Expect(console.RunCommand(vmi, "sudo echo 'test' /mnt/test", 60*time.Second)).To(Succeed())
			}
			checkFileOnHotpluggedVol := func(vmi *v1.VirtualMachineInstance) {
				Expect(console.LoginToCirros(vmi)).To(Succeed())
				out, err := console.RunCommandAndStoreOutput(vmi, "sudo cat /mnt/test", 60*time.Second)
				Expect(err).ShouldNot(HaveOccurred(), fmt.Sprintf("command failed with: %s", out))
			}
			...
			It("with a datavolume and an hotplugged datavolume using block volumes for the destination", func() {
				ns := testsuite.GetTestNamespace(nil)
				rootVolName := "root"
				hpVolName := "hp"
				sc, exist := libstorage.GetRWOFileSystemStorageClass()
				...
				hpDV := createBlankDV(virtClient, ns, "1G")
				...
				blockSc, exist := libstorage.GetRWOBlockStorageClass()
				Expect(exist).To(BeTrue())
				dvRootDst := libdv.NewDataVolume(
					libdv.WithBlankImageSource(),
					libdv.WithStorage(libdv.StorageWithStorageClass(blockSc),
