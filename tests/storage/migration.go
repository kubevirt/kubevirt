	...
	Describe("Update volumes with the migration updateVolumesStrategy", func() {
		...
		updateVMWithPVC := func(vm *virtv1.VirtualMachine, volName, claim string) {
			...
			p, err := patch.New(
				...
				patch.WithReplace(fmt.Sprintf("/spec/template/spec/volumes[%d]/volumeSource/persistentVolumeClaim", replacedIndex), fmt.Sprintf("{claimName: %s}", claim)),
				patch.WithRemove(fmt.Sprintf("/spec/template/spec/volumes[%d]/volumeSource/dataVolume", replacedIndex)),
		...
		updateVMWithDV := func(vm *virtv1.VirtualMachine, volName, name string) {
			...
			p, err := patch.New(
				patch.WithReplace("/spec/dataVolumeTemplates[0].name", name),
				patch.WithReplace(fmt.Sprintf("/spec/template/spec/volumes[%d]/dataVolume", replacedIndex), fmt.Sprintf("{name: %s}", name)),
				patch.WithReplace("/spec/updateVolumesStrategy", virtv1.UpdateVolumesStrategyMigration),
			).GeneratePayload()
			Expect(err).ToNot(HaveOccurred())
			vm, err = virtClient.VirtualMachine(vm.Namespace).Patch(context.Background(), vm.Name, types.JSONPatchType, p, metav1.PatchOptions{})
