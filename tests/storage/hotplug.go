...
var _ = SIGDescribe("Hotplug", func() {
	...
	Context("Offline VM", func() {
		...
		DescribeTable("Should add volumes on an offline VM", decorators.Conformance, func(addVolumeFunc addVolumeFunction, removeVolumeFunc removeVolumeFunction) {
	...
	Context("Offline VM with a block volume", decorators.RequiresRWXBlock, func() {
		...
		DescribeTable("Should start with a hotplug block", decorators.Conformance, func(addVolumeFunc addVolumeFunction) {
	...
	Context("[storage-req]", decorators.StorageReq, func() {
		Context("Online VM", decorators.RequiresRWXBlock, func() {
			...
			DescribeTable("should add/remove volume", decorators.Conformance, func(addVolumeFunc addVolumeFunction, removeVolumeFunc removeVolumeFunction, volumeMode k8sv1.PersistentVolumeMode, vmiOnly, waitToStart bool) {
		...
		Context("VMI migration", decorators.RequiresRWXBlock, func() {
			...
			DescribeTable("should allow live migration with attached hotplug volumes", decorators.Conformance, func(vmiFunc func() *v1.VirtualMachineInstance) {
				vmi = vmiFunc()
	...
	Context("hostpath", func() {
		...
		It("should attach a hostpath based volume to running VM", decorators.Conformance, func() {
			vmi, err := virtClient.VirtualMachineInstance(vm.Namespace).Get(context.Background(), vm.Name, metav1.GetOptions{})
			Expect(err).ToNot(HaveOccurred())
			libwait.WaitForSuccessfulVMIStart(vmi,
				libwait.WithTimeout(240),
