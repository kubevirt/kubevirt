...
var _ = Describe(SIG("Hotplug", func() {
	...
	Context("delete attachment pod several times", decorators.RequiresRWXBlock, func() {
		...
		deleteQuota := func(namespace string) {
			Expect(virtClient.CoreV1().ResourceQuotas(namespace).Delete(context.Background(), quotaName, metav1.DeleteOptions{})).To(Succeed())
		}
		...
		DescribeTable("should remain active", func(limitHotplugPodCreation bool, hotplugPodDeletionTimes int) {
			...
			if limitHotplugPodCreation {
				...
				defer deleteQuota(vmi.Namespace)
			}
			...
			Consistently(matcher.ThisVM(vm), 30*time.Second, 5*time.Second).Should(Not(matcher.HaveConditionTrue(v1.VirtualMachineInstancePaused)))
		},
			Entry("when deleting the hotplug pod and turning it unschedulable", true, 1),
