...
var _ = Describe(SIG("Backup", func() {
	...
	It("Full and Incremental Backup with BackupTracker", func() {
		...
		Expect(fullBackup.Status.IncludedDisks).To(HaveLen(1), "Should have one included disk")
		...
		Expect(incrementalBackup.Status.IncludedDisks).To(HaveLen(1), "Should have one included disk")
	...
	It("Full and Incremental Backup with 2 disks", func() {
		...
		Expect(fullBackup.Status.Type).To(Equal(backupv1.Full), "First backup should be Full")
		Expect(fullBackup.Status.CheckpointName).ToNot(BeNil())
		Expect(fullBackup.Status.IncludedDisks).To(HaveLen(2), "Should have two included disks")
		...
		Expect(incrementalBackup.Status.Type).To(Equal(backupv1.Incremental), "Second backup should be Incremental")
		Expect(incrementalBackup.Status.CheckpointName).ToNot(BeNil())
		Expect(incrementalBackup.Status.IncludedDisks).To(HaveLen(2), "Should have two included disks")
	...
	It("Incremental Backup after VM shutdown and restart", func() {
		...
		Expect(firstCheckpoint).ToNot(BeNil(), "Tracker should have checkpoint after first backup")
		Expect(firstCheckpoint.Name).To(Equal(*fullBackup.Status.CheckpointName), "First checkpoint should match backup checkpoint")
		Expect(firstCheckpoint.Disks).ToNot(BeEmpty(), "Checkpoint should have disk info for redefinition")
		...
		checkpointsMatch, _ := Equal(checkpointsBeforeStop).Match(checkpointsAfterRestart)
		if len(checkpointsAfterRestart) != 1 || !checkpointsMatch {
			fmt.Printf("WARNING: Checkpoint redefinition may have failed!\n")
			fmt.Printf("  Checkpoints before stop: %v\n", checkpointsBeforeStop)
			fmt.Printf("  Checkpoints after restart: %v\n", checkpointsAfterRestart)
			fmt.Printf("  Sleeping for 10 minutes to allow investigation...\n")
	...
	It("Incremental Backup after multiple VM restarts", func() {
		const (
			testDataSizeMB    = 30
			testDataSizeBytes = testDataSizeMB * 1024 * 1024
		)
		...
		By("Verifying full backup size")
		verifyBackupTargetPVCOutput(virtClient, fullBackupPVC, vm.Name, 1, []int64{expectedDiskSize.Value()})

		By("Getting checkpoint info after first backup")
		tracker, err = virtClient.VirtualMachineBackupTracker(tracker.Namespace).Get(context.Background(), tracker.Name, metav1.GetOptions{})
		Expect(err).ToNot(HaveOccurred())
		firstCheckpoint := tracker.Status.LatestCheckpoint
		Expect(firstCheckpoint).ToNot(BeNil())

		By("Listing checkpoints before first restart")
		vmi, err := virtClient.VirtualMachineInstance(vm.Namespace).Get(context.Background(), vm.Name, metav1.GetOptions{})
		Expect(err).ToNot(HaveOccurred())
		checkpointsBeforeFirstRestart := listDomainCheckpoints(vmi)
		Expect(checkpointsBeforeFirstRestart).To(HaveLen(1), "Should have exactly one checkpoint before first restart")
		Expect(checkpointsBeforeFirstRestart[0]).To(Equal(firstCheckpoint.Name), "Checkpoint name should match tracker checkpoint")

		By("Writing data to VM disk")
		Expect(console.LoginToAlpine(vmi)).To(Succeed())
		err = console.RunCommand(vmi, fmt.Sprintf("dd if=/dev/urandom of=/root/testfile1 bs=1M count=%d && sync", testDataSizeMB), 2*time.Minute)
		Expect(err).ToNot(HaveOccurred())

		By("First VM restart cycle")
		err = virtClient.VirtualMachine(vm.Namespace).Stop(context.Background(), vm.Name, &v1.StopOptions{})
		Expect(err).ToNot(HaveOccurred())
		Eventually(func() error {
			_, err := virtClient.VirtualMachineInstance(vm.Namespace).Get(context.Background(), vm.Name, metav1.GetOptions{})
		...
		By("Verifying checkpoint was redefined after first restart")
		vmi, err = virtClient.VirtualMachineInstance(vm.Namespace).Get(context.Background(), vm.Name, metav1.GetOptions{})
		Expect(err).ToNot(HaveOccurred())
		checkpointsAfterFirstRestart := listDomainCheckpoints(vmi)
		Expect(checkpointsAfterFirstRestart).To(HaveLen(1), "Should have exactly one checkpoint after first restart")
		Expect(checkpointsAfterFirstRestart[0]).To(Equal(firstCheckpoint.Name),
			"Checkpoint should be the same after first restart (was redefined)")

		By("Writing more data after first restart")
		Expect(console.LoginToAlpine(vmi)).To(Succeed())
		err = console.RunCommand(vmi, fmt.Sprintf("dd if=/dev/urandom of=/root/testfile2 bs=1M count=%d && sync", testDataSizeMB), 2*time.Minute)
		Expect(err).ToNot(HaveOccurred())

		By("Second VM restart cycle")
		err = virtClient.VirtualMachine(vm.Namespace).Stop(context.Background(), vm.Name, &v1.StopOptions{})
		Expect(err).ToNot(HaveOccurred())
		Eventually(func() error {
			_, err := virtClient.VirtualMachineInstance(vm.Namespace).Get(context.Background(), vm.Name, metav1.GetOptions{})
		...
		By("Verifying checkpoint was redefined after second restart")
		vmi, err = virtClient.VirtualMachineInstance(vm.Namespace).Get(context.Background(), vm.Name, metav1.GetOptions{})
		Expect(err).ToNot(HaveOccurred())
		checkpointsAfterSecondRestart := listDomainCheckpoints(vmi)
		Expect(checkpointsAfterSecondRestart).To(HaveLen(1), "Should have exactly one checkpoint after second restart")
		Expect(checkpointsAfterSecondRestart[0]).To(Equal(firstCheckpoint.Name),
			"Checkpoint should be the same after second restart (was redefined)")

		By("Verifying BackupTracker still has the same checkpoint after multiple restarts")
		tracker, err = virtClient.VirtualMachineBackupTracker(tracker.Namespace).Get(context.Background(), tracker.Name, metav1.GetOptions{})
		Expect(err).ToNot(HaveOccurred())
		Expect(tracker.Status.LatestCheckpoint).ToNot(BeNil(), "Checkpoint should survive multiple restarts")
		Expect(tracker.Status.LatestCheckpoint.Name).To(Equal(firstCheckpoint.Name),
			"BackupTracker checkpoint should be the same after multiple restarts")

		By("Creating incremental backup after multiple restarts")
		incrementalBackup := createAndVerifyBackupWithTracker(virtClient, backupName(vm.Name), vm.Namespace, incrementalBackupPVC.Name, tracker.Name)
		Expect(incrementalBackup.Status.Type).To(Equal(backupv1.Incremental),
			"Backup after multiple VM restarts should be Incremental")

		By("Verifying incremental backup captured all data written across restarts")
		// We wrote testDataSizeMB twice, so expect approximately 2x the data
		expectedIncrementalSize := int64(testDataSizeBytes * 2)
		verifyBackupTargetPVCOutput(virtClient, incrementalBackupPVC, vm.Name, 1, []int64{expectedIncrementalSize})
	})
