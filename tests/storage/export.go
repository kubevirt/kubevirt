...
var _ = Describe(SIG("Export", func() {
	...
	Context("OCI export", Serial, func() {
		...
		It("should export VirtualMachineTemplate as OCI artifact", func() {
			...
			tplReq, err := tplClient.TemplateV1alpha1().VirtualMachineTemplateRequests(vm.Namespace).Create(
				context.Background(),
			...
			Eventually(func(g Gomega) {
				tplReq, err = tplClient.TemplateV1alpha1().VirtualMachineTemplateRequests(vm.Namespace).Get(
					context.Background(), tplReq.Name, metav1.GetOptions{})
				g.Expect(err).ToNot(HaveOccurred())
				g.Expect(meta.IsStatusConditionTrue(tplReq.Status.Conditions, templatev1alpha1.ConditionReady)).
