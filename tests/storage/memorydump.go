...
var _ = Describe(SIG("Memory dump", func() {
	...
	Context("Memory dump with existing PVC", func() {
		...
		BeforeEach(func() {
			...
			memoryDumpPVC = libstorage.CreateFSPVC(memoryDumpPVCName, testsuite.GetTestNamespace(vm), memoryDumpPVCSize)
		})
		...
		It("[test_id:8503]Run memory dump to a pvc, remove and run memory dump to different pvc", func() {
			...
			memoryDumpPVC2 = libstorage.CreateFSPVC(memoryDumpPVCName2, testsuite.GetTestNamespace(vm), memoryDumpPVCSize)
		...
		It("[test_id:8501]Run memory dump with pvc too small should fail", func() {
			...
			memoryDumpPVC2 = libstorage.CreateFSPVC(memoryDumpPVCName2, testsuite.GetTestNamespace(vm), "200Mi")
			Eventually(func() error {
