...
var _ = Describe("[sig-compute]Cross-architecture software emulation", Serial, decorators.SigCompute, func() {
	...
	BeforeEach(func() {
		config.EnableFeatureGate(featuregate.MultiArchitectureSoftwareEmulation)
	})

	DescribeTable("should boot a guest using QEMU TCG emulation on a cross-architecture host",
		func(guestArch, containerDiskImage, expectedEmulator string) {
			vmi := libvmi.New(
			...
			Fail("compute container not found in virt-launcher pod")
		},
