		...
		It("should immediately terminate paused VMI when VM is stopped", func() {
			...
			Eventually(matcher.ThisVMI(vmi), 30*time.Second, time.Second).Should(
				And(
					...
					matcher.HaveConditionMissingOrFalse(v1.VirtualMachineInstanceReady),
				),
			)
			...
			Eventually(func() error {
				...
				return err
			}, 60*time.Second, time.Second).Should(MatchError(k8serrors.IsNotFound, "VMI should be deleted"))
		})
