...
func waitForKvWithTimeout(newKv *v1.KubeVirt, timeoutSeconds int) {
	Eventually(func(g Gomega) {
		...
		g.Expect(kv.Status.Phase).To(Equal(v1.KubeVirtPhaseDeployed),
		...
		for _, condition := range kv.Status.Conditions {
			...
			case v1.KubeVirtConditionAvailable:
				available = condition.Status == k8sv1.ConditionTrue

			case v1.KubeVirtConditionProgressing:
				progressing = condition.Status != k8sv1.ConditionFalse

			case v1.KubeVirtConditionDegraded:
				degraded = condition.Status != k8sv1.ConditionFalse

			case v1.KubeVirtConditionCreated:
				created = condition.Status == k8sv1.ConditionTrue
			}
		}

		g.Expect(available && !progressing && !degraded && created).To(BeTrue(), func() string {
