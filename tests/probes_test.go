...
var _ = Describe("[ref_id:1182]Probes", func() {
	...
	Context("for readiness", func() {
		...
		table.DescribeTable("should succeed", func(readinessProbe *v12.Probe, IPFamily v1.IPFamily) {
			...
			table.Entry("[QUARANTINE][owner:@sig-network][sig-network][test_id:1202][posneg:positive]with working HTTP probe and http server, no ip family is specified ", httpProbe, blankIPFamily),
