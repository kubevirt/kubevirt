...
var _ = Describe("VmMigration", func() {
	...
	Context("New Migration given", func() {
		...
		It("Should fail if the VM does not exist", func() {
			err = restClient.Post().Resource("migrations").Namespace(kubev1.NamespaceDefault).Body(migration).Do().Error()
			Expect(err).To(BeNil())
			Eventually(func() v1.MigrationPhase {
				r, err := restClient.Get().Resource("migrations").Namespace(kubev1.NamespaceDefault).Name(migration.ObjectMeta.Name).Do().Get()
		...
		It("Should go to MigrationInProgress state if the VM exists", func(done Done) {
			...
			vm, err := restClient.Post().Resource("vms").Namespace(kubev1.NamespaceDefault).Body(sourceVM).Do().Get()
			Expect(err).ToNot(HaveOccurred())
			...
			Eventually(func() v1.MigrationPhase {
				obj, err := restClient.Get().Resource("migrations").Namespace(kubev1.NamespaceDefault).Name(migration.ObjectMeta.Name).Do().Get()
				Expect(err).ToNot(HaveOccurred())
		...
		It("Should update the Status.MigrationNodeName after the migration target pod was started", func(done Done) {
			...
			// Create the VM
			obj, err := restClient.Post().Resource("vms").Namespace(kubev1.NamespaceDefault).Body(sourceVM).Do().Get()
			Expect(err).ToNot(HaveOccurred())
			...
			Eventually(func() string {
				obj, err := restClient.Get().Resource("vms").Namespace(kubev1.NamespaceDefault).Name(obj.(*v1.VM).ObjectMeta.Name).Do().Get()
				Expect(err).ToNot(HaveOccurred())
				fetchedVM = obj.(*v1.VM)
		...
		It("Should migrate the VM", func(done Done) {
			...
			// Wait for the job
			Eventually(func() int {
				jobs, err := coreClient.CoreV1().Pods(kubev1.NamespaceDefault).List(kubev1.ListOptions{LabelSelector: selector.String()})
				Expect(err).ToNot(HaveOccurred())
				return len(jobs.Items)
			}, TIMEOUT*2, POLLING_INTERVAL).Should(Equal(1))

			// Wait for the successful completion of the job
			Eventually(func() kubev1.PodPhase {
				jobs, err := coreClient.CoreV1().Pods(kubev1.NamespaceDefault).List(kubev1.ListOptions{LabelSelector: selector.String()})
				Expect(err).ToNot(HaveOccurred())
				return jobs.Items[0].Status.Phase
			}, TIMEOUT*2, POLLING_INTERVAL).Should(Equal(kubev1.PodSucceeded))
