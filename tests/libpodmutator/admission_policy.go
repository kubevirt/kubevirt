...
func envFromJSONPatchExpression(configMapName string) string {
	...
	currentEnvFrom := compute + `.?envFrom.orValue([])`
	alreadyHasConfigMap := fmt.Sprintf(
		`%s.exists(e, has(e.configMapRef) && e.configMapRef.name == %q)`,
		currentEnvFrom, configMapName,
	)
	computeContainerDoesNotExist := `!object.spec.containers.exists(c, c.name == "compute")`

	return alreadyHasConfigMap + ` || ` + computeContainerDoesNotExist + ` ? [] : 
[JSONPatch{
	op: "add",
	path: "/spec/containers/" + ` + computeIdx + ` + "/envFrom",
	value: ` + currentEnvFrom + ` + [{"configMapRef": {"name": ` + strconv.Quote(configMapName) + `}}]
}]`
}
...
func SetupEnvInjectionPolicy(opts EnvInjectionPolicyOptions) *EnvInjectionPolicy {
	...
	eip := &EnvInjectionPolicy{
		PolicyName:  opts.Name,
		BindingName: opts.bindingName(),
	}

	// MAPs are cluster scoped, so clean up even if a later Create/Expect fails before return.
	setupSucceeded := false
	defer func() {
		if !setupSucceeded {
			TeardownEnvInjectionPolicy(eip)
		}
	}()
				...
				"matchConstraints": map[string]interface{}{
					...
					"namespaceSelector": map[string]interface{}{
						"matchExpressions": []interface{}{
							map[string]interface{}{
								"key":      k8sv1.LabelMetadataName,
								"operator": "In",
								"values":   []interface{}{testNamespace},
							},
						},
					},
				},
