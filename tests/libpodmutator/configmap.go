...
// EnvVar is a typed environment override that can parse its raw string value.
...
// variables. Each override is parsed as its declared type before it is written.
// The ConfigMap is created in namespace; when empty, testsuite.GetTestNamespace(nil) is used.
func CreateOrUpdateEnvConfigMap[T EnvVar](virtClient kubecli.KubevirtClient, name string, vars []T, namespace ...string) {
	data := make(map[string]string, len(vars))
	for _, envVar := range vars {
		_, err := envVar.Parse()
		Expect(err).To(Succeed(), "env %s=%q must parse", envVar.Key(), envVar.Raw())
		data[envVar.Key()] = envVar.Raw()
	}
