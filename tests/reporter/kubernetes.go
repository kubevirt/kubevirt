...
func (r *KubernetesReporter) Dump(duration time.Duration, isExternal bool) {
	...
	r.logNodeCommands(virtCli)
	r.logVirtLauncherCommands(virtCli)
...
func (r *KubernetesReporter) logVirtLauncherCommands(virtCli kubecli.KubevirtClient) {
	...
	for _, pod := range pods.Items {
		...
		if found := checkPodComputeContainer(pod); found != true {
...
func (r *KubernetesReporter) logNodeCommands(virtCli kubecli.KubevirtClient) {
...
func checkPodComputeContainer(pod v1.Pod) bool {
	for _, container := range pod.Spec.Containers {
...
func (r *KubernetesReporter) executeNodeCommands(virtCli kubecli.KubevirtClient, logsdir string, pod v1.Pod) {
	...
	cmds := []commands{
		...
		{command: hostPrefix + "/usr/bin/" + networkPrefix + "/usr/sbin/iptables --list", fileNameSuffix: "iptables"},

		//{command: hostPrefix + "/usr/bin/" + networkPrefix + "/usr/bin/ls -l /proc/1/root/sys/class/net/ens2f0/device/", fileNameSuffix: "virtfn"},
...
func (r *KubernetesReporter) executeVirtLauncherCommands(virtCli kubecli.KubevirtClient, logsdir string, pod v1.Pod) {
	cmds := []commands{
		...
		{command: "nft list ruleset", fileNameSuffix: "nftlist"},
	}
