...
func (r *KubernetesReporter) dumpTestObjects(duration time.Duration, vmiNamespaces []string) {
	...
	// Collect containerd, cri-o, and runc stacks from all nodes, not just nodes with test pods
	allNodeNames := []string{}
	if nodes != nil {
	...
	r.logRuncStacks(virtCli, nodesDir, allNodeNames)
}
...
func (r *KubernetesReporter) logRuncStacks(virtCli kubecli.KubevirtClient, logsdir string, nodes []string) {

	if logsdir == "" {
		printError("logsdir is empty, skipping logRuncStacks")
		return
	}
	...
	for _, node := range nodes {
		...
		// Check if runc is running on the node
		checkCommand := []string{
			virt_chroot.GetChrootBinaryPath(),
			"--mount",
			virt_chroot.GetChrootNSMountPath(),
		...
		stdout, _, err := exec.ExecuteCommandOnPodWithResults(pod, virtHandlerName, checkCommand)
		if err != nil || stdout == "" {
			printInfo("runc not running on node %s, skipping runc debug collection", node)
			continue
		}

		// Send USR1 signal to runc to trigger stack dump
		signalCommand := []string{
			virt_chroot.GetChrootBinaryPath(),
			"--mount",
			virt_chroot.GetChrootNSMountPath(),
		...
		_, stderr, err := exec.ExecuteCommandOnPodWithResults(pod, virtHandlerName, signalCommand)
		if err != nil {
			printError("failed to send USR1 to runc on node %s, stderr: %s, error: %v", node, stderr, err)
			continue
		}

		// Wait a moment for runc to write the stacks
		time.Sleep(2 * time.Second)

		// Collect runc logs which should contain the stack traces
		journalCommand := []string{
			virt_chroot.GetChrootBinaryPath(),
			"--mount",
			virt_chroot.GetChrootNSMountPath(),
		...
		stdout, stderr, err = exec.ExecuteCommandOnPodWithResults(pod, virtHandlerName, journalCommand)
		if err != nil {
			printError("failed to collect runc logs on node %s, stderr: %s, error: %v", node, stderr, err)
			continue
		}

		fileName := fmt.Sprintf(logFileNameFmt, r.failureCount, "runc-journal", node)
		err = writeStringToFile(filepath.Join(logsdir, fileName), stdout)
		if err != nil {
			printError("failed to write runc journal for node %s: %v", node, err)
			continue
		}

		// Collect runc debug information
		r.logRuncDebug(pod, node, logsdir)
	}
}

func (r *KubernetesReporter) logRuncDebug(pod *v1.Pod, node string, logsdir string) {
	runcCommands := []commands{
		{command: "runc --version", fileNameSuffix: "runc-version"},
		{command: "runc list", fileNameSuffix: "runc-list"},
	}

	for _, cmd := range runcCommands {
		command := []string{
			virt_chroot.GetChrootBinaryPath(),
			"--mount",
			virt_chroot.GetChrootNSMountPath(),
