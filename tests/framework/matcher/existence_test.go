...
var _ = Describe("Existence matchers", func() {
	...
	It("formating", func() {
		...
		Expect(BeGone().FailureMessage(obj.DeepCopy())).ToNot(SatisfyAll(ContainSubstring("something"), ContainSubstring("Spec"), ContainSubstring("ManagedFields")))
		Expect(BeGone().FailureMessage(obj.DeepCopy())).To(And(ContainSubstring("metadata"), ContainSubstring("status")))
		Expect(BeGone().NegatedFailureMessage(obj.DeepCopy())).ToNot(SatisfyAll(ContainSubstring("something"), ContainSubstring("Spec"), ContainSubstring("ManagedFields")))
		Expect(BeGone().NegatedFailureMessage(obj.DeepCopy())).To(And(ContainSubstring("metadata"), ContainSubstring("status")))
	})
})
