...
func formatObject(actual interface{}) string {
	if actual == nil || helper.IsNil(actual) {
		return fmt.Sprintf("%v", actual)
	}
	obj := reflect.ValueOf(helper.ToPointer(actual))
	...
	managedFields := metadata.FieldByName("ManagedFields")
	if !managedFields.IsZero() {
		managedFields.SetZero()
	}
