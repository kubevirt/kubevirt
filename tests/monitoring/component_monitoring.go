...
const (
	...
	virtHandlerDownNodeSelectorKey = "kubevirt.io/e2e-virt-handler-down"
	gomegaHelperCallerOffset       = 2
)
...
var _ = Describe("[sig-monitoring]Component Monitoring", Serial, Ordered, decorators.SigMonitoring, func() {
	...
	Context("Up metrics", func() {
		...
		It("VirtHandlerDown should be triggered when virt-handler is down", func() {
			...
			daemonSet.Spec.Template.Spec.NodeSelector[virtHandlerDownNodeSelectorKey] = rand.String(randNodeSelectorSuffixLength)
			...
			defer restoreVirtHandlerDownNodeSelector(virtClient)
...
// waitForScrapedMetricSumAtLeast queries the raw kubevirt_* series, not the
// cluster:* recording rules. Those rules use `or vector(0)`, so a scrape or
// label-join miss looks like a real 0 and AfterEach times out while the pod is
// already Ready. Missing series stay -1 here and keep polling.
...
func restoreVirtHandlerDownNodeSelector(virtClient kubecli.KubevirtClient) {
	By("Removing the virt-handler e2e nodeSelector")
	patch := fmt.Sprintf(
		`{"spec":{"template":{"spec":{"nodeSelector":{%q:null}}}}}`,
		virtHandlerDownNodeSelectorKey,
	)
	_, patchErr := virtClient.AppsV1().DaemonSets(flags.KubeVirtInstallNamespace).Patch(
		context.Background(), virtHandler.deploymentName, types.MergePatchType, []byte(patch), metav1.PatchOptions{},
	)
	Expect(patchErr).ToNot(HaveOccurred())

	By("Waiting for virt-handler pods to run again")
	libmonitoring.WaitForMetricValueWithLabelsToBe(
		virtClient, "cluster:kubevirt_virt_handler_pods_running:count", nil, gomegaHelperCallerOffset, ">", 0,
	)
