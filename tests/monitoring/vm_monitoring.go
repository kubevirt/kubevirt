...
var _ = Describe("[sig-monitoring]VM Monitoring", decorators.SigMonitoring, func() {
	...
	Context("VM alerts", Serial, func() {
		...
		It("OutdatedVirtualMachineInstanceWorkloads should be triggered when VMIs run outdated launcher images", func() {
			...
			By("Waiting for kubevirt_vmi_number_of_outdated metric to be > 0")
			libmonitoring.WaitForMetricValueWithLabelsToBe(virtClient, "kubevirt_vmi_number_of_outdated", nil, 1, ">", 0)
			...
			By("Waiting for OutdatedVirtualMachineInstanceWorkloads alert to clear")
