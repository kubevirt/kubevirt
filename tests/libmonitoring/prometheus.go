...
func WaitForMetricValueWithLabels(client kubecli.KubevirtClient, metric string, expectedValue float64, labels map[string]string, offset int) {
	EventuallyWithOffset(offset, func() float64 {
		...
		return i
	}, 3*time.Minute, 1*time.Second).Should(BeNumerically("==", expectedValue), fmt.Sprintf("Metric %s with labels %v has value %f, not the expected %f", metric, labels, expectedValue, expectedValue))
}
