...
var _ = Describe("[crit:medium][vendor:cnv-qe@redhat.com][level:component][sig-compute] Instancetype and Preferences", decorators.SigCompute, func() {
	...
	Context("A VirtualMachine", func() {
		...
		DescribeTable("referencing a non-existent", func(vm *virtv1.VirtualMachine, expectedCondition virtv1.VirtualMachineConditionType, createResourceFunc func(string) error) {
			// Create the VM and assert that it doesn't start and has the expected condition in place
			vm, err := virtClient.VirtualMachine(testsuite.GetTestNamespace(vm)).Create(context.Background(), vm, metav1.CreateOptions{})
			Expect(err).ToNot(HaveOccurred())
			Eventually(matcher.ThisVM(vm), time.Minute, time.Second).Should(matcher.HaveConditionTrue(expectedCondition))
			...
			// Now create the missing resource and assert that the condition is removed and the VM starts
			Expect(createResourceFunc(vm.Namespace)).To(Succeed())
			Eventually(matcher.ThisVM(vm), time.Minute, time.Second).ShouldNot(matcher.HaveConditionTrue(expectedCondition))
			...
			Entry("[test_id:CNV-9086] cluster instance type should still be created and eventually start when missing resource created",
				...
				virtv1.VirtualMachineInstancetypeNotFound,
				func(_ string) error {
			...
			Entry("[test_id:CNV-9091] cluster preference should still be created and eventually start when missing resource created",
				...
				virtv1.VirtualMachinePreferenceNotFound,
				func(_ string) error {
