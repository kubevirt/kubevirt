...
var _ = Describe(SIG("[rfe_id:694][crit:medium][vendor:cnv-qe@redhat.com][level:component]Networking", decorators.Networking, func() {
	...
	Describe("Multiple virtual machines connectivity using bridge binding interface", decorators.OncePerOrderedCleanup, Ordered, func() {
		...
		Context("Common ordered container", Ordered, decorators.OncePerOrderedCleanup, func() {
			Context("with a test outbound VMI", func() {
				BeforeAll(func() {
					...
					inboundVMIWithPodNetworkSet, err = virtClient.VirtualMachineInstance(testsuite.GetTestNamespace(nil)).Create(context.Background(), inboundVMIWithPodNetworkSet, metav1.CreateOptions{})
					Expect(err).ToNot(HaveOccurred())
					inboundVMIWithCustomMacAddress, err = virtClient.VirtualMachineInstance(testsuite.GetTestNamespace(nil)).Create(context.Background(), inboundVMIWithCustomMacAddress, metav1.CreateOptions{})
					Expect(err).ToNot(HaveOccurred())
			...
			It("clients should be able to reach VM workload, with propagated IP from a pod", func() {
				...
				var err error
				inboundVMI, err = kubevirt.Client().VirtualMachineInstance(inboundVMI.Namespace).Get(context.Background(), inboundVMI.Name, metav1.GetOptions{})
				Expect(err).ToNot(HaveOccurred())
