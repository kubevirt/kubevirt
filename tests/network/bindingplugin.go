...
var _ = SIGDescribe("[Serial]network binding plugin", Serial, decorators.NetCustomBindingPlugins, func() {
	...
	Context("with domain attachment managedTap type", func() {
		...
		It("can establish communication between two VMs", func() {
			...
			const (
				linuxBridgeConfNAD = `{"apiVersion":"k8s.cni.cncf.io/v1","kind":"NetworkAttachmentDefinition","metadata":{"name":"%s","namespace":"%s"},"spec":{"config":"{ \"cniVersion\": \"0.3.1\", \"name\": \"mynet\", \"plugins\": [{\"type\": \"bridge\", \"bridge\": \"%s\"}]}"}}`
				linuxBridgeNADName = "bridge0"
			)
			bridgeNAD := fmt.Sprintf(linuxBridgeConfNAD, linuxBridgeNADName, testsuite.GetTestNamespace(nil), "br10")
			namespace := testsuite.GetTestNamespace(nil)
			...
			opts := []libvmi.Option{
				libvmi.WithInterface(primaryIface),
				libvmi.WithNetwork(libvmi.MultusNetwork("mynet1", linuxBridgeNADName)),
