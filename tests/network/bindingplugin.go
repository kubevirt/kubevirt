...
var _ = SIGDescribe("[Serial]network binding plugin", Serial, decorators.NetCustomBindingPlugins, func() {
	...
	Context("with domain attachment managedMacvtap type", func() {
		const (
			bindingName = "managed-macvtap"
			networkName = "default"
		)

		BeforeEach(func() {
			err := config.WithNetBindingPlugin(bindingName, v1.InterfaceBindingPlugin{DomainAttachmentType: v1.ManagedMacvtap})
			Expect(err).NotTo(HaveOccurred())
		})

		It("can run a virtual machine with one primary managed-macvtap interface", func() {
			var vmi *v1.VirtualMachineInstance
