...
var _ = Describe(SIG(" VirtualMachineInstance with passt network binding", func() {
	...
	Context("migration", Ordered, decorators.OncePerOrderedCleanup, func() {
		...
		DescribeTable("connectivity should be preserved", func(ipFamily k8sv1.IPFamily) {
			...
			Expect(libnet.PingFromVMConsole(migrateVMI, anotherVmiIP)).To(Succeed(), fmt.Sprintf("ping from migrated VM (%s) to static VM (%s) failed", migrateVMI.Name, anotherVMI.Name))
			Expect(libnet.PingFromVMConsole(anotherVMI, migrateVmiAfterMigIP)).To(Succeed(), fmt.Sprintf("ping from static VM (%s) to migrated VM (%s) failed", anotherVMI.Name, migrateVMI.Name))
		},
			Entry("[IPv4]", k8sv1.IPv4Protocol),
			Entry("[IPv6]", k8sv1.IPv6Protocol),
		)
