	...
	Describe("Multiple virtual machines connectivity using bridge binding interface", func() {
		...
		Context("with a test outbound VMI", func() {
			...
			DescribeTable("should be able to reach", func(vmiRef **v1.VirtualMachineInstance) {
				...
				//
				// NOTE: Alpine ping doesn't support -M do that could be used to
		...
		Context("with propagated IP from a pod", func() {
			...
			DescribeTable("should be able to reach", func(op k8sv1.NodeSelectorOperator, hostNetwork bool) {
				...
				inboundVMI, err := virtClient.VirtualMachineInstance(namespace).Create(context.Background(), libvmifact.NewAlpine(), metav1.CreateOptions{})
				Expect(err).ToNot(HaveOccurred())
		...
		Context("VirtualMachineInstance with default interface model", func() {
			BeforeEach(func() {
				...
				Expect(err).ToNot(HaveOccurred())
				outboundVMI, err = virtClient.VirtualMachineInstance(testsuite.GetTestNamespace(nil)).Create(context.Background(), libvmifact.NewAlpine(), metav1.CreateOptions{})
				Expect(err).ToNot(HaveOccurred())

				inboundVMI = libwait.WaitUntilVMIReady(inboundVMI, console.LoginToAlpine)
				outboundVMI = libwait.WaitUntilVMIReady(outboundVMI, console.LoginToAlpine)
			})
