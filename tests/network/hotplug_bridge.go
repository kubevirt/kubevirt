	...
	Context("a running VM", func() {
		...
		BeforeEach(func() {
			...
			hotPluggedVM, err = kubevirt.Client().VirtualMachine(testsuite.GetTestNamespace(nil)).Create(
				context.Background(), hotPluggedVM, metav1.CreateOptions{})
			Expect(err).ToNot(HaveOccurred())
			Eventually(matcher.ThisVM(hotPluggedVM)).WithTimeout(6 * time.Minute).WithPolling(3 * time.Second).Should(
			...
			hotPluggedVMI, err = kubevirt.Client().VirtualMachineInstance(testsuite.GetTestNamespace(nil)).Get(
				context.Background(), hotPluggedVM.Name, metav1.GetOptions{})
			Expect(err).ToNot(HaveOccurred())
		...
		DescribeTable("can be hotplugged a network interface", func(plugMethod hotplugMethod) {
			...
			Eventually(func(g Gomega) {
				updatedVMI, err := kubevirt.Client().VirtualMachineInstance(hotPluggedVMI.Namespace).Get(
					context.Background(), hotPluggedVMI.Name, metav1.GetOptions{})
				Expect(err).NotTo(HaveOccurred())
			...
			}).WithTimeout(time.Second * 30).WithPolling(time.Second * 3).Should(Succeed())
		},
		...
		DescribeTable("has connectivity over the secondary network", func(plugMethod hotplugMethod) {
			...
			anotherVmi, err = kubevirt.Client().VirtualMachineInstance(testsuite.GetTestNamespace(anotherVmi)).Create(
				context.Background(), anotherVmi, metav1.CreateOptions{})
			Expect(err).ToNot(HaveOccurred())
			Eventually(matcher.ThisVMI(anotherVmi)).WithTimeout(6 * time.Minute).WithPolling(3 * time.Second).Should(
				matcher.HaveConditionTrue(v1.VirtualMachineInstanceAgentConnected))
		...
		DescribeTable("is able to hotplug multiple network interfaces", func(plugMethod hotplugMethod) {
			...
			var err error
			hotPluggedVM, err = kubevirt.Client().VirtualMachine(testsuite.GetTestNamespace(nil)).Get(
				context.Background(), hotPluggedVM.GetName(), metav1.GetOptions{})
			Expect(err).NotTo(HaveOccurred())
			...
			EventuallyWithOffset(1, func() []v1.Network {
				var err error
				hotPluggedVMI, err = kubevirt.Client().VirtualMachineInstance(hotPluggedVMI.GetNamespace()).Get(
					context.Background(), hotPluggedVMI.GetName(), metav1.GetOptions{})
				Expect(err).NotTo(HaveOccurred())
			...
			Expect(console.RunCommand(hotPluggedVMI,
				libnet.NewLinkStateAssersionCmd(secondIfaceMac.String(), v1.InterfaceStateLinkDown),
				timeout)).To(Succeed())
		},
			Entry("In place", decorators.InPlaceHotplugNICs, inPlace),
			Entry("Migration based", decorators.WgS390x, decorators.MigrationBasedHotplugNICs, migrationBased),
...
var _ = Describe(SIG("bridge nic-hotunplug", Serial, func() {
	...
	Context("a running VM", func() {
		...
		BeforeEach(func() {
			...
			vm, err = kubevirt.Client().VirtualMachine(testsuite.GetTestNamespace(nil)).Create(
				context.Background(), vm, metav1.CreateOptions{})
			Expect(err).ToNot(HaveOccurred())
			Eventually(matcher.ThisVM(vm)).WithTimeout(6 * time.Minute).WithPolling(3 * time.Second).Should(
				matcher.HaveConditionTrue(v1.VirtualMachineInstanceAgentConnected))
...
func verifyBridgeDynamicInterfaceChange(vmi *v1.VirtualMachineInstance, plugMethod hotplugMethod) *v1.VirtualMachineInstance {
	...
	if plugMethod == migrationBased {
		...
		Eventually(matcher.ThisVMI(vmi)).WithTimeout(1 * time.Minute).WithPolling(2 * time.Second).Should(
			matcher.HaveConditionTrue(v1.VirtualMachineInstanceMigrationRequired))
...
func verifyUnpluggedIfaceClearedFromVMandVMI(namespace, vmName, netName string) (*v1.VirtualMachine, *v1.VirtualMachineInstance) {
	...
	}).WithTimeout(30 * time.Second).WithPolling(3 * time.Second).WithOffset(1).Should(Succeed())
