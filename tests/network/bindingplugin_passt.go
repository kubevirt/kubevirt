...
var _ = Describe(SIG(" VirtualMachineInstance with passt network binding plugin", decorators.NetCustomBindingPlugins, Serial, func() {
	...
	Context("TCP with lower port specification", Ordered, decorators.OncePerOrderedCleanup, func() {
		var clientVMI *v1.VirtualMachineInstance
		var serverVMI *v1.VirtualMachineInstance
	...
	Context("UDP", Ordered, decorators.OncePerOrderedCleanup, func() {
		...
		BeforeAll(func() {
			var ports = []v1.Port{
				{Port: udpPort1, Protocol: "UDP"},
				{Port: udpPort2, Protocol: "UDP"},
			}
		...
		DescribeTable("connectivity", func(udpPort int, ipFamily k8sv1.IPFamily) {
			...
			Entry("[IPv4]", udpPort1, k8sv1.IPv4Protocol),
			Entry("[IPv6]", udpPort2, k8sv1.IPv6Protocol),
		)
	})
	...
	Context("migration", Ordered, decorators.OncePerOrderedCleanup, func() {
		...
		BeforeAll(func() {
			...
			waitUntilVMIsReady(console.LoginToFedora, migrateVMI)
			waitUntilVMIsReady(console.LoginToFedora, anotherVMI)
		})
