		...
		Context("VMI connected to the pod network using bridge binding", func() {
			When("Guest Agent exists", func() {
				...
				BeforeEach(func() {
					...
					vmi, err = newFedoraWithParametrizedIfaceAndGuestAgent(libvmi.InterfaceDeviceWithBridgeBinding(), networkData)
					Expect(err).NotTo(HaveOccurred())
		...
		Context("VMI connected to the pod network using masquerade binding", func() {
			When("Guest Agent exists", func() {
				...
				BeforeEach(func() {
					...
					Expect(err).NotTo(HaveOccurred())
					tmpVmi, err := newFedoraWithParametrizedIfaceAndGuestAgent(libvmi.InterfaceDeviceWithMasqueradeBinding(), networkData)
					Expect(err).NotTo(HaveOccurred())
...
func newFedoraWithParametrizedIfaceAndGuestAgent(iface v1.Interface, networkData string) (*v1.VirtualMachineInstance, error) {
	networkData, err := libnet.CreateDefaultCloudInitNetworkData()
	if err != nil {
		return nil, err
	}
