...
var _ = Describe("[crit:medium][vendor:cnv-qe@redhat.com][level:component][sig-compute] Instancetype and Preferences", decorators.SigCompute, func() {
	...
	Context("with inferFromVolume", func() {
		...
		DescribeTable("should infer defaults from existing DataVolume", func(generateDataVolumeFunc func() *cdiv1beta1.DataVolume) {
			By("Creating a DataVolume")
			dataVolume := generateDataVolumeFunc()
			...
			Entry("without labels, DataVolumeSourceRef, DataSource and labelled PVC",
				func() *cdiv1beta1.DataVolume {
