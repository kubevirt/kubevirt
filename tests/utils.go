...
func WaitUntilVMIReadyWithNamespace(namespace string, vmi *v1.VirtualMachineInstance, expecterFactory libvmi.VMIExpecterFactory) *v1.VirtualMachineInstance {
	// Wait for VirtualMachineInstance start
	WaitForSuccessfulVMIStart(vmi)
...
func StartTCPServer(vmi *v1.VirtualMachineInstance, port int) {
	...
	resp, err := libvmi.ExpectBatchWithValidatedSend(expecter, []expect.Batcher{
		&expect.BSnd{S: "\n"},
		&expect.BExp{R: "\\$ "},
