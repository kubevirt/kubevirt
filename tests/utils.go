...
func NewRandomVMIWithEphemeralDiskAndUserdataNetworkData(containerImage string, userData string, networkData string) *v1.VirtualMachineInstance {
	vmi := NewRandomVMIWithEphemeralDisk(containerImage)
	AddCloudInitData(vmi, "disk1", userData, networkData)
	return vmi
}
...
func AddCloudInitData(vmi *v1.VirtualMachineInstance, name string, userData string, networkData string) {
	vmi.Spec.Domain.Devices.Disks = append(vmi.Spec.Domain.Devices.Disks, v1.Disk{
		Name:       name,
		VolumeName: name,
		DiskDevice: v1.DiskDevice{
