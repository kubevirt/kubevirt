	...
	Describe("Starting a VirtualMachineInstance ", func() {
		...
		Context("with an Alpine DataVolume", func() {
			...
			It("should reject additional migrations on the same VMI if the first one is not finished", func() {
				...
				tests.ExpectMigrationSuccess(virtClient, migration1, migrationWaitTime)
		...
		Context("migration monitor", func() {
			...
			It("Target pod should exit after failed migration", func() {
				...
				Eventually(func() error {
					...
					pod, err := virtClient.CoreV1().Pods(vmi.Namespace).Get(vmi.Status.MigrationState.TargetPod, metav1.GetOptions{})
					Expect(err).ToNot(HaveOccurred())
			...
			It("Migration should fail if target pod fails during target preparation", func() {
				...
				err = virtClient.CoreV1().Pods(vmi.Namespace).Delete(vmi.Status.MigrationState.TargetPod, &metav1.DeleteOptions{})
				Expect(err).ToNot(HaveOccurred())
