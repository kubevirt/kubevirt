...
var _ = Describe(SIGSerial("[rfe_id:3187][crit:medium][vendor:cnv-qe@redhat.com][level:component]Prometheus Endpoints", func() {
	...
	It("should expose kubevirt_deprecated_machine_types metric with supported types", func() {
		ip := libnet.GetIP(handlerMetricIPs, k8sv1.IPv4Protocol)

		By("Scraping the Prometheus endpoint for kubevirt_deprecated_machine_types metric")
		metrics := collectMetrics(ip, "kubevirt_deprecated_machine_types")
