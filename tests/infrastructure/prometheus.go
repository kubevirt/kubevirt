...
var _ = Describe(SIGSerial("[rfe_id:3187][crit:medium][vendor:cnv-qe@redhat.com][level:component]Prometheus Endpoints", func() {
	...
	DescribeTable("should include the storage metrics for a running VM", func(metricName, operator string) {
		...
		Entry("[test_id:4142] I/O read operations metric by using IPv4", "kubevirt_vmi_storage_iops_read_total", ">="),
		Entry("[test_id:4142] I/O write operations metric by using IPv4", "kubevirt_vmi_storage_iops_write_total", ">="),
	...
	DescribeTable("should include metrics for a running VM", func(metricSubstring, operator string) {
		...
		Entry("[test_id:4143] network metrics by IPv4", "kubevirt_vmi_network_", ">="),
		Entry("[test_id:4144] memory metrics by IPv4", "kubevirt_vmi_memory", ">="),
		Entry("[test_id:4553] vcpu wait by IPv4", "kubevirt_vmi_vcpu_wait", "=="),
		Entry("[test_id:4554] vcpu seconds by IPv4", "kubevirt_vmi_vcpu_seconds_total", ">="),
		Entry("[test_id:4556] vmi unused memory by IPv4", "kubevirt_vmi_memory_unused_bytes", ">="),
	)
	...
	Context("VMI eviction blocker status", func() {
		...
		It("[test_id:4148]should include VMI eviction blocker status for all running VMs", func() {
			metricsPayload := libmonitoring.GetKubevirtVMMetrics(pod)
