...
var _ = Describe(SIGSerial("Node-labeller", func() {
	...
	BeforeEach(func() {
		...
		hypervisorName := virt_config.GetHypervisorFromKvConfig(
			&kv.Spec.Configuration, checks.HasFeature(featuregate.ConfigurableHypervisor)).Name
		launcherRenderer := hypervisor.NewLauncherResourceRenderer(hypervisorName)
		hypervisorDevice = launcherRenderer.GetHypervisorDevice()
