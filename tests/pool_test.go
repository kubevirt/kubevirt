...
var _ = Describe("[sig-compute]VirtualMachinePool", decorators.SigCompute, func() {
	...
	It("should use ordered downscale strategy when specified", func() {
		By("Create a new VirtualMachinePool with ordered downscale policy")
		pool := newPoolFromVMI(libvmifact.NewCirros())

		// Set up ordered downscale policy
		pool.Spec.DownscalePolicy = &poolv1.DownscalePolicy{
			Strategy: "ordered", // Use the DownscaleStrategyOrdered constant value
		}
		pool.Spec.VirtualMachineTemplate.Spec.RunStrategy = pointer.P(v1.RunStrategyAlways)
