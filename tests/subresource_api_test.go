	...
	Describe("ExpandSpec subresource", func() {
		Context("instancetype", func() {
			...
			Context("with existing VM", func() {
				...
				DescribeTable("[test_id:TODO] should reject user unauthorized to access instancetypes", func(matcherFn func() *v1.InstancetypeMatcher) {
					vm := tests.NewRandomVirtualMachine(libvmi.New(), false)
					vm.Spec.Instancetype = matcherFn()
					...
					client, err := kubecli.GetKubevirtClientFromRESTConfig(
						getConfigForSA(virtCli, testsuite.SubresourceServiceAccountName),
					)
					Expect(err).ToNot(HaveOccurred())

					_, err = client.VirtualMachine(util.NamespaceTestDefault).
						GetWithExpandedSpec(vm.GetName())
					Expect(err).To(HaveOccurred())
					Expect(err).To(MatchError(matcherFn().Kind + " \"" + matcherFn().Name + "\" is forbidden: Subject not allowed to access resource"))
				},
					Entry("with VirtualMachineInstancetype", instancetypeMatcherFn),
					Entry("with VirtualMachineClusterInstancetype", clusterInstancetypeMatcherFn),
				)
			...
			Context("with passed VM in request", func() {
				...
				DescribeTable("[test_id:TODO] should reject user unauthorized to access instancetypes", func(matcherFn func() *v1.InstancetypeMatcher) {
					...
					_, err = client.ExpandSpec().ForVirtualMachine(vm)
					Expect(err).To(HaveOccurred())
					Expect(err).To(MatchError(matcherFn().Kind + " \"" + matcherFn().Name + "\" is forbidden: Subject not allowed to access resource"))
				},
					Entry("with VirtualMachineInstancetype", instancetypeMatcherFn),
		...
		Context("preference", func() {
			...
			Context("with existing VM", func() {
				...
				DescribeTable("[test_id:TODO] should reject user unauthorized to access preferences", func(matcherFn func() *v1.PreferenceMatcher) {
					// Using NewCirros() here to have some data in spec.
					vm := tests.NewRandomVirtualMachine(libvmi.NewCirros(), false)
					vm.Spec.Preference = matcherFn()
...
func getConfigForSA(virtCli kubecli.KubevirtClient, saName string) *rest.Config {
	secret, err := virtCli.CoreV1().Secrets(util.NamespaceTestDefault).Get(context.Background(), saName, metav1.GetOptions{})
	Expect(err).ToNot(HaveOccurred())
