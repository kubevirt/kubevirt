...
`, virtiofsMountPath, pvcName, virtiofsMountPath)
			...
			Expect(console.RunCommand(vmi, fmt.Sprintf("test -f %s/visible_file", virtiofsMountPath), 30*time.Second)).To(Succeed())
			Expect(console.RunCommand(vmi, fmt.Sprintf("test ! -e %s/hidden_root_file", virtiofsMountPath), 30*time.Second)).To(Succeed())
		})
	})
...
`, virtiofsMountPath, pvcName, virtiofsMountPath)
			...
			newFilePath := fmt.Sprintf("%s/new_file", virtiofsMountPath)
			Expect(console.RunCommand(vmi, fmt.Sprintf("touch %s", newFilePath), 30*time.Second)).ToNot(Succeed())
			output, err := console.RunCommandAndStoreOutput(vmi, fmt.Sprintf("touch %s 2>&1", newFilePath), 30*time.Second)
			Expect(err).ToNot(HaveOccurred())
			Expect(strings.ToLower(output)).To(ContainSubstring("read-only"))
		})
	})
})
