...
var _ = compute.SIGDescribe("[rfe_id:1177][crit:medium][vendor:cnv-qe@redhat.com][level:component] VirtualMachine subresource", func() {
	...
	It("[test_id:1529]should start a VirtualMachine only once", decorators.Conformance, func() {
		By("getting a VM")
	...
	It("[test_id:1530]should stop a VirtualMachine only once", decorators.Conformance, func() {
		By("getting a VM")
		vm := libvmi.NewVirtualMachine(libvmifact.NewGuestless())
		vm, err := virtClient.VirtualMachine(testsuite.GetTestNamespace(vm)).Create(context.Background(), vm, metav1.CreateOptions{})
		Expect(err).ToNot(HaveOccurred())
		...
		Eventually(matcher.ThisVM(vm), 360*time.Second, 1*time.Second).Should(And(Not(matcher.BeCreated()), Not(matcher.BeReady())))
