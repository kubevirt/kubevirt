...
var _ = Describe(SIG("GuestAgent", decorators.GuestAgentProbes, func() {
	...
	Context("Liveness probe with guest agent ping and annotation-paused probes", func() {
		It("should not kill the VMI when probes are paused by annotation", func() {
			...
			By("Waiting for annotation to be visible on the VMI")
			Eventually(func(g Gomega) {
				freshVMI, err := kubevirt.Client().VirtualMachineInstance(testsuite.GetTestNamespace(vmi)).
					Get(context.Background(), vmi.Name, metav1.GetOptions{})
				g.Expect(err).ToNot(HaveOccurred())
				g.Expect(freshVMI.Annotations).To(HaveKeyWithValue(v1.PauseGuestAgentProbesAnnotation, "true"))
			}).WithTimeout(30 * time.Second).
				WithPolling(1 * time.Second).
				Should(Succeed())
			...
			By("Verifying the VMI eventually fails after annotation is removed (agent still stopped)")
			Eventually(matcher.ThisVMI(vmi)).
				WithTimeout(2 * time.Minute).
				WithPolling(1 * time.Second).
				Should(Or(matcher.BeInPhase(v1.Failed), matcher.HaveSucceeded()))
