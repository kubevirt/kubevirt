...
const (
	...
	guestAgentProbePauseStateChangedTrue   = "Guest agent probe pause state changed: paused=true"
...
func waitForGuestAgentProbePausePropagation(vmi *v1.VirtualMachineInstance, virtLauncherPodName string) {
	...
	EventuallyWithOffset(1, func() (string, error) {
		...
		WithPolling(2*time.Second).
		Should(ContainSubstring(guestAgentProbePauseStateChangedTrue),
			"virt-launcher pod %s/%s never logged probe-pause state change", namespace, podName)
}
