...
var _ = Describe(SIG("VMI Memory Overhead Reporting", decorators.RequiresTwoSchedulableNodes, Serial, func() {
	...
	It("should update memory overhead after CPU hotplug changes the vCPU count", func() {
		...
		By("Triggering a live migration to apply the CPU hotplug")
		migration := libmigration.New(vmi.Name, vmi.Namespace)
		libmigration.RunMigrationAndExpectToCompleteWithDefaultTimeout(virtClient, migration)

		By("Verifying memory overhead increased after CPU hotplug")
		vmi, err = virtClient.VirtualMachineInstance(vmi.Namespace).Get(context.Background(), vmi.Name, metav1.GetOptions{})
		Expect(err).ToNot(HaveOccurred())
		Expect(vmi.Status.Memory.MemoryOverhead).ToNot(BeNil(),
			"status.memory.memoryOverhead should be set after hotplug")
		Expect(vmi.Status.Memory.MemoryOverhead.Value()).To(BeNumerically(">", overheadBeforeHotplug.Value()),
			"memory overhead should increase after adding a CPU socket")
	})
}))
