...
var _ = Describe("[sig-compute]DRA", Serial, decorators.SigCompute, decorators.DRAGPU, func() {
	...
	It("should fail when a ResourceClaimTemplate requests more than one device per request", func() {
		...
		createVFIOGPUResourceClaimTemplate(WithVfioGPUDeviceCount(3))
		...
		By("Verify the claim allocated multiple devices for the request")
		expectAllocatedDeviceCountForVMI(vmi.Name, 3)

		By("Waiting for the VMI to fail to start due to count > 1 per request")
		expectVMISyncFailed(vmi.Name, ContainSubstring("count > 1 is not supported"))
		Consistently(matcher.ThisVMI(vmi), 30*time.Second, pollingInterval).Should(Not(matcher.BeRunning()))
	})
