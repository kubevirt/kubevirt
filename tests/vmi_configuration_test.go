	...
	Describe("VirtualMachineInstance definition", func() {
		...
		Context("[Serial]with geust-to-request memory ", Serial, func() {
			...
			It("should add guest-to-memory headroom", func() {
				...
				origVmiWithoutHeadroom := libvmi.New("test-vmi-"+rand.String(5), libvmi.WithResourceMemory(guestMemoryStr))
				origVmiWithHeadroom := libvmi.New("test-vmi-"+rand.String(5), libvmi.WithResourceMemory(guestMemoryStr))
