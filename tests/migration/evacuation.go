		...
		Context("VirtualMachineInstanceEvictionRequested condition", func() {
			...
			Context("when eviction fails", func() {
				...
				It("should keep VirtualMachineInstanceEvictionRequested condition when migration fails", func() {
					...
					Consistently(matcher.ThisVMI(vmi), 30*time.Second, 1*time.Second).Should(matcher.HaveConditionTrueWithReason(v1.VirtualMachineInstanceEvictionRequested, v1.VirtualMachineInstanceReasonEvictionRequested))
