	...
	Context("during evacuation", func() {
		...
		Context("VirtualMachineInstanceEvictionRequested condition", func() {
			It("should set VirtualMachineInstanceEvictionRequested condition when VMI marked for eviction", func() {
				...
				Expect(getConditionByType(vmi, v1.VirtualMachineInstanceEvictionRequested)).To(BeNil())
				...
				Eventually(func() *v1.VirtualMachineInstanceCondition {
					...
					Expect(err).NotTo(HaveOccurred())
					return getConditionByType(vmi, v1.VirtualMachineInstanceEvictionRequested)
				}).WithTimeout(30 * time.Second).WithPolling(1 * time.Second).Should(Not(BeNil()))

				By("Verifying the condition details")
				condition := getConditionByType(vmi, v1.VirtualMachineInstanceEvictionRequested)
				Expect(condition.Status).To(Equal(k8sv1.ConditionTrue))
				Expect(condition.Reason).To(Equal(v1.VirtualMachineInstanceReasonEvictionRequested))
				Expect(condition.Message).To(Equal("VMI is marked for eviction"))
				...
				Eventually(func() *v1.VirtualMachineInstanceCondition {
					...
					Expect(err).NotTo(HaveOccurred())
					return getConditionByType(vmi, v1.VirtualMachineInstanceEvictionRequested)
				}).WithTimeout(30 * time.Second).WithPolling(1 * time.Second).Should(BeNil())
			...
			It("should handle external eviction strategy correctly with VirtualMachineInstanceEvictionRequested condition", func() {
				vmi := libvmifact.NewCirros(
					libvmi.WithInterface(libvmi.InterfaceDeviceWithMasqueradeBinding()),
					libvmi.WithNetwork(v1.DefaultPodNetwork()),
				...
				By("Verifying the condition details for external eviction")
				condition := getConditionByType(vmi, v1.VirtualMachineInstanceEvictionRequested)
				Expect(condition.Status).To(Equal(k8sv1.ConditionTrue))
				Expect(condition.Reason).To(Equal(v1.VirtualMachineInstanceReasonEvictionRequested))
				Expect(condition.Message).To(Equal("VMI is marked for eviction"))

				By("Verifying VMI is marked for eviction but no migration occurs")
				Expect(vmi.IsMarkedForEviction()).To(BeTrue())
				Expect(vmi.Status.EvacuationNodeName).To(Equal(virtLauncherPod.Spec.NodeName))

				By("Ensuring no migration is triggered for external eviction strategy")
				Consistently(func() *v1.VirtualMachineInstanceMigrationState {
					vmi, err = kubevirt.Client().VirtualMachineInstance(vmi.Namespace).Get(ctx, vmi.Name, metav1.GetOptions{})
					Expect(err).NotTo(HaveOccurred())
					return vmi.Status.MigrationState
				}).WithTimeout(30 * time.Second).WithPolling(2 * time.Second).Should(BeNil())

				By("Manually clearing EvacuationNodeName to simulate external handling")
				patchBytes := []byte(`[{"op": "remove", "path": "/status/evacuationNodeName"}]`)
				_, err = kubevirt.Client().VirtualMachineInstance(vmi.Namespace).Patch(ctx, vmi.Name, types.JSONPatchType, patchBytes, metav1.PatchOptions{}, "status")
				Expect(err).ToNot(HaveOccurred())

				By("Verifying VirtualMachineInstanceEvictionRequested condition is cleared")
				Eventually(func() *v1.VirtualMachineInstanceCondition {
					vmi, err = kubevirt.Client().VirtualMachineInstance(vmi.Namespace).Get(ctx, vmi.Name, metav1.GetOptions{})
					Expect(err).NotTo(HaveOccurred())
					return getConditionByType(vmi, v1.VirtualMachineInstanceEvictionRequested)
...
func getConditionByType(vmi *v1.VirtualMachineInstance, conditionType v1.VirtualMachineInstanceConditionType) *v1.VirtualMachineInstanceCondition {
