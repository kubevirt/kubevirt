	...
	Context("during evacuation", func() {
		It("should add eviction-in-progress annotation to source virt-launcher pod", func() {
			...
			Eventually(func() error {
				return kubevirt.Client().CoreV1().Pods(virtLauncherPod.Namespace).EvictV1(ctx, &policy.Eviction{
					ObjectMeta: metav1.ObjectMeta{
						Name:      virtLauncherPod.Name,
						Namespace: virtLauncherPod.Namespace,
					},
				})
			}).WithTimeout(60 * time.Second).WithPolling(1 * time.Second).Should(MatchError(ContainSubstring("Eviction triggered evacuation of VMI")))
		...
		Context("when evacuating fails", func() {
			...
			It("should not remove eviction-in-progress annotation from source virt-launcher pod", func() {
				...
				Eventually(func() int {
					...
					for _, migration := range migrations.Items {
						switch migration.Status.Phase {
						case v1.MigrationFailed:
							failedMigrations += 1
						}
					}
					return failedMigrations
				}).WithTimeout(60 * time.Second).WithPolling(1 * time.Second).Should(BeNumerically(">=", 1))
