...
func maxDowntimeSetString(maxDowntime uint64, factor float64) string {
	return fmt.Sprintf("max downtime set to %dms: estimated downtime ([0-9]+)ms within tolerable factor %gx to max allowed downtime %dms", maxDowntime, factor, maxDowntime)
}
func abortImpossibleString(downtime uint64, factor float64) string {
	return fmt.Sprintf("aborting migration: estimated downtime [0-9]+ms exceeds max allowed downtime %dms by a factor of more than %gx", downtime, factor)
}

func migrationTestEnv(overrides ...envutil.Binding) []envutil.Binding {
	byName := map[string]envutil.Binding{
		...
		migrations.DisableMultifd.Name:            migrations.DisableMultifd.Binding("true"),
		migrations.StallProgressTimeout.Name:      migrations.StallProgressTimeout.Binding("5"),
		migrations.SwitchoverTimeout.Name:         migrations.SwitchoverTimeout.Binding("30"),
		migrations.StallMargin.Name:               migrations.StallMargin.Binding("7"),
		migrations.EwmaAlpha.Name:                 migrations.EwmaAlpha.Binding("0.4"),
		migrations.PrecopyPossibleFactor.Name:     migrations.PrecopyPossibleFactor.Binding("1.5"),
		migrations.PatienceWindowDecayFactor.Name: migrations.PatienceWindowDecayFactor.Binding("0.5"),
		migrations.SearchLocalMinima.Name:         migrations.SearchLocalMinima.Binding("true"),
		migrations.CompletionTimeoutFactor.Name:   migrations.CompletionTimeoutFactor.Binding("2"),
	}
	for _, override := range overrides {
		byName[override.Key()] = override
	}
	env := make([]envutil.Binding, 0, len(byName))
...
var _ = Describe(SIG("Migration Stall Detection", Serial, decorators.RequiresTwoSchedulableNodes, func() {
	...
	BeforeEach(func() {
		...
		libpodmutator.CreateOrUpdateEnvConfigMap(virtClient, stallDetectionConfigMapName, migrationTestEnv(), stallTestNamespace)
	...
	Context("with natural convergence after reducing dirty rate", func() {
		It("should detect stall then converge naturally after stress is stopped", func() {
			...
			CreateMigrationPolicy(virtClient, policy)
			libpodmutator.CreateOrUpdateEnvConfigMap(virtClient, stallDetectionConfigMapName, migrationTestEnv(
				migrations.PrecopyPossibleFactor.Binding("1000"),
			), stallTestNamespace)
	...
	Context("with relaxation mechanism", func() {
		It("should relax best remaining bytes when workload ramps up after initial progress", func() {
			...
			CreateMigrationPolicy(virtClient, policy)
			libpodmutator.CreateOrUpdateEnvConfigMap(virtClient, stallDetectionConfigMapName, migrationTestEnv(
				migrations.PrecopyPossibleFactor.Binding("10000"),
				migrations.StallMargin.Binding("0"),
				migrations.CompletionTimeoutFactor.Binding("100"),
			), stallTestNamespace)
	...
	Context("with post-copy and workload disruption both disabled", func() {
		It("should detect stall and abort migration when convergence is impossible", func() {
			...
			CreateMigrationPolicy(virtClient, policy)
			libpodmutator.CreateOrUpdateEnvConfigMap(virtClient, stallDetectionConfigMapName, migrationTestEnv(
				migrations.PrecopyPossibleFactor.Binding("1"),
				migrations.CompletionTimeoutFactor.Binding("100000"),
			), stallTestNamespace)
	...
	Context("with completion timeout", func() {
		It("should force switchover when completion timeout is reached", func() {
			...
			CreateMigrationPolicy(virtClient, policy)
			libpodmutator.CreateOrUpdateEnvConfigMap(virtClient, stallDetectionConfigMapName, migrationTestEnv(
				migrations.StallProgressTimeout.Binding("600"),
				migrations.CompletionTimeoutFactor.Binding("100"),
			), stallTestNamespace)
