...
var _ = SIGMigrationDescribe("Live Migrate A Paused VMI", func() {
	...
	Describe("Starting a VirtualMachineInstance ", func() {
		...
		Context("paused vm during migration", func() {
			...
			Context("should migrate paused", func() {
				...
				Context("when acceptiable time exceeded and post-copy is forbidden", Serial, func() {
					...
					DescribeTable("should pause the VMI and ", func(expectSuccess bool, bandwidth string, settingsType applySettingsType) {
						...
						waitUntilMigrationMode(vmi, v1.MigrationPaused, 300)
						...
						By("Making sure that the VM is unpaused")
						...
						Entry("migrate successfully (CR change)", expectSuccess, "50Mi", applyWithKubevirtCR),
						Entry("fail migration", expectFailure, "1Mi", applyWithKubevirtCR),
					)
				})
			})
		})
