	...
	Context("should migrate using post-copy", func() {
		...
		Context("and fail", Serial, func() {
			...
			runVirtHandlerKillerPod := func(nodeName string) {
				...
				// Kill the handler every 15 seconds
			...
			It("and make sure VMs restart after failure", func() {
				...
				// Need to wait for cloud init to finish and start the agent inside the vmi.
				Eventually(matcher.ThisVMI(vmi), 4*time.Minute, time.Second).Should(matcher.HaveConditionTrue(v1.VirtualMachineInstanceAgentConnected))
				...
				// check VMI, confirm migration state
				libmigration.WaitUntilMigrationMode(virtClient, vmi, v1.MigrationPostCopy, 4*time.Minute)
				...
				Eventually(matcher.ThisMigration(migration), 4*time.Minute, time.Second).Should(matcher.BeInPhase(v1.MigrationFailed))

				By("Removing virt-handler killer pod")
				removeVirtHandlerKillerPod()

				By("Ensuring the VirtualMachineInstance is restarted")
				Eventually(matcher.ThisVMI(vmi), 4*time.Minute, time.Second).Should(matcher.BeRestarted(vmi.UID))
			})
		})
	})
})
