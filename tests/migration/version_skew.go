...
var _ = Describe(SIG("Live migration and virt-handler version skew", Serial, func() {
	...
	pinFakeOutdatedVirtHandlerLabel := func(nodeName string) time.Time {
		...
		Eventually(func() string {
			node, err := virtClient.CoreV1().Nodes().Get(context.Background(), nodeName, metav1.GetOptions{})
			Expect(err).ToNot(HaveOccurred())
			newBeat = node.Annotations[v1.VirtHandlerHeartbeat]
			return newBeat
		...
		// anchor on the recorded heartbeat time, not our observation time
		var beatTime metav1.Time
		Expect(json.Unmarshal([]byte(`"`+newBeat+`"`), &beatTime)).To(Succeed())
		deadline := beatTime.Time.Add(virtHandlerHeartbeatInterval)
	...
	It("allows migrating a VMI off an outdated node onto an updated one",
		decorators.RequiresTwoSchedulableNodes, func() {
			...
			libnode.SetNodeUnschedulable(updatedNode, virtClient)
			DeferCleanup(func() { libnode.SetNodeSchedulable(updatedNode, virtClient) })
			...
			By("uncordoning the updated node so the migration has somewhere to go")
			libnode.SetNodeSchedulable(updatedNode, virtClient)

			deadline := pinFakeOutdatedVirtHandlerLabel(outdatedNode)
			requireSafetyMarginBefore(deadline, 20*time.Second, "create the migration and inspect its target pod")
			...
			Expect(targetPodHasVersionSkewAffinity(createdMigration)).To(BeFalse(),
				"migrating off an outdated node must not be restricted by the version-skew guard")

			createdMigration = libmigration.ExpectMigrationToSucceed(virtClient, createdMigration, flags.MigrationTimeout())
			vmi = libmigration.ConfirmVMIPostMigration(virtClient, vmi, createdMigration)
	...
	It("refuses to migrate a VMI from an updated node onto an outdated node",
		decorators.RequiresTwoSchedulableNodes, func() {
			...
			Consistently(matcher.ThisMigration(createdMigration), 23*time.Second, 2*time.Second).Should(matcher.BeInPhase(v1.MigrationScheduling))
