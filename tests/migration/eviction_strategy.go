...
var _ = SIGMigrationDescribe("Live Migration", func() {
	...
	fedoraVMIWithEvictionStrategy := func() *v1.VirtualMachineInstance {
		strategy := v1.EvictionStrategyLiveMigrate
		opts := append(libvmi.WithMasqueradeNetworking(), libvmi.WithResourceMemory(fedoraVMSize), withEvictionStrategy(strategy), libvmi.WithNamespace(testsuite.GetTestNamespace(nil)))
		return libvmi.NewFedora(
			opts...,
		)
	}

	alpineVMIWithEvictionStrategy := func() *v1.VirtualMachineInstance {
		strategy := v1.EvictionStrategyLiveMigrate
		opts := append(libvmi.WithMasqueradeNetworking(), withEvictionStrategy(strategy), libvmi.WithNamespace(testsuite.GetTestNamespace(nil)))
	...
	Describe("with no cluster-wide eviction strategy set", func() {
			...
			It("[sig-compute][test_id:3243]should recreate the PDB if VMIs with similar names are recreated", func() {
				for x := 0; x < 3; x++ {
					...
					Eventually(func() []policyv1.PodDisruptionBudget {
						pdbs, err := virtClient.PolicyV1().PodDisruptionBudgets(createdVmi.Namespace).List(context.Background(), metav1.ListOptions{})
						Expect(err).ToNot(HaveOccurred())
						return pdbs.Items
					...
					By("deleting the VMI")
					Expect(virtClient.VirtualMachineInstance(createdVmi.Namespace).Delete(context.Background(), createdVmi.Name, &metav1.DeleteOptions{})).To(Succeed())
					...
					Eventually(func() bool {
						_, err := virtClient.VirtualMachineInstance(createdVmi.Namespace).Get(context.Background(), createdVmi.Name, &metav1.GetOptions{})
		...
		It("[release-blocker][test_id:3245][Serial]should not migrate more than two VMIs at the same time from a node", Serial, func() {
			...
			for i, vmi := range vmis {
				vmis[i], err = virtClient.VirtualMachineInstance(vmi.Namespace).Create(context.Background(), vmi)
				Expect(err).ToNot(HaveOccurred())
			}
