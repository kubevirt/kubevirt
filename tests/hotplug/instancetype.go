...
var _ = Describe("[sig-compute]Instance Type and Preference Hotplug", decorators.SigCompute, decorators.SigComputeInstancetype, decorators.SigComputeMigrations, decorators.RequiresTwoSchedulableNodes, Serial, func() {
	...
	DescribeTable("should plug extra resources from new instance type", func(withMaxGuestSockets bool) {
		...
		Expect(vm.Status.InstancetypeRef.Resources.CPU.Sockets).To(Equal(originalSockets))
		...
		Expect(vm.Status.InstancetypeRef.Resources.Memory).To(Equal(originalGuest))
		...
		Expect(vm.Status.InstancetypeRef).ToNot(BeNil())
		Expect(vm.Status.InstancetypeRef.Resources).ToNot(BeNil())
		Expect(vm.Status.InstancetypeRef.Resources.CPU.Sockets).To(Equal(maxSockets))
		Expect(vm.Status.InstancetypeRef.Resources.CPU.Cores).To(Equal(uint32(1)))
		Expect(vm.Status.InstancetypeRef.Resources.CPU.Threads).To(Equal(uint32(1)))
		Expect(vm.Status.InstancetypeRef.Resources.Memory).To(Equal(maxGuest))
	},
