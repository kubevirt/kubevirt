...
// Check if the secure execution tests should be skipped.
func checkSecureExecutionTestSkip() {
	if !decorators.SecureExecution.MatchesLabelFilter(GinkgoLabelFilter()) {
		// As these tests need a bare metal Secure Execution enabled cluster, skip if not explicitly called for.
		Skip("Not called with secure-execution label")
	}
	...
	if !hasS390XNodes {
		// Ensure this is skipped when called by flake detection, as that uses an x86_64 cluster
		Skip("Cluster has no s390x nodes")
	}
}
