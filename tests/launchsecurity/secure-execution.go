...
var _ = Describe("[sig-compute]IBM Secure Execution",
	decorators.SecureExecution, decorators.SigCompute, decorators.RequiresS390X, Serial,
	func() {
		...
		Context("Ensure cluster can run Secure Execution VMs", func() {
			...
			BeforeEach(func() {
				...
				genprotimgCmd := "genprotimg --no-verify -o /boot/sdboot " +
					"-i /boot/vmlinuz-*.s390x -r /boot/initramfs-*.s390x.img " +
					"-p /tmp/parmfile.txt -k /mnt/secex-hostkey.crt"
