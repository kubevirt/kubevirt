...
var _ = Describe("[sig-compute]AMD Secure Encrypted Virtualization (SEV)", decorators.SEV, decorators.SigCompute, func() {
	...
	verifyMeasurement := func(sevMeasurementInfo *v1.SEVMeasurementInfo, tikBase64 string) []byte {
		...
		const sevFlagsValue = 0x04
		err = binary.Write(h, binary.LittleEndian, uint8(sevFlagsValue))
		ExpectWithOffset(1, err).ToNot(HaveOccurred())
	...
	encryptSecret := func(diskSecret string, measure []byte, tikBase64, tekBase64 string) *v1.SEVSecretOptions {
		...
		const headerSize = 52
		header := bytes.NewBuffer(make([]byte, 0, headerSize))
		...
		h := hmac.New(sha256.New, tik)
		const sevSecretFlagsValue = 0x01
		err = binary.Write(h, binary.LittleEndian, uint8(sevSecretFlagsValue))
		ExpectWithOffset(1, err).ToNot(HaveOccurred())
	...
	parseVirshInfo := func(info string, expectedKeys []string) map[string]string {
		...
		for scanner.Scan() {
			...
			ExpectWithOffset(1, data).To(HaveLen(expectedParts))
	...
	prepareSession := func(virtClient kubecli.KubevirtClient, nodeName string, pdh string) (*v1.SEVSessionOptions, string, string) {
		...
		defer func() {
			delErr := virtClient.CoreV1().Pods(helperPod.Namespace).Delete(context.Background(), helperPod.Name, k8smetav1.DeleteOptions{})
			ExpectWithOffset(1, delErr).ToNot(HaveOccurred())
		}()
		...
		execOnHelperPod := func(command string) (string, error) {
			stdout, execErr := exec.ExecuteCommandOnPod(
				helperPod,
				helperPod.Spec.Containers[0].Name,
				[]string{"/bin/bash", "-c", command})
			return strings.TrimSpace(stdout), execErr
		}
	...
	Context("device management", Serial, func() {
		...
		BeforeEach(func() {
			...
			Eventually(func() bool {
				node, nodeErr := virtClient.CoreV1().Nodes().Get(context.Background(), nodeName, k8smetav1.GetOptions{})
	...
	Context("lifecycle", func() {
		...
		It("[QUARANTINE] should run guest attestation", decorators.Quarantine, func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Eventually(matcher.ThisVMI(vmi), 60).Should(And(
				matcher.BeRunning(),
				matcher.HaveConditionTrue(v1.VirtualMachineInstancePaused),
			))
