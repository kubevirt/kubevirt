...
var _ = VirtctlDescribe("[sig-storage]ImageUpload", decorators.SigStorage, Serial, func() {
	...
	BeforeEach(func() {
		...
		targetName = "alpine-" + rand.String(randLength)
	...
	DescribeTable("[test_id:4621]Upload an image and start a VMI should succeed",
		func(resource string, validateFn func(string, string), diskFn func(string, string) libvmi.Option) {
	...
	Context("Create upload archive volume", func() {
		...
		DescribeTable("[test_id:11658]fails when provisioning fails", func(resource, expected string) {
			sc := "invalid-sc-" + rand.String(randLength)
			libstorage.CreateStorageClass(sc, nil)
			err := runImageUploadCmd(
				resource, "alpine-archive-"+rand.String(randLength),
				"--archive-path", archivePath,
				"--storage-class", sc,
				"--force-bind",
			)
