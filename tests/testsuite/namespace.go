...
func CleanNamespaces() {
	...
	for _, namespace := range TestNamespaces {
		...
		stripFinalizersFromGVR(vmiGVR, namespace)
		...
		stripFinalizersFromGVR(migrationGVR, namespace)
		...
		vmtrGVR := schema.GroupVersionResource{Group: "template.kubevirt.io", Version: "v1alpha1", Resource: "virtualmachinetemplaterequests"}
		Expect(removeAllGroupVersionResourceFromNamespace(vmtrGVR, namespace)).To(Succeed())
		stripFinalizersFromGVR(vmtrGVR, namespace)

		// Remove VirtualMachineTemplates
		vmtGVR := schema.GroupVersionResource{Group: "template.kubevirt.io", Version: "v1alpha1", Resource: "virtualmachinetemplates"}
...
func stripFinalizersFromGVR(gvr schema.GroupVersionResource, namespace string) {
	virtCli := kubevirt.Client()
