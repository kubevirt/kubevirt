	...
	*/
	...
	Context("Network connectivity preserved after NAD change", Ordered, decorators.OncePerOrderedCleanup, func() {
		...
		BeforeAll(func() {
			...
			vm, vmi = createAndStartVMWithVMI(ctx, namespace, "connectivity-test-vm", vmiSpec)
			vmi = libwait.WaitUntilVMIReady(vmi, console.LoginToFedora)
		})
	...
	Context("Feature gate controls NAD update capability", Ordered, decorators.OncePerOrderedCleanup, func() {
		...
		*/
		It("[test_id:TS-CNV72329-003] should reject NAD update when feature gate is disabled", func() {
			...
			if kv.Items[0].Spec.Configuration.DeveloperConfiguration != nil {
				for _, fg := range kv.Items[0].Spec.Configuration.DeveloperConfiguration.FeatureGates {
			...
			if featureGateEnabled {
				Skip("Test requires LiveUpdateNADRef feature gate to be disabled - skipping in enabled environment")
			}
		...
		*/
		It("[test_id:TS-CNV72329-005] should handle NAD update for stopped VM appropriately", func() {
			...
			Consistently(func() bool {
				migrations, listErr := kubevirt.Client().VirtualMachineInstanceMigration(namespace).List(ctx, metav1.ListOptions{})
				if listErr != nil {
					// Fail on unexpected errors instead of masking them
					GinkgoWriter.Printf("Error listing migrations: %v\n", listErr)
					return false
				}
				for _, m := range migrations.Items {
					if m.Spec.VMIName == vm.Name {
	...
	Context("Multi-interface VM NAD update", Ordered, decorators.OncePerOrderedCleanup, func() {
		...
		BeforeAll(func() {
			...
			vm, vmi = createAndStartVMWithVMI(ctx, namespace, "multi-iface-test-vm", vmiSpec)
			vmi = libwait.WaitUntilVMIReady(vmi, console.LoginToFedora)
		})
...
// createAndStartVMWithVMI creates a VM, starts it, and returns both the VM and its VMI.
// This ensures consistent handling of VMI retrieval across all tests.
func createAndStartVMWithVMI(ctx context.Context, namespace, name string, vmiSpec *v1.VirtualMachineInstance) (*v1.VirtualMachine, *v1.VirtualMachineInstance) {
	vm, err := kubevirt.Client().VirtualMachine(namespace).Create(ctx, &v1.VirtualMachine{
		ObjectMeta: metav1.ObjectMeta{
			Name:      name,
			Namespace: namespace,
