...
if [[ -z ${KUBEVIRT_E2E_FOCUS} && -z ${KUBEVIRT_E2E_SKIP} && -z ${label_filter} ]]; then
  ...
  if [[ $TARGET =~ windows_sysprep.* ]]; then
    ...
    label_filter='(sig-compute-realtime) && !(SEV, SEVES, secure-execution, CCA)'
  elif [[ $TARGET =~ sig-compute-migrations ]]; then
    label_filter='(sig-compute-migrations && !(GPU,VGPU)) && !(SEV, SEVES, secure-execution, CCA)'
  elif [[ $TARGET =~ sig-compute-serial ]]; then
    export KUBEVIRT_E2E_PARALLEL=false
    label_filter='((sig-compute && Serial) && !(GPU,VGPU,DRA-GPU,sig-compute-migrations) && !(SEV, SEVES, secure-execution, CCA))'
  elif [[ $TARGET =~ sig-compute-parallel ]]; then
    label_filter='(sig-compute && !(Serial,GPU,VGPU,sig-compute-migrations,sig-storage,storage-req) && !(SEV, SEVES, secure-execution, CCA))'
    ...
    label_filter='(sig-compute && !(GPU,VGPU,sig-compute-migrations,sig-storage,DRA-GPU) && !(SEV, SEVES, secure-execution, CCA))'
