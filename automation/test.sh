...
if [[ -z ${KUBEVIRT_E2E_FOCUS} && -z ${KUBEVIRT_E2E_SKIP} && -z ${label_filter} ]]; then
  ...
  if [[ $TARGET =~ windows_sysprep.* ]]; then
    ...
    export KUBEVIRT_E2E_PARALLEL=false
    label_filter='((sig-compute && Serial) && !(GPU,VGPU,sig-compute-migrations) && !(SEV, SEVES, secure-execution, CCA))'
    ...
    label_filter='(sig-compute && !(GPU,VGPU,sig-compute-migrations,sig-storage) && !(SEV, SEVES, secure-execution, CCA))'
