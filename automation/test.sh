...
if [[ -z ${KUBEVIRT_E2E_FOCUS} && -z ${KUBEVIRT_E2E_SKIP} && -z ${label_filter} ]]; then
  ...
  if [[ $TARGET =~ windows_sysprep.* ]]; then
    ...
    label_filter='(sig-compute && !(Serial,GPU,VGPU,sig-compute-migrations,sig-storage,storage-req) && !(SEV, SEVES))'
  elif [[ $TARGET =~ sig-compute-conformance ]]; then
  ...
  if [[ $KUBEVIRT_PROVIDER =~ k8s ]] && (echo $KUBEVIRT_PROVIDER | grep -i -E "(1\.3[1-4])"); then
