...
if [[ -z ${KUBEVIRT_E2E_FOCUS} && -z ${KUBEVIRT_E2E_SKIP} && -z ${label_filter} ]]; then
  ...
  if [[ $TARGET =~ windows_sysprep.* ]]; then
    ...
    export KUBEVIRT_E2E_PARALLEL=false
    label_filter='((sig-compute && Serial) && !(GPU,VGPU,DRA-GPU,sig-compute-migrations,sig-compute-migrations-stall) && !(SEV, SEVES, secure-execution))'
    ...
    label_filter='(sig-compute && !(GPU,VGPU,sig-compute-migrations,sig-compute-migrations-stall,sig-storage,DRA-GPU) && !(SEV, SEVES, secure-execution))'
