...
function up() {
    ...
    (
        ...
        ./cluster-hack/local-up-cluster.sh
    )
}

function prepare_config() {
    cat >cluster-hack/config-provider-local.sh <<EOF
master_ip=$(_main_ip)
docker_tag=devel
...
function build() {
    ${KUBEVIRT_PATH}cluster-hack/dockerized "DOCKER_TAG=${DOCKER_TAG}
    KUBEVIRT_PROVIDER=${KUBEVIRT_PROVIDER} ./cluster-hack/build-manifests.sh"
    make docker
}
