...
function dind::run() {
    ...
    docker run \
        ...
        -e KUBEVIRT_PROVIDER="external" \
        -e KUBECONFIG="/etc/kubernetes/admin.conf" \
        -e DOCKER_PREFIX="kubevirt" \
        ...
        -l "${DIND_LABEL}" \
        -v "${volume_name}:/dind" \
        -v ${KUBEVIRT_FOLDER}:/go/src/kubevirt.io/kubevirt \
        -v /dev/kvm:/dev/kvm \
        -v /dev/vfio:/dev/vfio \
        ${opts[@]+"${opts[@]}"} \
        "${DIND_IMAGE}" \
...
function dind::init() {
    ...
    *) # Includes 1.13 master branch
    ...
    for e in $(
        set -o posix
        set | grep -E "^APISERVER_[a-z_]+=" | cut -d'=' -f 1
    ); do
    ...
    for e in $(
        set -o posix
        set | grep -E "^CONTROLLER_MANAGER_[a-z_]+=" | cut -d'=' -f 1
    ); do
    ...
    for e in $(
        set -o posix
        set | grep -E "^SCHEDULER_[a-z_]+=" | cut -d'=' -f 1
    ); do
