...
if [[ -z $TARGET ]] || [[ $TARGET =~ .*-dev ]]; then
    for manifest in ${MANIFESTS_OUT_DIR}/dev/{rbac-operator.authorization.k8s.yaml,kv-resource.yaml,virt-operator.yaml}; do
        _kubectl create -f $manifest
    done
elif [[ $TARGET =~ .*-release ]] || [[ $TARGET =~ windows.* ]]; then
    for manifest in ${MANIFESTS_OUT_DIR}/release/{kubevirt-operator.yaml}; do
        _kubectl create -f $manifest
    done
fi
