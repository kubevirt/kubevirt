...
type AdditionalVirtHandlerConfig struct {
    ...
    // NodePlacement describes scheduling configuration for the additional
    // virt-handler pods. The NodeSelector is also used to match VMIs
    // to determine which virt-launcher image to use.
...
A VMI matches an additional handler if the VMI's `spec.nodeSelector` contains all key-value pairs from the handler's `nodePlacement.nodeSelector`. This is a superset match - the VMI may have additional selectors beyond those in the handler configuration.
