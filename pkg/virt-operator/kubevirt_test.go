...
var _ = Describe("KubeVirt Operator", func() {
	...
	Context("On valid KubeVirt object", func() {
		...
		It("should apply the infra placement config on the install job", func() {
			...
			}, util.GetTargetConfigFromKV(kv, kvTestData.virtClient))
			Expect(err).ToNot(HaveOccurred())
		...
		It("should label install strategy creation job", func() {
			...
			job, err := kvTestData.controller.generateInstallStrategyJob(nil, util.GetTargetConfigFromKV(kv, kvTestData.virtClient))
			Expect(err).ToNot(HaveOccurred())
		...
		It("should delete install strategy creation job if job has failed", func() {
			...
			job, err := kvTestData.controller.generateInstallStrategyJob(&v1.ComponentConfig{}, util.GetTargetConfigFromKV(kv, kvTestData.virtClient))
			Expect(err).ToNot(HaveOccurred())
		...
		It("should not delete completed install strategy creation job if job has failed less that 10 seconds ago", func() {
			...
			job, err := kvTestData.controller.generateInstallStrategyJob(kv.Spec.Infra, util.GetTargetConfigFromKV(kv, kvTestData.virtClient))
			Expect(err).ToNot(HaveOccurred())
