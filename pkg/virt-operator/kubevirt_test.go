...
const (
	...
	resourceCount = 89 + virtTemplateResourceCount
	patchCount    = 57 + virtTemplatePatchCount
	...
	configMapCount             = 2 + externalCAConfigMapCount // kubevirt-ca + kubevirt-export-ca + kubevirt-external-ca
	externalCAConfigMapCount   = 1

	// virtTemplateResourceCount consists of 30 objects from the bundled template + 3 secrets + 2 PDBs
	virtTemplateResourceCount = 35
	virtTemplatePatchCount    = 18
...
func virtTemplateDeploymentEnabled(kv *v1.KubeVirt) bool {
	...
	return kv.Spec.Configuration.VirtTemplateDeployment == nil ||
		kv.Spec.Configuration.VirtTemplateDeployment.Enabled == nil ||
		*kv.Spec.Configuration.VirtTemplateDeployment.Enabled
}
...
func appendVirtTemplateResources(all []runtime.Object, resources *components.VirtTemplateResources) []runtime.Object {
	...
	for _, obj := range resources.Deployments {
		all = append(all, obj)
		all = append(all, components.NewPodDisruptionBudgetForDeployment(obj))
	}
...
func (k *KubeVirtTestData) makeDeploymentsReady(kv *v1.KubeVirt) {
	...
	if virtTemplateDeploymentEnabled(kv) {
		deployments = append(deployments, "/virt-template-apiserver", "/virt-template-controller")
	}
...
func (k *KubeVirtTestData) makePodDisruptionBudgetsReady(kv *v1.KubeVirt) {
	...
	if virtTemplateDeploymentEnabled(kv) {
		pdbs = append(pdbs, "/virt-template-apiserver-pdb", "/virt-template-controller-pdb")
	}
...
var _ = Describe("KubeVirt Operator", func() {
	...
	Context("On valid KubeVirt object", func() {
		...
		It("should add resources on create", func() {
			...
			// 10 in total are yet missing at this point
			...
			// also virt-template-apiserver + PDB and virt-template-controller + PDB
			expectedUncreatedResources := 10

			Expect(kvTestData.totalAdds).To(Equal(resourceCount - expectedUncreatedResources + expectedTemporaryResources + configMapCount))
		...
		It("should pause rollback until api server is rolled over.", func() {
			...
			// 11 because 2 for virt-controller service and deployment,
			...
			// also virt-template-apiserver + pdb and virt-template-controller + PDB
			Expect(kvTestData.totalPatches).To(Equal(patchCount - 11))
		...
		It("should pause update after daemonsets are rolled over", func() {
			...
			// 12 because virt-controller, virt-api, PDBs and the namespace are not patched
			// also virt-exportproxy and pdb and route
			// also virt-template-apiserver + PDB and virt-template-controller + PDB
			Expect(kvTestData.totalPatches).To(Equal(patchCount - 12))
		...
		It("should pause update after controllers are rolled over", func() {
			...
			// 11 because virt-handler, virt-api, PDB and the namespace should not be patched
			// also virt-exportproxy and pdb and route
			// also virt-template-apiserver + PDB and virt-template-controller + PDB
			Expect(kvTestData.totalPatches).To(Equal(patchCount - 11))
		...
		DescribeTable("should update kubevirt resources when Operator version changes if no imageTag and imageRegistry is explicitly set.", func(withExport bool, patchCount, resourceCount, numPDBs int) {
			...
			Entry("without export", false, patchCount-1-3, resourceCount-3, 4),
			Entry("with export", true, patchCount-1, resourceCount, 5),
		)
		...
		DescribeTable("should patch poddisruptionbudgets when changing KubeVirt version.", func(withExport bool, numPDBs int) {
			...
			Entry("without export", false, 4),
			Entry("with export", true, 5),
		)
