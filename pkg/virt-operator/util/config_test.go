...
var _ = Describe("Operator Config", func() {
	...
	Context("kubelet root directory configuration", func() {
		...
		It("should validate that KubeletRootDir is absolute", func() {
			// Note: relative paths are rejected by the API validation pattern ^/.*
			// This test verifies that only absolute paths would be accepted.
			// The API validation ensures this, so this test documents the expectation.
		})
