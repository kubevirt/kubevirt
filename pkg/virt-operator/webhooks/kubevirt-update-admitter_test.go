...
var _ = Describe("Validating KubeVirtUpdate Admitter", func() {
	...
	Context("validateCPUModel", func() {
		...
		DescribeTable("should accept valid values without querying nodes", func(cpuModel string) {
			causes := validateCPUModel(context.Background(), cpuModel, mockClient)
		...
		It("should accept a CPU model that exists on cluster nodes", func() {
			...
			causes := validateCPUModel(context.Background(), "Haswell", mockClient)
			Expect(causes).To(BeEmpty())
		})
		...
		It("should reject a CPU model not supported by any node", func() {
			...
			causes := validateCPUModel(context.Background(), "Homer-Simpson", mockClient)
			Expect(causes).To(HaveLen(1))
		...
		It("should reject a misspelled CPU model", func() {
			...
			causes := validateCPUModel(context.Background(), "CascadelakeServer", mockClient)
			Expect(causes).To(HaveLen(1))
			Expect(causes[0].Message).To(ContainSubstring("CascadelakeServer"))
		})

		It("should not fail when client is nil", func() {
			causes := validateCPUModel(context.Background(), "SomeModel", nil)
			Expect(causes).To(BeEmpty())
		})
	})
