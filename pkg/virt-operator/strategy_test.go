...
var _ = Describe("Install Strategy Cache Idempotency", func() {
	It("should not mutate cached strategy across multiple patch reconciliations", func() {
		...
		mockEnvVarManager := &operatorutil.EnvVarManagerMock{}
		Expect(mockEnvVarManager.Setenv(operatorutil.OldOperatorImageEnvName, "quay.io/kubevirt/virt-operator:v1.0.0")).To(Succeed())
		origEnvVarManager := operatorutil.DefaultEnvVarManager
		operatorutil.DefaultEnvVarManager = mockEnvVarManager
		DeferCleanup(func() {
			operatorutil.DefaultEnvVarManager = origEnvVarManager
		})
		config := operatorutil.GetTargetConfigFromKVWithEnvVarManager(kv, mockEnvVarManager)
		...
		// Verify cached strategy is a clean baseline copy before patching
		for _, ds := range cached2.DaemonSets() {
			if ds.Name == components.VirtHandlerName {
				Expect(ds.Spec.Template.Spec.Containers[0].Args).ToNot(ContainElement("--custom-flag"),
					"cached strategy must return an unmutated baseline copy")
			}
		}
