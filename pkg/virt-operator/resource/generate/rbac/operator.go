...
func getKubeVirtComponentsClusterRules() []rbacv1.PolicyRule {
	...
	// During upgrades the operator reconciles ClusterRoles generated by the
	// previous release's install-strategy job. Kubernetes RBAC escalation
	// prevention requires the operator to hold every permission it grants.
	// These rules cover permissions that older releases included but the
	// current release no longer needs. To be removed in a following release.
	backwardsCompatRules := []rbacv1.PolicyRule{
		{
	...
	rules = append(rules, backwardsCompatRules...)
		...
		Rules: []rbacv1.PolicyRule{
			{
				...
				ResourceNames: []string{
					...
					components.VirtHandlerMonitoringClientCertSecretName,
