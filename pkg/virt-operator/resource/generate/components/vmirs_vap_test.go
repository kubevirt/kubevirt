...
var _ = Describe("VirtualMachineInstanceReplicaSet Validation Admission Policy", func() {
	Context("ValidatingAdmissionPolicy", func() {
		It("should generate the expected policy", func() {
			...
			Expect(messages).To(ContainElements(
				components.VMIRSErrTemplateRequired,
				components.VMIRSErrSelectorRequired,
				components.VMIRSErrSelectorMatchesLabels,
			))
		})
	...
	Context("ValidatingAdmissionPolicyBinding", func() {
		It("should generate the expected policy binding", func() {
			...
			Expect(binding.Name).To(Equal("vmirs-policy-binding"))
