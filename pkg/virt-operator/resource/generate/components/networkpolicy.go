...
func NewKubeVirtNetworkPolicies(namespace string) []*networkv1.NetworkPolicy {
	return []*networkv1.NetworkPolicy{
		newApiServerNP(namespace),
		newToDNSNP(namespace),
...
func newApiServerNP(namespace string) *networkv1.NetworkPolicy {
...
func newToDNSNP(namespace string) *networkv1.NetworkPolicy {
	return newNetworkPolicy(
		namespace,
		allowToDNS,
		&networkv1.NetworkPolicySpec{
			...
			Egress: []networkv1.NetworkPolicyEgressRule{
				{
					...
					Ports: []networkv1.NetworkPolicyPort{
						{
							Port:     pointer.P(intstr.FromString("dns-tcp")),
							Protocol: pointer.P(k8sv1.ProtocolTCP),
						},
						{
							Port:     pointer.P(intstr.FromString("dns")),
