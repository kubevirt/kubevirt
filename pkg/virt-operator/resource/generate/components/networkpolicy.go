...
const (
	allowEgressToApiServer            = "kubevirt-allow-egress-to-api-server"
	allowToDNS                        = "kubevirt-allow-to-dns"
...
var virtComponentsSelector = metav1.LabelSelector{
...
func NewKubeVirtNetworkPolicies(
	namespace,
	apiNamespace,
	apiLabelKey,
	apiLabelValue,
	dnsNamespace,
	dnsLabelKey,
	dnsLabelValue string) []*networkv1.NetworkPolicy {
	return []*networkv1.NetworkPolicy{
		newApiServerNP(namespace, apiNamespace, apiLabelKey, apiLabelValue),
		newToDNSNP(namespace, dnsNamespace, dnsLabelKey, dnsLabelValue),
...
func newApiServerNP(namespace, apiNamespace, apiLabelKey, apiLabelValue string) *networkv1.NetworkPolicy {
	return newNetworkPolicy(
		namespace,
		allowEgressToApiServer,
		&networkv1.NetworkPolicySpec{
			PodSelector: virtComponentsSelector,
...
func newToDNSNP(namespace, dnsNamespace, dnsLabelKey, dnsLabelValue string) *networkv1.NetworkPolicy {
	return newNetworkPolicy(
		namespace,
		allowToDNS,
		&networkv1.NetworkPolicySpec{
			...
			Egress: []networkv1.NetworkPolicyEgressRule{
				{
					...
					Ports: []networkv1.NetworkPolicyPort{
						...
						{
							Protocol: pointer.P(k8sv1.ProtocolUDP),
						},
					},
				},
			},
...
func newVirtApiWebhookNP(namespace string) *networkv1.NetworkPolicy {
	return newNetworkPolicy(
		...
		&networkv1.NetworkPolicySpec{
			PodSelector: metav1.LabelSelector{
				MatchLabels: map[string]string{"kubevirt.io": "virt-api"},
			},
			PolicyTypes: []networkv1.PolicyType{networkv1.PolicyTypeIngress},
			Ingress: []networkv1.NetworkPolicyIngressRule{
				{
			...
			Egress: []networkv1.NetworkPolicyEgressRule{
				{
					To: []networkv1.NetworkPolicyPeer{
						{
							PodSelector: &metav1.LabelSelector{
								MatchLabels: map[string]string{"kubevirt.io": "virt-handler"},
							},
						},
					},
					Ports: []networkv1.NetworkPolicyPort{
						{
							Port:     pointer.P(intstr.FromInt32(8186)),
							Protocol: pointer.P(k8sv1.ProtocolTCP),
						},
					},
				},
...
func newVirtOperatorWebhookNP(namespace string) *networkv1.NetworkPolicy {
	return newNetworkPolicy(
		...
		&networkv1.NetworkPolicySpec{
			PodSelector: metav1.LabelSelector{
				MatchLabels: map[string]string{"kubevirt.io": "virt-operator"},
			},
			PolicyTypes: []networkv1.PolicyType{networkv1.PolicyTypeIngress},
			Ingress: []networkv1.NetworkPolicyIngressRule{
				{
...
func newExportProxyNP(namespace string) *networkv1.NetworkPolicy {
	return newNetworkPolicy(
		...
		&networkv1.NetworkPolicySpec{
			PodSelector: metav1.LabelSelector{
				MatchLabels: map[string]string{"kubevirt.io": "virt-exportproxy"},
			},
