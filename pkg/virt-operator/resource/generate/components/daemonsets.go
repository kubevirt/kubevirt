...
func NewHandlerDaemonSet(config *operatorutil.KubeVirtDeploymentConfig, productName, productVersion, productComponent string) *appsv1.DaemonSet {
	...
	podTemplateSpec := newPodTemplateSpec(deploymentName, imageName, config.GetImageRegistry(), config.GetHandlerVersion(), productName, productVersion, productComponent, config.VirtHandlerImage, config.GetImagePullPolicy(), config.GetImagePullSecrets(), nil, env)
