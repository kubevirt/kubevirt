...
func NewWorkerPoolDaemonSet(primaryDS *appsv1.DaemonSet, pool *workerv1.WorkerPool, namespace string) *appsv1.DaemonSet {
	...
	ds.Labels[workerv1.WorkerPoolLabel] = pool.Name

	if ds.Spec.Template.Labels == nil {
		ds.Spec.Template.Labels = make(map[string]string)
	}
	ds.Spec.Template.Labels[workerv1.WorkerPoolLabel] = pool.Name

	// Update the selector to include the pool label
	ds.Spec.Selector = &metav1.LabelSelector{
		MatchLabels: map[string]string{
			"kubevirt.io":            VirtHandlerName,
			workerv1.WorkerPoolLabel: pool.Name,
		},
	}

	// Override virt-handler image if specified
	if pool.Spec.VirtHandlerImage != "" && len(ds.Spec.Template.Spec.Containers) > 0 {
		ds.Spec.Template.Spec.Containers[0].Image = pool.Spec.VirtHandlerImage
	}

	// Apply the pool's nodeSelector
	ds.Spec.Template.Spec.NodeSelector = pool.Spec.NodeSelector
...
// pool DaemonSets.
func ApplyPoolAntiAffinityToPrimaryHandler(primaryDS *appsv1.DaemonSet, pools []workerv1.WorkerPool) {
	if len(pools) == 0 {
		return
	}
	...
	for _, pool := range pools {
		for key, value := range pool.Spec.NodeSelector {
