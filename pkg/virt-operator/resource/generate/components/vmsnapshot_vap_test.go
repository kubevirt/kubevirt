...
var _ = Describe("VirtualMachineSnapshot Validation Admission Policy", func() {
	...
	Context("ValidatingAdmissionPolicy", func() {
		It("should generate the expected policy", func() {
			...
			Expect(messages).To(ContainElements(
				components.VMSnapshotErrFeatureGateDisabled,
				components.VMSnapshotErrMissingAPIGroup,
				components.VMSnapshotErrInvalidAPIGroup,
				components.VMSnapshotErrInvalidKind,
				components.VMSnapshotErrSpecImmutable,
			))
		})
	})
	...
	Context("ValidatingAdmissionPolicyBinding", func() {
		It("should generate the expected policy binding", func() {
			...
			Expect(binding.Name).To(Equal("vmsnapshot-policy-binding"))
