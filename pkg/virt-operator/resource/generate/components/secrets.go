...
const (
	...
	VirtHandlerMonitoringClientCertSecretName         = "kubevirt-virt-handler-monitoring-client-certs"
...
var populationStrategy = map[string]CertificateCreationCallback{
	...
	VirtHandlerMonitoringClientCertSecretName: func(secret *k8sv1.Secret, caCert *tls.Certificate, duration time.Duration) (cert *x509.Certificate, key *ecdsa.PrivateKey) {
		caKeyPair := &triple.KeyPair{
			Key:  caCert.PrivateKey.(*ecdsa.PrivateKey),
			Cert: caCert.Leaf,
		}
		keyPair, _ := triple.NewClientKeyPair(
			caKeyPair,
			"kubevirt.io:system:client:monitoring",
			nil,
			duration,
		)
		return keyPair.Cert, keyPair.Key
...
func NewCertSecrets(installNamespace string, operatorNamespace string) []*k8sv1.Secret {
	secrets := []*k8sv1.Secret{
		...
		},
		{
			...
			ObjectMeta: metav1.ObjectMeta{
				Name:      VirtHandlerMonitoringClientCertSecretName,
				Namespace: installNamespace,
				Labels: map[string]string{
					v1.ManagedByLabel: v1.ManagedByLabelOperatorValue,
				},
