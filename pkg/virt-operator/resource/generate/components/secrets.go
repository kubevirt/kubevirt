...
func filterValidCertificates(certs []*x509.Certificate, now time.Time) []*x509.Certificate {
...
func sortCertificatesByAge(certs []*x509.Certificate) []*x509.Certificate {
...
func limitCertificateCount(certs []*x509.Certificate, maxCount int) []*x509.Certificate {
	if len(certs) > maxCount {
		log.Log.Warningf("more than %d CA certificates found in the CA bundle, truncating to %d", maxCount, maxCount)
		return certs[:maxCount]
	}
	return certs
}
...
func MergeCABundle(currentCert *tls.Certificate, currentBundle []byte, overlapDuration time.Duration) ([]byte, int, error) {
	...
	now := time.Now()
	validCerts := filterValidCertificates(certs, now)
	validCerts = sortCertificatesByAge(validCerts)
	validCerts = limitCertificateCount(validCerts, maxCertificatesInBundle)
