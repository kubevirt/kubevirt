...
// Target per-pod maximum active transfers for the gated max metric (load-test
// capacity). Soft admission allows headroom above this value (see
// admission.SoftTransferLimit) so kubevirt_exportproxy_active_transfers_pod_max
// can exceed the HPA target by more than the default 10% tolerance before 429.
