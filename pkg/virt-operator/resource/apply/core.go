...
func (r *Reconciler) syncKubevirtNamespaceLabels() error {
	...
	_, err = r.k8sClient.CoreV1().Namespaces().Patch(context.Background(),
		targetNamespace,
...
func (r *Reconciler) getSecret(secret *corev1.Secret) (*corev1.Secret, bool, error) {
	...
	cachedSecret, err := r.k8sClient.CoreV1().Secrets(secret.Namespace).Get(context.Background(), secret.Name, metav1.GetOptions{})
	if err != nil {
...
func (r *Reconciler) createOrUpdateCertificateSecret(queue workqueue.TypedRateLimitingInterface[string], ca *tls.Certificate, secret *corev1.Secret, duration *metav1.Duration, renewBefore *metav1.Duration, caRenewBefore *metav1.Duration) (*tls.Certificate, error) {
	...
	if !exists {
		...
		_, err := r.k8sClient.CoreV1().Secrets(secret.Namespace).Create(context.Background(), secret, metav1.CreateOptions{})
		if err != nil {
	...
	_, err = r.k8sClient.CoreV1().Secrets(secret.Namespace).Patch(context.Background(), secret.Name, types.JSONPatchType, patchBytes, metav1.PatchOptions{})
	if err != nil {
...
func (r *Reconciler) cleanupExternalCACerts(configMap *corev1.ConfigMap) error {
	...
	if !exists {
		_, err := r.k8sClient.CoreV1().ConfigMaps(configMap.Namespace).Create(context.Background(), configMap, metav1.CreateOptions{})
		...
		_, err := r.k8sClient.CoreV1().ConfigMaps(configMap.Namespace).Update(context.Background(), configMap, metav1.UpdateOptions{})
		if err != nil {
...
func (r *Reconciler) createOrUpdateKubeVirtCAConfigMap(queue workqueue.TypedRateLimitingInterface[string], caCert *tls.Certificate, externalCACerts []*tls.Certificate, overlapInterval *metav1.Duration, configMap *corev1.ConfigMap) (caBundle []byte, err error) {
	...
	_, err = r.k8sClient.CoreV1().ConfigMaps(configMap.Namespace).Patch(context.Background(), configMap.Name, types.JSONPatchType, patchBytes, metav1.PatchOptions{})
	if err != nil {
...
func (r *Reconciler) updateSynchronizationAddress() (err error) {
	...
	if len(ips) == 0 && pod.Status.PodIPs != nil &&
		!(r.isDecentralizedLiveMigrationProxyEnabled() && r.isCrossClusterMigrationNetworkConfigured()) {
