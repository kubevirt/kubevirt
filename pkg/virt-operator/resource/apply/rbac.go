...
func (r *Reconciler) createOrUpdateClusterRole(cr *rbacv1.ClusterRole, imageTag string, imageRegistry string, id string) error {
	...
	crCopy := cr.DeepCopy()
	disableAggregation := r.kv.Spec.Configuration.AggregateToDefaultRoles != nil && !*r.kv.Spec.Configuration.AggregateToDefaultRoles
	if disableAggregation {
		// Remove aggregate labels when explicitly set to false
