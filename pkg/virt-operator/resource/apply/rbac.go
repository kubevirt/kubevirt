...
func (r *Reconciler) createOrUpdateClusterRole(cr *rbacv1.ClusterRole, imageTag string, imageRegistry string, id string) error {
	...
	crCopy := cr.DeepCopy()
	disableAggregation := r.kv.Spec.Configuration.RoleAggregationStrategy != nil && *r.kv.Spec.Configuration.RoleAggregationStrategy == v1.RoleAggregationStrategyManual
	if disableAggregation {
		// Remove aggregate labels when RoleAggregationStrategy is set to Manual
		removeAggregateLabels(crCopy)
		// Check if ClusterRole already exists - if so, add removal markers for MergeMap
		_, exists, _ := r.stores.ClusterRoleCache.Get(crCopy)
		if exists {
