...
func (r *Reconciler) syncDaemonSet(daemonSet *appsv1.DaemonSet) (bool, error) {
	...
	if daemonSet.GetName() == components.VirtHandlerName {
		...
		// Additional virt-handler: find matching config and use its nodePlacement
		...
		if handlerConfig := findAdditionalHandler(kv, suffix); handlerConfig != nil {
			componentConfig := &v1.ComponentConfig{NodePlacement: handlerConfig.NodePlacement}
...
func injectAdditionalHandlerAntiAffinity(kv *v1.KubeVirt, podSpec *corev1.PodSpec) {
	...
	for _, handler := range kv.Spec.AdditionalVirtHandlers {
		if handler.NodePlacement == nil {
			continue
		}
		// Add anti-affinity for each node selector key-value pair
		for key, value := range handler.NodePlacement.NodeSelector {
