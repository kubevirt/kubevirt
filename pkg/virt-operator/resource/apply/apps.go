...
// Excludes Env which may contain field references that resolve differently.
...
func daemonSetCoreSpecMismatch(cached, desired *appsv1.DaemonSet) string {
	...
	if mismatch := containersSpecMismatch(cachedSpec.InitContainers, desiredSpec.InitContainers); mismatch != "" {
		return fmt.Sprintf("initContainer: %s", mismatch)
	}
	return ""
}
...
func (r *Reconciler) processCanaryUpgrade(cachedDaemonSet, newDS *appsv1.DaemonSet, forceUpdate bool) (bool, error, CanaryUpgradeStatus) {
	...
	specMismatch := daemonSetCoreSpecMismatch(cachedDaemonSet, newDS)
	if specMismatch != "" && isVersionUpToDate {
		log.Warningf("detected spec modification in daemonset %s (%s), will revert to expected configuration", cachedDaemonSet.Name, specMismatch)
	}
