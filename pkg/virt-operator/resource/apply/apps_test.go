	...
	Describe("on updating/creating virt-handler", func() {
		...
		Context("updating virt-handler", func() {
			...
			DescribeTable("process canary upgrade",
				...
				// Note: Env var modifications are not detected because Env vars may contain
				// field references that resolve differently between expected and actual DaemonSets.
