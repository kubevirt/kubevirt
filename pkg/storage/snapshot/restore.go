...
func (t *vmRestoreTarget) handleVMStateClone(restoredVM *kubevirtv1.VirtualMachine) (bool, error) {
	...
	job, err := t.controller.getJob(restoredVM.Namespace, jobName)
	if err != nil {
...
func (t *vmRestoreTarget) createVMStateCloneJob(restoredVM *kubevirtv1.VirtualMachine, snapshotVM *snapshotv1.VirtualMachine, jobName, backendPVCName string) (bool, error) {
...
func (ctrl *VMRestoreController) getJob(namespace, name string) (*batchv1.Job, error) {
	objKey := cacheKeyFunc(namespace, name)
	obj, exists, err := ctrl.JobInformer.GetStore().GetByKey(objKey)
	if err != nil {
		return nil, err
	}
