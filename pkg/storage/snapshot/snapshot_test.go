...
var _ = Describe("Snapshot controlleer", func() {
	...
	Context("One valid Snapshot controller given", func() {
		...
		Context("with VolumeSnapshot and VolumeSnapshotContent informers", func() {
			...
			It("should set paused indication when VM is paused with guest agent", func() {
				vm := createLockedVM()
				vmSource.Add(vm)

				vmi := createVMI(vm)
				// Add both paused and guest agent conditions
				...
				expectedIndications := []snapshotv1.Indication{
					snapshotv1.VMSnapshotOnlineSnapshotIndication,
					snapshotv1.VMSnapshotPausedIndication,
				}
