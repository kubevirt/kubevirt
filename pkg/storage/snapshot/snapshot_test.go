		...
		Context("with VolumeSnapshot and VolumeSnapshotContent informers", func() {
			...
			DescribeTable("should complete lock source", func(vmiExists bool) {
				...
				if vmiExists {
					...
					updatedSnapshot.Status.SourceIndications = []snapshotv1.SourceIndications{
						{
							Indication: snapshotv1.VMSnapshotNoGuestAgentIndication,
							Message:    getIndicationMessage(snapshotv1.VMSnapshotNoGuestAgentIndication),
						},
						{
							Indication: snapshotv1.VMSnapshotOnlineSnapshotIndication,
							Message:    getIndicationMessage(snapshotv1.VMSnapshotOnlineSnapshotIndication),
						},
					}
				}
				updateStatusCalls := expectVMSnapshotUpdateStatus(vmSnapshotClient, updatedSnapshot)
				...
				updatedSnapshot.Status.SourceIndications = []snapshotv1.SourceIndications{
					{
						...
						Message:    getIndicationMessage(snapshotv1.VMSnapshotGuestAgentIndication),
					},
					{
						Indication: snapshotv1.VMSnapshotOnlineSnapshotIndication,
						Message:    getIndicationMessage(snapshotv1.VMSnapshotOnlineSnapshotIndication),
					},
					{
						...
						Message:    getIndicationMessage(snapshotv1.VMSnapshotQuiesceFailedIndication),
					},
				}
				updatedSnapshot.Status.Conditions = []snapshotv1.Condition{
					newProgressingCondition(corev1.ConditionFalse, "In error state"),
			...
			It("should not unset QuiesceFailed indication if freeze succeeded afterwards", func() {
				...
				vmSnapshot.Status.SourceIndications = []snapshotv1.SourceIndications{
					{
						Indication: snapshotv1.VMSnapshotGuestAgentIndication,
						Message:    getIndicationMessage(snapshotv1.VMSnapshotGuestAgentIndication),
					},
			...
			DescribeTable("should set appropriate indications based on VM state",
				func(vmiCondition *v1.VirtualMachineInstanceCondition, expectedIndications []snapshotv1.Indication) {
					...
					var expectedSourceIndications []snapshotv1.SourceIndications
					for _, indication := range expectedIndications {
						expectedSourceIndications = append(expectedSourceIndications, snapshotv1.SourceIndications{
