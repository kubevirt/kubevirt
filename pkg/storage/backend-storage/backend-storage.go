...
func IsPVCReady(vmi *corev1.VirtualMachineInstance, client kubecli.KubevirtClient, scInformer cache.SharedIndexInformer) (bool, error) {
	if !IsBackendStorageNeededForVMI(&vmi.Spec) {
		return true, nil
	}
	...
	case v1.ClaimPending:
		...
		obj, exists, err := scInformer.GetStore().GetByKey(*pvc.Spec.StorageClassName)
		if err != nil {
			return false, err
		}
		if !exists {
