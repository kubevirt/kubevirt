...
var _ = Describe("Storage Hotplug Admitter", func() {
	...
	DescribeTable("Should return proper admission response", testHotplugResponse,
		...
		Entry("Should reject if we add a disk without a matching volume",
			...
			makeFilesystems(),
			makeStatus(1, 0),
			makeExpected("volume volume-name-1 doesn't have a matching disk or filesystem", "")),
	...
	Context("with filesystem devices", func() {
		...
		DescribeTable("Should return proper admission response", testHotplugResponse,
			...
			Entry("Should accept existing hotplug volume backed by filesystem instead of disk",
				makeVolumes(0, 1),
				makeVolumes(0, 1),
				makeDisks(0),
				makeDisks(0),
				...
				makeStatus(1, 0),
				nil),
			Entry("Should accept new hotplug volume backed by filesystem instead of disk",
				makeVolumes(0, 1),
				makeVolumes(0),
				makeDisks(0),
				makeDisks(0),
