...
func GetCBTState(status *v1.ChangedBlockTrackingStatus) v1.ChangedBlockTrackingState {
	if status == nil {
...
func CompareCBTState(status *v1.ChangedBlockTrackingStatus, state v1.ChangedBlockTrackingState) bool {
	return GetCBTState(status) == state
}
...
func enableChangedBlockTrackingVMIExists(vm *v1.VirtualMachine, vmi *v1.VirtualMachineInstance) {
	vmState := GetCBTState(vm.Status.ChangedBlockTracking)
	vmiState := GetCBTState(vmi.Status.ChangedBlockTracking)
...
func disableChangedBlockTracking(vm *v1.VirtualMachine, vmi *v1.VirtualMachineInstance) {
	...
	switch GetCBTState(vm.Status.ChangedBlockTracking) {
