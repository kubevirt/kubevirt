...
const (
	...
	backupSourceNoChangedBlockTracking   = "BackupSourceNoChangedBlockTracking"
	...
	backupSourceNameEmptyMsg      = "Source name is empty for backup %s/%s"
	backupDeletingMsg             = "Backup %s/%s is being deleted"
)
...
func (ctrl *VMBackupController) sync(backup *backupv1.VirtualMachineBackup) *SyncInfo {
	...
	if sourceName == "" {
		log.Log.Errorf(backupSourceNameEmptyMsg, backup.Namespace, backup.Name)
	...
	log.Log.Object(vmi).Infof("Starting backup %s with mode %s", backup.Name, backupOptions.Mode)
	...
	log.Log.Object(vmi).Info("Started backup command successfully")
...
func (ctrl *VMBackupController) verifyBackupSource(backup *backupv1.VirtualMachineBackup, sourceName string) (*v1.VirtualMachineInstance, *SyncInfo) {
	...
	if vmi.Status.ChangedBlockTracking == nil {
		log.Log.Errorf("vm %s has no ChangedBlockTracking, cannot start backup", sourceName)
		return nil, &SyncInfo{
			event:  backupSourceNoChangedBlockTracking,
			reason: fmt.Sprintf("vm %s has no ChangedBlockTracking, cannot start backup", sourceName),
		}
	}
