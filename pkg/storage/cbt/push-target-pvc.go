...
func (ctrl *VMBackupController) backupTargetPVCDetached(vmi *v1.VirtualMachineInstance) bool {
	if vmi == nil {
		return true
	}
	for _, volumeStatus := range vmi.Status.VolumeStatus {
...
func (ctrl *VMBackupController) detachBackupTargetPVC(vmi *v1.VirtualMachineInstance) *SyncInfo {
	if len(vmi.Spec.UtilityVolumes) == 0 {
		return nil
	}
