...
func (c *MemoryCalculator) Calculate(vmi *v1.VirtualMachineInstance) resource.Quantity {
	totalMemory := resource.Quantity{}

	if !c.clusterConfig.IncrementalBackupEnabled() || !HasCBTStateEnabled(vmi.Status.ChangedBlockTracking) {
		return totalMemory
	}
	...
	trackerCount := c.backupTrackerCountForVMI(vmi.Name, vmi.Namespace)
	logger := log.Log.Object(vmi)
	for _, q := range c.cbtVolumeCapacities(vmi.Spec.Volumes, vmi.Namespace) {
	...
	logger.V(3).Infof("CBT memory overhead for VMI %s/%s: %s (including %s buffer)", //nolint:mnd
		vmi.Namespace, vmi.Name, totalMemory.String(), bufferOverhead)
	return totalMemory
}

func (c *MemoryCalculator) cbtVolumeCapacities(volumes []v1.Volume, namespace string) []resource.Quantity {
	var capacities []resource.Quantity
	for _, vol := range volumes {
		if !IsCBTEligibleVolume(&vol) {
			continue
		}
		if vol.HostDisk != nil {
			if !vol.HostDisk.Capacity.IsZero() {
				capacities = append(capacities, vol.HostDisk.Capacity)
			}
			continue
		}
		...
		if q, ok := pvc.Status.Capacity[k8sv1.ResourceStorage]; ok {
			capacities = append(capacities, q)
		} else if q := pvc.Spec.Resources.Requests.Storage(); q != nil {
			capacities = append(capacities, *q)
		}
	}
	return capacities
...
func (c *MemoryCalculator) backupTrackerCountForVMI(name, namespace string) int64 {
	trackers, err := c.backupTrackerInformer.GetIndexer().ByIndex("vmi", controller.NamespacedKey(namespace, name))
	if err != nil {
