...
func newTestServer(token string) *exportServer {
	config := ExportServerConfig{
		...
		VmHandler: func([]VolumeInfo, func() (string, error), func() (*v1.ConfigMap, error)) http.Handler {
			return http.HandlerFunc(successHandler)
		},
...
var _ = Describe("exportserver", func() {
	DescribeTable("should handle", func(vmURI string, vi *VolumeInfo, uri string) {
		token := "foo"
		es := newTestServer(token)
		es.Paths = &ExportPaths{VMURI: vmURI}
		if vi != nil {
			es.Paths.Volumes = []VolumeInfo{*vi}
		}
		es.initHandler()
	...
	DescribeTable("should handle (query param version)", func(vmURI string, vi *VolumeInfo, uri string) {
		token := "foo"
		es := newTestServer(token)
		es.Paths = &ExportPaths{VMURI: vmURI}
		if vi != nil {
	...
	DescribeTable("should fail bad token", func(vmURI string, vi *VolumeInfo, uri string) {
		token := "foo"
		es := newTestServer(token)
		es.Paths = &ExportPaths{VMURI: vmURI}
		if vi != nil {
	...
	DescribeTable("should fail bad token (query param version)", func(vmURI string, vi *VolumeInfo, uri string) {
		token := "foo"
		es := newTestServer(token)
		es.Paths = &ExportPaths{VMURI: vmURI}
		if vi != nil {
	...
	Context("Vm handler", func() {
		...
		DescribeTable("Secret handler should return error on non GET", func(verb string) {
			...
			Expect(err).ToNot(HaveOccurred())
			handler := vmHandler([]VolumeInfo{}, getBasePath, getCaConfigMap)
		...
		It("Should return 500 if path returns error", func() {
			...
			resp := httptest.NewRecorder()
			Expect(err).ToNot(HaveOccurred())
			handler := vmHandler([]VolumeInfo{}, getErrorBasePath, getCaConfigMap)
			handler.ServeHTTP(resp, req)
			Expect(resp.Code).To(BeEquivalentTo(http.StatusInternalServerError))
		})
		...
		It("Should return 500 if reading CAConfigMap returns error", func() {
			...
			resp := httptest.NewRecorder()
			Expect(err).ToNot(HaveOccurred())
			handler := vmHandler([]VolumeInfo{}, getBasePath, getErrorCaConfigMap)
			handler.ServeHTTP(resp, req)
			Expect(resp.Code).To(BeEquivalentTo(http.StatusInternalServerError))
		})
		...
		It("Should return 500 if path returns other error", func() {
			...
			resp := httptest.NewRecorder()
			Expect(err).ToNot(HaveOccurred())
			handler := vmHandler([]VolumeInfo{}, getErrorBasePath, getInternalCAConfigMap)
			handler.ServeHTTP(resp, req)
			Expect(resp.Code).To(BeEquivalentTo(http.StatusInternalServerError))
		})
