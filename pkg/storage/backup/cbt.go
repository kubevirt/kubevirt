...
func enableChangedBlockTrackingVMIExists(vm *v1.VirtualMachine, vmi *v1.VirtualMachineInstance) {
	vmState := vm.Status.ChangedBlockTracking
	vmiState := vmi.Status.ChangedBlockTracking
...
func disableChangedBlockTracking(vm *v1.VirtualMachine, vmi *v1.VirtualMachineInstance) {
	...
	// No action needed if already undefined
	if cbtState == v1.ChangedBlockTrackingUndefined {
		return
	}

	// Disable immediately if no VMI or already disabled
	...
	case v1.ChangedBlockTrackingPendingRestart,
		...
		if vmiHasCBTState(vmi) {
			// VMI still has CBT active - need restart to disable
			vm.Status.ChangedBlockTracking = v1.ChangedBlockTrackingPendingRestart
		} else {
			// VMI has no CBT state - can disable immediately
			vm.Status.ChangedBlockTracking = v1.ChangedBlockTrackingDisabled
		}
...
// vmiHasCBTState checks if VMI has any CBT state set (not undefined)
func vmiHasCBTState(vmi *v1.VirtualMachineInstance) bool {
	return vmi.Status.ChangedBlockTracking != v1.ChangedBlockTrackingUndefined
}
...
func SetChangedBlockTrackingOnVMI(vm *v1.VirtualMachine, vmi *v1.VirtualMachineInstance, clusterConfig *virtconfig.ClusterConfig, nsStore cache.Store) {
	...
	} else if vm.Status.ChangedBlockTracking != v1.ChangedBlockTrackingUndefined {
