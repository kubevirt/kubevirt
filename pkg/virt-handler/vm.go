...
func (c *VirtualMachineController) calculateLiveMigrationCondition(vmi *v1.VirtualMachineInstance) (*v1.VirtualMachineInstanceCondition, bool) {
	...
	if vmiContainsNonMigratablePCIHostDevices(vmi, c.clusterConfig.LibvirtHooksServerAndClientEnabled()) {
...
func vmiContainsNonMigratablePCIHostDevices(vmi *v1.VirtualMachineInstance, libvirtHooksServerAndClientEnabled bool) bool {
	var nonMigratableGPUs bool
	if len(vmi.Spec.Domain.Devices.GPUs) == 1 {
		if !libvirtHooksServerAndClientEnabled  || !strings.HasPrefix(vmi.Spec.Domain.Devices.GPUs[0].DeviceName, "nvidia.com") {
	...
	return len(vmi.Spec.Domain.Devices.HostDevices) > 0 || nonMigratableGPUs
}
