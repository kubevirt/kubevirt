...
func (c *VirtualMachineController) updateHotplugVolumeStatus(vmi *v1.VirtualMachineInstance, volumeStatus v1.VolumeStatus, specVolumeMap map[string]bool) (v1.VolumeStatus, bool) {
	needsRefresh := false
...
func (c *VirtualMachineController) updateVolumeStatusesFromDomain(vmi *v1.VirtualMachineInstance, domain *api.Domain) bool {
	...
	specVolumeMap := make(map[string]bool)
	for _, volume := range vmi.Spec.Volumes {
		specVolumeMap[volume.Name] = true
	}
	for _, utilityVolume := range vmi.Spec.UtilityVolumes {
		specVolumeMap[utilityVolume.Name] = true
	}
