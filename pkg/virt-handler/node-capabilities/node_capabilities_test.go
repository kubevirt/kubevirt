...
var _ = Describe("node-capabilities", func() {
	...
	Describe("Host capabilities", func() {
		It("should be able to read the TSC timer frequency from the host", func() {
			capabilities, err := nodecapabilities.HostCapabilities(hostCapabilitiesXML)
		...
		It("should read the NUMA topology from the host", func() {
			...
			capabilities, err := nodecapabilities.HostCapabilities(hostCapablitiesWithNUMAXML)
			Expect(err).ToNot(HaveOccurred())
	...
	Describe("Domain capabilities", func() {
		It("should return correct cpu models", func() {
			domainCapabilities, err := nodecapabilities.DomCapabilities(domainCapabilitiesXML)
			Expect(err).ToNot(HaveOccurred())

			cpuFeatures, err := nodecapabilities.SupportedFeatures(supportedFeaturesXML, runtime.GOARCH)
			Expect(err).ToNot(HaveOccurred())

			supportedCPUs, err := nodecapabilities.SupportedHostCPUs(domainCapabilities.CPU.Modes, runtime.GOARCH)
			Expect(err).ToNot(HaveOccurred())
		...
		It("No cpu model is usable", func() {
			domainCapabilities, err := nodecapabilities.DomCapabilities(domainCapabilitiesNothingUsableXML)
			Expect(err).ToNot(HaveOccurred())
		...
		It("Should return the cpu features on s390x even without policy='require' property", func() {
			cpuFeatures, err := nodecapabilities.SupportedFeatures(s390xSupportedFeaturesXML, "s390x")
			Expect(err).ToNot(HaveOccurred())
		...
		It("Should return the cpu features on amd64 only with policy='require' property", func() {
			cpuFeatures, err := nodecapabilities.SupportedFeatures(s390xSupportedFeaturesXML, "amd64")
			Expect(err).ToNot(HaveOccurred())
		...
		It("Should default to IBM as CPU Vendor on s390x if none is given", func() {
			domainCapabilities, err := nodecapabilities.DomCapabilities(s390xDomainCapabilitiesXML)
			Expect(err).ToNot(HaveOccurred())

			supportedCPUs, err := nodecapabilities.SupportedHostCPUs(domainCapabilities.CPU.Modes, "s390x")
		...
		Context("return correct SEV capabilities", func() {
			DescribeTable("for SEV and SEV-ES", func(domCapabilitiesXML string) {
				domCapabilities, err := nodecapabilities.DomCapabilities(domCapabilitiesXML)
				Expect(err).ToNot(HaveOccurred())
