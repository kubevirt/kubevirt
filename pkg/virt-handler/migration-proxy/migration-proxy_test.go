...
var _ = Describe("MigrationProxy", func() {
	...
	Describe("migration proxy", func() {
		...
		Context("createUnixListener source safepath", func() {
			...
			It("fails when migrationproxy directory cannot be created", func() {
				const sourceRelativePath = "/isolated-parent/migrationproxy/source.sock"

				parentDir := filepath.Join(tmpDir, "isolated-parent")
				...
				Expect(os.Symlink(externalDir, filepath.Join(parentDir, "migrationproxy"))).To(Succeed())
...
func newTestUnixProxy(mountRoot *safepath.Path, targetAddress string) *migrationProxy {
