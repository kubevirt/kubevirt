...
func (d *VirtualMachineController) prepareTap(vmi *v1.VirtualMachineInstance, res isolation.IsolationResult) error {
	networkToTapDeviceNamesMap, err := getTapDevices(vmi)
	if err != nil {
		return err
	}
	for networkName, tap := range networkToTapDeviceNamesMap {
		path, err := FindInterfaceIndexPath(res, tap, networkName, vmi.Spec.Networks)
		if err != nil {
			return err
		}
...
// If path not found it will try to find the path using ordinal pod interface name based on its position in the given
