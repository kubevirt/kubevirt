...
func (s *socketBasedIsolationDetector) DetectForSocket(vm *v1.VirtualMachineInstance, socket string) (IsolationResult, error) {
	var pid, ppid, dedicatedCpuContainerPid int
	var dedicatedCpuContainerPidExists bool
	var err error
	...
	if vmiCpu := vm.Spec.Domain.CPU; vmiCpu != nil && vmiCpu.DedicatedCPUPlacement {
		dedicatedCpuContainerPid, dedicatedCpuContainerPidExists, err = s.detectDedicatedCgroupPid(vm)
		if err != nil {
			log.Log.Object(vm).Reason(err).Errorf("could not get dedicated cpu pid for vmi %s/%s", vm.Namespace, vm.Name)
		}
	}

	if dedicatedCpuContainerPidExists {
		return NewIsolationResultWithDedicatedCpuPid(pid, ppid, dedicatedCpuContainerPid), nil
	} else {
...
func (s *socketBasedIsolationDetector) detectDedicatedCgroupPid(vmi *v1.VirtualMachineInstance) (pid int, exists bool, err error) {
	socketPath, err := cmdclient.FindSocketOnHostWithSocketName(vmi, cmdclient.DedicatedCpuContainerSocketName, false)
	if err != nil {
		return
	}
