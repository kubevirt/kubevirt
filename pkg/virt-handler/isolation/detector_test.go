...
var _ = Describe("Isolation Detector", func() {
	...
	Context("With an existing socket", func() {
		...
		Context("emulator container ambassador socket", func() {
			var stop func()

			BeforeEach(func() {
				vm.Spec.Domain.CPU = &v1.CPU{DedicatedCPUPlacement: true}

				socketPath := cmdclient.SocketFilePathOnHostWithName(podUID, cmdclient.EmulatorContainerSocketName)
				stop = createListeningSocket(socketPath)
			})
			...
			It("should detect the emulator container ambassador's PID", func() {
				result, err := NewSocketBasedIsolationDetector(tmpDir).Detect(vm)
				Expect(err).ToNot(HaveOccurred())

				pid, exists := result.EmulatorAmbassadorPid()
