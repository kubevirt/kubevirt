...
type HotplugMounter interface {
	...
	MoundAndVerifyFromPod(vmi *v1.VirtualMachineInstance, sourceUID types.UID) (map[string]*disk.DiskInfo, error)
...
func (m *hotplugMounter) MoundAndVerifyFromPod(vmi *v1.VirtualMachineInstance, sourceUID types.UID) (map[string]*disk.DiskInfo, error) {
	return m.mountAndVerify(vmi, sourceUID)
}

func (m *hotplugMounter) MountAndVerify(vmi *v1.VirtualMachineInstance) (map[string]*disk.DiskInfo, error) {
	return m.mountAndVerify(vmi, "")
}

func (m *hotplugMounter) mountAndVerify(vmi *v1.VirtualMachineInstance, sourceUID types.UID) (map[string]*disk.DiskInfo, error) {
	virtLauncherUID := m.findVirtlauncherUID(vmi)
	if virtLauncherUID == "" {
		return nil, nil
	}
	...
	for _, volume := range vmi.Spec.Volumes {
		if volume.ContainerDisk != nil && volume.ContainerDisk.Hotpluggable {
			...
			if err := disk.VerifyImage(imageInfo); err != nil {
...
func ExecChroot(args ...string) *exec.Cmd {
	return exec.Command("/usr/bin/virt-chroot", args...)
}
