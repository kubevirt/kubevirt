...
const (
	failedCheckMountPoint = "failed to check mount point for containerDisk %v: %v"
	failedUnmount         = "failed to unmount containerDisk %v: %v : %v"
)
	...
	if vmi.UID != "" {
		for _, file := range files {
			...
			if mounted, err := isolation.NodeIsolationResult().IsMounted(path); err != nil {
				return fmt.Errorf(failedCheckMountPoint, path, err)
			} else if mounted {
				// #nosec No risk for attacket injection. Parameters are predefined strings
				out, err := virt_chroot.UmountChroot(path).CombinedOutput()
				if err != nil {
					return fmt.Errorf(failedUnmount, path, string(out), err)
				}
			}
		}
	...
	unmount := func(targetDir string, artifactPaths ...string) error {
		for _, artifactPath := range artifactPaths {
			...
			if mounted, err := isolation.NodeIsolationResult().IsMounted(targetPath); err != nil {
				return fmt.Errorf(failedCheckMountPoint, targetPath, err)
			} else if mounted {
				...
				if err != nil {
					return fmt.Errorf(failedUnmount, targetPath, string(out), err)
				}
			}
		}
		return nil
