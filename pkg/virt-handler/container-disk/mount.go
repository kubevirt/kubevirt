...
func NewMounter(isoDetector isolation.PodIsolationDetector, mountStateDir string, clusterConfig *virtconfig.ClusterConfig, hostName string) Mounter {
	return &mounter{
		...
		bindMountNeededDetector:    containerdisk.NewBindMountNeededDetector("", hostName),
...
func (m *mounter) ComputeChecksums(vmi *v1.VirtualMachineInstance) (*DiskChecksums, error) {
	...
	// If the bind mount isn't needed, it means the imageVolume featureGate is enabled,
