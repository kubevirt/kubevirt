...
func (c *MigrationTargetController) finalCleanup(vmi *v1.VirtualMachineInstance, oldLabels map[string]string, domain *api.Domain) error {
	// At this point, we can usually clean up our (target) pod, since the migration is over.
	...
	// However, we can't just do nothing, because a failed VMI will be followed by the deletion of its pods, and our pod
	// won't be deletable if it still has things like volumes custom-mounted to it.
	// So in this case, we do all the cleanup but just don't tell virt-launcher yet to keep the pod, and therefore domain, alive.
	// We'll re-enqueue ourselves to finish cleaning up if the VMI gets updated and we're still around.
	keepPodRunning := domain != nil && domain.Status.Status == api.Paused && domain.Status.Reason == api.ReasonPausedPostcopyFailed && vmi.Status.Phase == v1.Running
	...
	return c.updateVMI(vmi, &vmi.Status, oldLabels, false)
}
...
func (c *MigrationTargetController) sync(vmi *v1.VirtualMachineInstance, domain *api.Domain) error {
	...
	if vmi.Status.MigrationState != nil && vmi.Status.MigrationState.EndTimestamp != nil &&
		...
		return c.finalCleanup(vmi, oldLabels, domain)
	}
	...
	updateVMIErr := c.updateVMI(vmi, &oldStatus, oldLabels, !syncReEnqueued && !updateReEnqueued)
	if updateVMIErr != nil {
		return updateVMIErr
	}
...
func (c *MigrationTargetController) processVMI(vmi *v1.VirtualMachineInstance) (error, bool) {
	...
	if !isInitialized {
		...
		return goerror.New(fmt.Sprintf("Can not update a VirtualMachineInstance with unresponsive command server.")), false
	}
