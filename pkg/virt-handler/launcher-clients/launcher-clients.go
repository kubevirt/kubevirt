...
func markIrrecoverable(err error) error {
	if err == nil {
		return nil
	}
	return fmt.Errorf("%w: %w", IrrecoverableError, err)
}
...
func (l *launcherClientsManager) GetVerifiedLauncherClient(vmi *v1.VirtualMachineInstance) (client cmdclient.LauncherClient, err error) {
	...
	if err := l.verifyLauncherClient(vmi, client); err != nil {
		return client, err
	}
	return client, nil
}

func (l *launcherClientsManager) verifyLauncherClient(vmi *v1.VirtualMachineInstance, client cmdclient.LauncherClient) error {
	if err := client.Ping(); err != nil {
		log.Log.Object(vmi).Warningf("Ping vmi failed with %s", err.Error())

		if _, socketErr := cmdclient.FindSocket(vmi); socketErr != nil {
			if errors.Is(socketErr, cmdclient.ErrNoSocketFound) {
				return markIrrecoverable(socketErr)
			}
			return socketErr
		}
		return err
...
func (l *launcherClientsManager) GetLauncherClient(vmi *v1.VirtualMachineInstance) (cmdclient.LauncherClient, error) {
	...
	result, err, _ := l.connGroup.Do(string(vmi.UID), func() (interface{}, error) {
		...
		if err != nil {
			if errors.Is(err, cmdclient.ErrNoSocketFound) {
				return nil, markIrrecoverable(err)
			}
			return nil, err
		}
