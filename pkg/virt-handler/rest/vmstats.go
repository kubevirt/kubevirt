...
func (h *VMStatsHandler) GetVMStats(request *restful.Request, response *restful.Response) {
	...
	body, err := parseVMStatsRequestBody(request)
	if err != nil {
...
type vmStatsRequestBody struct {
	AllVMIs *cmdv1.VMStatsRequest            `json:"allVMIs,omitempty"`
	VMIs    map[string]*cmdv1.VMStatsRequest `json:"vmis,omitempty"`
}

func parseVMStatsRequestBody(request *restful.Request) (*vmStatsRequestBody, error) {
	body := &vmStatsRequestBody{}
...
func (b *vmStatsRequestBody) validate() error {
	if b.AllVMIs == nil && len(b.VMIs) == 0 {
		return fmt.Errorf("either allVMIs or vmis must be set")
	}

	if b.AllVMIs != nil && *b.AllVMIs == (cmdv1.VMStatsRequest{}) {
		return fmt.Errorf("at least one stats category must be requested for allVMIs")
	}

	for key, req := range b.VMIs {
		if req == nil || *req == (cmdv1.VMStatsRequest{}) {
			return fmt.Errorf("at least one stats category must be requested for %q", key)
		}
	}
...
func (b *vmStatsRequestBody) resolve(onNode map[string]*v1.VirtualMachineInstance) map[string]*cmdv1.VMStatsRequest {
	requests := make(map[string]*cmdv1.VMStatsRequest, len(onNode)+len(b.VMIs))

	if b.AllVMIs != nil {
		for key := range onNode {
			requests[key] = b.AllVMIs
		}
	}

	maps.Copy(requests, b.VMIs)
	return requests
}
