...
type NodeHookExecutor interface {
	CallNodeHooks(hookPoint pluginv1alpha1.NodeHookPoint, vmi *virtv1.VirtualMachineInstance) error
}
...
func (m *nodeHookManager) CallNodeHooks(hookPoint pluginv1alpha1.NodeHookPoint, vmi *virtv1.VirtualMachineInstance) error {
	...
	for _, obj := range items {
		...
		for _, nh := range plugin.Spec.NodeHooks {
			...
			if err != nil {
				log.Log.Object(vmi).Warningf("CEL evaluation failed for plugin %s: %v", plugin.Name, err)
				continue
			}
			if !match {
	...
	for _, match := range matches {
		...
		if err := executeHook(match.nodeHook.Socket, string(hookPoint), vmi, match.pluginName, timeout); err != nil {
...
func executeHook(socketPath, hookPoint string, vmi *virtv1.VirtualMachineInstance, pluginName string, timeout time.Duration) error {
	...
	// The nodeName is extracted from the VMI's status since the manager
	// is always running on the node that owns the VMI.
	return hooksclient.ExecuteNodeHook(client, hookPoint, vmi, vmi.Status.NodeName, timeout)
}
