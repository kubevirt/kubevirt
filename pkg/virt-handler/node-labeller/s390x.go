...
func (a archLabellerS390X) ShouldLabelNodes() bool {
	return true
}

func (a archLabellerS390X) defaultVendor() string {
	// On s390x the xml does not include a CPU Vendor, however there is only one company selling them anyway.
	return "IBM"
}

func (a archLabellerS390X) requirePolicy(policy string) bool {
	// On s390x, the policy is not set
	return policy == util.RequirePolicy || policy == ""
}

func (a archLabellerS390X) hasHostSupportedFeatures() bool {
	return true
}

func (a archLabellerS390X) supportsHostModel() bool {
	return true
}
