...
var _ = Describe("Node-labeller ", func() {
	...
	initNodeLabeller := func(kubevirt *v1.KubeVirt, virtClient *kubecli.MockKubevirtClient) *NodeLabeller {
		config, _, _ := testutils.NewFakeClusterConfigUsingKV(kubevirt)
		recorder := record.NewFakeRecorder(100)
		recorder.IncludeObject = true

		nodeLabeller, err := newNodeLabeller(config, virtClient, "testNode", k8sv1.NamespaceDefault, "testdata", recorder)
		Expect(err).ToNot(HaveOccurred())

		// Override queue to have no rate limiting because we only execute "execute" once
		// and want to assert without wait time
		nodeLabeller.queue = workqueue.NewRateLimitingQueue(&workqueue.BucketRateLimiter{Limiter: rate.NewLimiter(rate.Limit(10), 100)})
	...
	Context("", func() {
		...
		BeforeEach(func() {
			...
			shadowNode := &v1.ShadowNode{ObjectMeta: *node.ObjectMeta.DeepCopy()}
