...
func PasstSocketDirOnHost(vmi *v1.VirtualMachineInstance) (string, error) {
	return FindPodDirOnHost(vmi, func(podUID string) string {
		return fmt.Sprintf("/%s/%s/volumes/kubernetes.io~empty-dir/libvirt-runtime/qemu/run/passt",
			podsBaseDir, podUID)
	})
}
