	...
	Context("updateBackupStatus", func() {
		...
		DescribeTable("should",
			func(vmiBackupName, domainBackupName string, expectUpdate bool) {
				...
				disksJSON := `[{"volumeName":"rootdisk","diskTarget":"vda"},{"volumeName":"datadisk","diskTarget":"vdb"}]`
				domain.Spec.Metadata.KubeVirt.Backup = &api.BackupMetadata{
				...
				if expectUpdate {
					...
					Expect(vmi.Status.ChangedBlockTracking.BackupStatus.Disks).To(HaveLen(2))
					Expect(vmi.Status.ChangedBlockTracking.BackupStatus.Disks[0].VolumeName).To(Equal("rootdisk"))
					Expect(vmi.Status.ChangedBlockTracking.BackupStatus.Disks[0].DiskTarget).To(Equal("vda"))
					Expect(vmi.Status.ChangedBlockTracking.BackupStatus.Disks[1].VolumeName).To(Equal("datadisk"))
					Expect(vmi.Status.ChangedBlockTracking.BackupStatus.Disks[1].DiskTarget).To(Equal("vdb"))
				} else {
					...
					Expect(vmi.Status.ChangedBlockTracking.BackupStatus.Disks).To(BeNil())
				}
			},
