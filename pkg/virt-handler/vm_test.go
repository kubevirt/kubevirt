	...
	Context("When VirtualMachineInstance is connected to one or more networks", func() {
		...
		Context("with a secondary network", func() {
			...
			BeforeEach(func() {
				...
				err = controller.networkCacheStoreFactory.CacheForVMI(vmi).Write(secondaryInterfaceName, podCacheSecondaryInterface)
				Expect(err).ToNot(HaveOccurred())
			...
			table.DescribeTable("should report on all networks in status", func(overrideWithGuestAgent bool) {
				...
				controller.Execute()
				testutils.ExpectEvent(recorder, VMIStarted)
				Expect(len(controller.podInterfaceCache)).To(Equal(2))
			},
				table.Entry("with cached pod interface info", false),
				table.Entry("with guest agent interface info", true),
			)
		})
