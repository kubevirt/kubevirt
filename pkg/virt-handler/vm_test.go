...
var _ = Describe("VirtualMachineInstance", func() {
	...
	Context("VirtualMachineInstance controller gets informed about a Domain change through the Domain controller", func() {
		...
		It("should requeue VMI when domain UID mismatches and old launcher is still responsive", func() {
			...
			expectedKey := virtcontroller.VirtualMachineInstanceKey(vmi)

			sanityExecute()

			// The key should have been requeued via AddAfter, not dropped
			Expect(mockQueue.GetAddAfterKeys()).To(ContainElement(expectedKey),
				"expected the exact VMI key to be requeued when old launcher is responsive during UID mismatch")
		})
