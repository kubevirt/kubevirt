...
func (c *DeviceController) updatePermittedHostDevicePlugins() []Device {
	...
	if c.virtConfig.PersistentReservationEnabled() {
		d, err := NewSocketDevicePlugin(reservation.GetPrResourceName(), reservation.GetPrHelperSocketDir(), reservation.GetPrHelperSocket(), c.maxDevices, selinux.SELinuxExecutor{})
		if err != nil {
