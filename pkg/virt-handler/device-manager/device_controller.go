...
func SecureGuestCapacityDevicePlugins(nodeStore cache.Store, nodeName string) []Device {
	plugins := []Device{}

	cvmPlugin, err := NewSecureGuestCapacityDevicePlugin(nodeStore, nodeName)
	if err != nil {
		log.Log.V(3).Infof("Secure guest capacity plugin not available: %v", err)
		return plugins
	}

	log.Log.Infof("Secure guest capacity device plugin created: type=%s, capacity=%d, resource=%s, managed=%v",
		cvmPlugin.secureGuestType, cvmPlugin.capacity, cvmPlugin.resourceName, cvmPlugin.isManaged)
...
func NewDeviceController(
	...
	cvmPlugins := SecureGuestCapacityDevicePlugins(nodeStore, host)
