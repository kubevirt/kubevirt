...
var _ = Describe("Mediated Device Health check validation", func() {
	...
	BeforeEach(func() {
		...
		dpi.contract.(*MediatedDevicePlugin).socketPath = filepath.Join(workDir, "test.sock")
		dpi.server = grpc.NewServer([]grpc.ServerOption{}...)
		dpi.contract.(*MediatedDevicePlugin).deviceRoot = workDir
		dpi.contract.(*MediatedDevicePlugin).devicePath = "/dev/vfio"
	...
	It("Should return error if device directory cannot be watched", func() {
		dpi := NewMediatedDevicePlugin([]*MDEV{{iommuGroup: "0"}}, fakeMdevResourceName)
		dpi.contract.(*MediatedDevicePlugin).deviceRoot = "/nonexistent"
		dpi.contract.(*MediatedDevicePlugin).devicePath = "/dev/vfio"
