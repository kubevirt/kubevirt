...
var _ = Describe("PCI device plugin VFIO cdev allocation", func() {
	...
	It("should include VFIO cdev device specs in Allocate response", func() {
		...
		var cdevSpec *pluginapi.DeviceSpec
		for _, spec := range resp.ContainerResponses[0].Devices {
			if spec.ContainerPath == "/dev/vfio/devices/vfio0" {
				cdevSpec = spec
				break
			}
		}
		Expect(cdevSpec).ToNot(BeNil())
		Expect(cdevSpec.HostPath).To(Equal("/dev/vfio/devices/vfio0"))
		Expect(cdevSpec.ContainerPath).To(Equal(cdevSpec.HostPath))
		Expect(cdevSpec.Permissions).To(Equal("mrw"))
	})
