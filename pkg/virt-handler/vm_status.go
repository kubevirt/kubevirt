...
func (c *VirtualMachineController) updateSoftwareEmulationCondition(vmi *v1.VirtualMachineInstance, domain *api.Domain, condManager *controller.VirtualMachineInstanceConditionManager) {
	if !c.clusterConfig.CrossArchitectureVirtualizationEnabled() {
		return
	}
...
func (c *VirtualMachineController) evaluateCommonMigrationConstraints(vmi *v1.VirtualMachineInstance) []NonMigratableReason {
	...
	message, ok := vmiContainsNonMigratablePCIHostDevices(vmi, c.clusterConfig)
	if ok {
		reasons = append(reasons, NonMigratableReason{
...
func (c *VirtualMachineController) calculateLiveStorageMigrationCondition(vmi *v1.VirtualMachineInstance) *v1.VirtualMachineInstanceCondition {
	...
	for _, r := range c.evaluateCommonMigrationConstraints(vmi) {
		// SecureExecution is only a constraint for live migration, not storage live migration
		if r.Reason == v1.VirtualMachineInstanceReasonSecureExecutionNotMigratable {
			continue
		}
	...
	if reservation.HasVMIPersistentReservation(vmi) {
		multiCond.addNonMigratableReason(NonMigratableReason{
