...
var _ = Describe("MemoryDump", func() {
	...
	Context("Download of memory dump", func() {
		...
		DescribeTable("should call download memory dump with port-forward", func(extraArgs ...string) {
			vmexport.HandleHTTPGetRequestFn = func(client kubernetes.Interface, vmexport *exportv1.VirtualMachineExport, downloadUrl string, insecure bool, exportURL string, headers map[string]string) (*http.Response, error) {
