...
//nolint:funlen // the function is long because it has to handle many flags
...
//nolint:lll // the logic is clearer in one line despite being slightly over the limit
...
//nolint:lll // text specially formatted for usage
...
func (c *createVM) hasCloudInitRelevantFlags() bool {
...
func (c *createVM) cloudInitConfig(vm *v1.VirtualMachine) error {
	if !c.hasCloudInitRelevantFlags() {
		return nil
	}
...
func (c *createVM) withImportedVolume(vm *v1.VirtualMachine) error {
	for _, volume := range c.volumeImport {
		...
		if err != nil {
			return params.FlagErr(VolumeImportFlag, "%v", err)
		}
...
func parseVolumeSource(paramStr, kind string) (name, namespace string, bootOrder *uint, err error) {
	...
	if src.Source == "" {
		err = params.FlagErr(VolumeImportFlag, "src must be specified")
		return
	}
	namespace, name, err = params.SplitPrefixedName(src.Source)
	if err != nil {
		err = params.FlagErr(VolumeImportFlag, "src invalid: %w", err)
		return
	}
	if namespace == "" {
		err = params.FlagErr(VolumeImportFlag, "namespace of %s \"%s\" must be specified", kind, name)
		return
	}
	bootOrder = src.BootOrder
	return
}
