  ...
  subnets:
	...
	Context("Manifest is created successfully", func() {
		...
		DescribeTable("VM with user specified in cloud-init user data", func(userDataFn func(*v1.VirtualMachine) string, extraArgs ...string) {
			...
			Entry("with CloudInitNoCloud (explicit)", noCloudUserData, setFlag(CloudInitSourceTypeFlag, string(cloudinit.DataSourceNoCloud))),
			Entry("with CloudInitConfigDrive (explicit)", configDriveUserData, setFlag(CloudInitSourceTypeFlag, string(cloudinit.DataSourceConfigDrive))),
		)

		DescribeTable("VM with generated password in cloud-init user data", func(userDataFn func(*v1.VirtualMachine) string, extraArgs ...string) {
			args := append([]string{
				setFlag(GenPasswordFlag, "true"),
			}, extraArgs...)
			out, err := runCmd(args...)
			Expect(err).ToNot(HaveOccurred())
			vm, err := decodeVM(out)
			...
			Expect(vm.Spec.Template.Spec.Volumes).To(HaveLen(1))
			Expect(vm.Spec.Template.Spec.Volumes[0].Name).To(Equal(CloudInitDisk))
			Expect(userDataFn(vm)).To(MatchRegexp(`password: \w{12}\nchpasswd: { expire: False }`))
		...
		DescribeTable("VM password read from file in cloud-init user data", func(userDataFn func(*v1.VirtualMachine) string, extraArgs ...string) {
		...
		DescribeTable("VM with ssh key in cloud-init user data", func(argsFn func() ([]string, string), userDataFn func(*v1.VirtualMachine) string, extraArgs ...string) {
			...
			Entry("with CloudInitNoCloud (explicit) and single key", randomSingleKey, noCloudUserData, setFlag(CloudInitSourceTypeFlag, string(cloudinit.DataSourceNoCloud))),
			Entry("with CloudInitConfigDrive (explicit) and single key", randomSingleKey, configDriveUserData, setFlag(CloudInitSourceTypeFlag, string(cloudinit.DataSourceConfigDrive))),
			Entry("with CloudInitNoCLoud (default) and multiple keys in single flag", randomMultipleKeysSingleFlag, noCloudUserData),
			Entry("with CloudInitNoCLoud (explicit) and multiple keys in single flag", randomMultipleKeysSingleFlag, noCloudUserData, setFlag(CloudInitSourceTypeFlag, string(cloudinit.DataSourceNoCloud))),
			Entry("with CloudInitConfigDrive (explicit) and multiple keys in single flag", randomMultipleKeysSingleFlag, configDriveUserData, setFlag(CloudInitSourceTypeFlag, string(cloudinit.DataSourceConfigDrive))),
			Entry("with CloudInitNoCloud (default) and multiple keys in multiple flags", randomMultipleKeysMultipleFlags, noCloudUserData),
			Entry("with CloudInitNoCloud (explicit) and multiple keys in multiple flags", randomMultipleKeysMultipleFlags, noCloudUserData, setFlag(CloudInitSourceTypeFlag, string(cloudinit.DataSourceNoCloud))),
			Entry("with CloudInitConfigDrive (explicit) and multiple keys in multiple flags", randomMultipleKeysMultipleFlags, configDriveUserData, setFlag(CloudInitSourceTypeFlag, string(cloudinit.DataSourceConfigDrive))),
		)
		...
		DescribeTable("VM with specified cloud-init user data", func(userDataFn func(*v1.VirtualMachine) string, extraArgs ...string) {
			...
			Entry("with CloudInitNoCloud (explicit)", noCloudUserDataB64, setFlag(CloudInitSourceTypeFlag, string(cloudinit.DataSourceNoCloud))),
			Entry("with CloudInitConfigDrive (explicit)", configDriveUserDataB64, setFlag(CloudInitSourceTypeFlag, string(cloudinit.DataSourceConfigDrive))),
		)
		...
		DescribeTable("VM with specified cloud-init network data", func(networkDataFn func(*v1.VirtualMachine) string, extraArgs ...string) {
			...
			Entry("with CloudInitNoCloud (explicit)", noCloudNetworkDataB64, setFlag(CloudInitSourceTypeFlag, string(cloudinit.DataSourceNoCloud))),
			Entry("with CloudInitConfigDrive (explicit)", configDriveNetworkDataB64, setFlag(CloudInitSourceTypeFlag, string(cloudinit.DataSourceConfigDrive))),
		)
		...
		DescribeTable("VM with specified cloud-init user and network data", func(userDataFn func(*v1.VirtualMachine) string, networkDataFn func(*v1.VirtualMachine) string, extraArgs ...string) {
			...
			Entry("with CloudInitNoCloud (explicit)", noCloudUserDataB64, noCloudNetworkDataB64, setFlag(CloudInitSourceTypeFlag, string(cloudinit.DataSourceNoCloud))),
			Entry("with CloudInitConfigDrive (explicit)", configDriveUserDataB64, configDriveNetworkDataB64, setFlag(CloudInitSourceTypeFlag, string(cloudinit.DataSourceConfigDrive))),
		)
		...
		DescribeTable("VM with generated cloud-init user and specified network data", func(userDataFn func(*v1.VirtualMachine) string, networkDataFn func(*v1.VirtualMachine) string, extraArgs ...string) {
			...
			Entry("with CloudInitNoCloud (explicit)", noCloudUserData, noCloudNetworkDataB64, setFlag(CloudInitSourceTypeFlag, string(cloudinit.DataSourceNoCloud))),
			Entry("with CloudInitConfigDrive (explicit)", configDriveUserData, configDriveNetworkDataB64, setFlag(CloudInitSourceTypeFlag, string(cloudinit.DataSourceConfigDrive))),
		)
	...
	Describe("Manifest is not created successfully", func() {
		...
		It("Invalid argument to GenPasswordFlag", func() {
			out, err := runCmd(setFlag(GenPasswordFlag, "not-a-bool"))
			Expect(err).To(MatchError("invalid argument \"not-a-bool\" for \"--gen-password\" flag: strconv.ParseBool: parsing \"not-a-bool\": invalid syntax"))
			Expect(out).To(BeEmpty())
		})
		...
		It("GenPasswordFlag and PasswordFileFlag are mutually exclusive", func() {
			out, err := runCmd(
				setFlag(GenPasswordFlag, "true"),
				setFlag(PasswordFileFlag, "testpath"),
			)
			Expect(err).To(MatchError("if any flags in the group [gen-password password-file] are set none of the others can be; [gen-password password-file] were all set"))
			Expect(out).To(BeEmpty())
		})
		...
		DescribeTable("Invalid arguments to CloudInitSourceTypeFlag", func(sourceType string) {
			out, err := runCmd(setFlag(CloudInitSourceTypeFlag, sourceType))
			Expect(err).To(MatchError(fmt.Sprintf("failed to parse \"--cloud-init-source-type\" flag: invalid cloud init datasource type \"%s\", supported values are: noCloud, configDrive", sourceType)))
			Expect(out).To(BeEmpty())
		},
			Entry("some string", "not-a-bool"),
			Entry("float", "1.23"),
		...
		DescribeTable("CloudInitUserDataFlag and generated cloud-init config are mutually exclusive", func(flag string, arg string) {
			...
			Entry("CloudInitUserDataFlag and GenPasswordFlag", "gen-password", setFlag(GenPasswordFlag, "true")),
