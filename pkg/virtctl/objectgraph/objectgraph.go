...
func NewObjectGraphCommand() *cobra.Command {
	...
	cmd.Flags().StringToStringVar(&c.labelSelectors, "selector", map[string]string{}, "Label selectors to filter the object graph (can be specified multiple times).")
...
func usageObjectGraph() string {
  ...
  {{ProgramName}} objectgraph my-vmi --vmi

  # Include optional nodes in the object graph for a VM
  {{ProgramName}} objectgraph my-vm --include-optional
...
func (c *command) objectGraphRun(cmd *cobra.Command, args []string) error {
	...
	if len(c.labelSelectors) > 0 {
		opts.LabelSelector = &k8smetav1.LabelSelector{
