...
var _ = Describe("Add volume command", func() {
	...
	preCreatePVC := func(volName string) {
	...
	preCreateDv := func(volName string) {
		kubecli.MockKubevirtClientInstance.EXPECT().CdiClient().Return(cdiClient)
	...
	Context("addvolume cmd", func() {
		...
		DescribeTable("long volume names", func(preCreateFunc func(string)) {
			longVolumeName := k8srand.String(64)
			preCreateFunc(longVolumeName)

			nameFn := func(volumeOptions *v1.AddVolumeOptions) {
				expectedName := apimachinery.CalculateValidUniqueID(longVolumeName)
