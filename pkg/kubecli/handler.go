...
type VirtHandlerConn interface {
	...
	ConsoleURI(vm *virtv1.VirtualMachineInstance) (*url.URL, error)
...
func (v *virtHandlerConn) ConsoleURI(vm *virtv1.VirtualMachineInstance) (*url.URL, error) {
	ip, port, err := v.ConnectionDetails()
	if err != nil {
		return nil, err
	}
	return &url.URL{
		Path: fmt.Sprintf("/api/v1/namespaces/%s/virtualmachineinstances/%s/console", vm.ObjectMeta.Namespace, vm.ObjectMeta.Name),
		Host: ip + ":" + port,
	}, nil
}
