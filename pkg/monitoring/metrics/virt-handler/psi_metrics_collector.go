...
const (
	// args: QOS class, QOS class, pod UID, PSI metric
...
var (
	...
	psiMemoryPressureMetric = operatormetrics.NewGaugeVec(
	...
	psiCpuPressureMetric = operatormetrics.NewGaugeVec(
		operatormetrics.MetricOpts{
	...
	psiIoPressureMetric = operatormetrics.NewGaugeVec(
		operatormetrics.MetricOpts{
...
func psiMetricsCollectorCallback() []operatormetrics.CollectorResult {
	...
	return collectPSIMetricsFromVMIs(vmis)
}

func collectPSIMetricsFromVMIs(vmis []*k6tv1.VirtualMachineInstance) []operatormetrics.CollectorResult {
	...
	for _, vmi := range vmis {
		for podUid, _ := range vmi.Status.ActivePods {
			...
			results = append(results, collectPSIMetricsFromPod(vmi, string(podUid), qosClass)...)
		}
	}
...
func collectPSIMetricsFromPod(vmi *k6tv1.VirtualMachineInstance, podUid string, qosClass string) []operatormetrics.CollectorResult {
	var results []operatormetrics.CollectorResult

	filePath := path.Join(fmt.Sprintf(pathFormat, qosClass, qosClass, strings.ReplaceAll(podUid, "-", "_")))
	psiMemoryPressureValue, err := statPSI(filePath, psiMemoryPressure)
	if err != nil {
	...
	psiCpuPressureValue, err := statPSI(filePath, psiCpuPressure)
	if err != nil {
	...
	psiIoPressureValue, err := statPSI(filePath, psiIoPressure)
	if err != nil {
...
func statPSI(dirPath string, file string) (*PSIStats, error) {
	f, err := os.OpenFile(path.Join(dirPath, file), os.O_RDONLY, 0)
	if err != nil {
