...
var _ = Describe("Handler", func() {
	...
	Describe("completed migration results", func() {
		...
		It("should still observe downtime when the migration metadata is missing", func() {
			withoutMigration := func(downtime uint64) *api.Domain {
				domain := completedMigrationDomain(downtime, "")
				domain.Spec.Metadata.KubeVirt.Migration = nil
				return domain
			}

			h.handleDomainCompletedMigrationStats(withoutMigration(70))
			Expect(migrationDowntimeSampleCount()).To(Equal(uint64(1)))

			h.handleDomainCompletedMigrationStats(withoutMigration(70))
			Expect(migrationDowntimeSampleCount()).To(Equal(uint64(1)))

			h.handleDomainCompletedMigrationStats(withoutMigration(90))
			Expect(migrationDowntimeSampleCount()).To(Equal(uint64(2)))
		})
