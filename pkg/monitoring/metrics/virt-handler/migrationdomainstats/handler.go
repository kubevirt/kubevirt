...
func (h *handler) handleDomainCompletedMigrationStats(obj interface{}) {
	...
	isNewMigration := !exists || previousStats.vmiUID != string(vmi.UID)
	if !isNewMigration {
		if migrationUID != "" {
			isNewMigration = previousStats.migrationUID != migrationUID
		} else {
			// The UID is absent when the migration metadata is cleared before these
			// stats arrive. Downtime is then the only way to tell two migrations of
			// the same VMI apart, and is preferred over dropping the sample.
			isNewMigration = previousStats.domainJobInfo.Downtime != completedStats.Downtime
		}
	}
	if isNewMigration {
		if migrationUID == "" {
			log.Log.Warningf("observing completed migration downtime for VMI %s without a migration UID", key)
		}
