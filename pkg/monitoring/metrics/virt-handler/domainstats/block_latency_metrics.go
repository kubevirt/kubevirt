...
func collectBlockLatencyHistograms(
	...
	for _, block := range vmiReport.vmiStats.DomainStats.Block {
		...
		for _, operation := range operations {
			...
			// The histogram bins and the total-time counter must have the same
			// collection epoch. Latency histograms are therefore configured only when
			// the QEMU block backend is created. Reconfiguring a histogram on a running
			// domain resets its bins without resetting the total-time counter and would
			// make the Prometheus sample count and sum inconsistent.
...
func convertLatencyHistogram(
	histogram *stats.DomainStatsBlockLatencyHistogram,
) (buckets map[float64]uint64, count uint64, valid bool) {
