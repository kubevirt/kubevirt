...
func BenchmarkCollectBlockLatencyHistograms(b *stdtesting.B) {
	for _, testCase := range []struct {
		name       string
		diskCount  int
		histograms bool
	}{
		{name: "1_disk_without_histograms", diskCount: 1},
		{name: "1_disk_with_histograms", diskCount: 1, histograms: true},
		{name: "10_disks_without_histograms", diskCount: 10},
		{name: "10_disks_with_histograms", diskCount: 10, histograms: true},
		{name: "100_disks_without_histograms", diskCount: 100},
		{name: "100_disks_with_histograms", diskCount: 100, histograms: true},
	} {
		b.Run(testCase.name, func(b *stdtesting.B) {
			report := benchmarkBlockLatencyHistogramReport(
				testCase.diskCount, testCase.histograms,
			)
			emittedMetrics := 0
			if testCase.histograms {
				emittedMetrics = testCase.diskCount * 3
			}
			ch := make(chan prometheus.Metric, emittedMetrics)

			b.ReportAllocs()
			b.ResetTimer()
			for range b.N {
				collectBlockLatencyHistograms(report, ch)
				for range emittedMetrics {
					<-ch
				}
			}
		})
...
func benchmarkBlockLatencyHistogramReport(
	diskCount int,
	withHistograms bool,
) *VirtualMachineInstanceReport {
	blocks := make([]stats.DomainStatsBlock, diskCount)
	for i := range blocks {
		blocks[i] = stats.DomainStatsBlock{
			NameSet:    true,
			Name:       "disk",
			RdTimesSet: true,
			RdTimes:    500_000_000,
		...
		if withHistograms {
			histogram := &stats.DomainStatsBlockLatencyHistogram{
				Bins: []stats.DomainStatsBlockLatencyHistogramBin{
					...
					{StartSet: true, Start: 1_000_000, ValueSet: true, Value: 5},
					{StartSet: true, Start: 10_000_000, ValueSet: true, Value: 2},
					{StartSet: true, Start: 50_000_000, ValueSet: true, Value: 1},
					{StartSet: true, Start: 100_000_000, ValueSet: true, Value: 1},
					{StartSet: true, Start: 500_000_000, ValueSet: true, Value: 1},
					{StartSet: true, Start: 1_000_000_000, ValueSet: true, Value: 1},
					{StartSet: true, Start: 2_000_000_000, ValueSet: true, Value: 1},
				},
			}
			blocks[i].LatencyHistograms = stats.DomainStatsBlockLatencyHistograms{
				Read: histogram, Write: histogram, Flush: histogram,
			}
		}
	}

	vmi := &k6tv1.VirtualMachineInstance{
		ObjectMeta: metav1.ObjectMeta{Name: "benchmark-vmi", Namespace: "benchmark"},
		Status:     k6tv1.VirtualMachineInstanceStatus{NodeName: "benchmark-node"},
	}
	vmiStats := &VirtualMachineInstanceStats{
		DomainStats: &stats.DomainStats{Block: blocks},
	}

	return newVirtualMachineInstanceReport(vmi, vmiStats)
}
