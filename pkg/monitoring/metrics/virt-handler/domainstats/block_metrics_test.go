...
var _ = Describe("block metrics", func() {
	Context("on Collect", func() {
		...
		DescribeTable("should collect metrics values", func(metric operatormetrics.Metric, expectedValue float64) {
			...
			Entry("kubevirt_vmi_storage_allocation_bytes", storageAllocationBytes, 9.0),
			Entry("kubevirt_vmi_storage_capacity_bytes", storageCapacityBytes, 10.0),
			Entry("kubevirt_vmi_storage_physical_bytes", storagePhysicalBytes, 11.0),
		)
