...
var (
	dirtyRateMbs = operatormetrics.NewGauge(
		operatormetrics.MetricOpts{
			Name: "kubevirt_vmi_dirty_rate_mbs",
			Help: "Guest dirty-rate in MB/s.",
		},
	)
)
...
func (dirtyRateMetrics) Collect(vmiReport *VirtualMachineInstanceReport) []operatormetrics.CollectorResult {
	...
	if !dirtyRate.MegabytesPerSecondSet {
		log.Log.Warningf("No domain dirty rate stats is set for VMI %s/%s", vmiReport.vmi.Name, vmiReport.vmi.Namespace)
		return crs
	}

	crs = append(crs, vmiReport.newCollectorResult(dirtyRateMbs, float64(dirtyRate.MegabytesPerSecond)))
