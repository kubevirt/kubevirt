...
var _ = Describe("VM Stats Collector", func() {
	...
	Context("VM vNIC info", func() {
		It("should collect metrics for vNICs with various binding types, including PluginBinding", func() {
			...
			Expect(metrics[0].Labels).To(Equal([]string{"test-vm", "test-ns", "iface1", "bridge", "pod networking", "<none>"}))
			Expect(metrics[1].Labels).To(Equal([]string{"test-vm", "test-ns", "iface2", "masquerade", "pod networking", "<none>"}))
			Expect(metrics[2].Labels).To(Equal([]string{"test-vm", "test-ns", "iface3", "sriov", "multus-net", "<none>"}))
			Expect(metrics[3].Labels).To(Equal([]string{"test-vm", "test-ns", "iface4", "bindingPlugin", "custom-net", "custom-plugin"}))
		})
