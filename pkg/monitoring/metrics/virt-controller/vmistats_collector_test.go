...
var _ = Describe("VMI Stats Collector", func() {
	...
	Context("VMI vNIC info", func() {
		It("should collect kubevirt_vmi_vnic_info metric with correct labels", func() {
			...
			Expect(metrics[0].Labels).To(Equal([]string{"test-vmi", "test-ns", "iface1", "bridge", "pod networking", "<none>"}))
			Expect(metrics[1].Labels).To(Equal([]string{"test-vmi", "test-ns", "iface2", "masquerade", "pod networking", "<none>"}))
			Expect(metrics[2].Labels).To(Equal([]string{"test-vmi", "test-ns", "iface3", "sriov", "multus-net", "<none>"}))
			Expect(metrics[3].Labels).To(Equal([]string{"test-vmi", "test-ns", "iface4", "bindingPlugin", "custom-net", "custom-plugin"}))
		})
