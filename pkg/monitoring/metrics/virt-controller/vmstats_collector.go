...
func collectAllocatedMemoryValues(vm *k6tv1.VirtualMachine) []operatormetrics.CollectorResult {
	...
	if allocatedMemory != nil && !allocatedMemory.IsZero() {
		cr = append(cr, operatormetrics.CollectorResult{
			...
			Labels: []string{vm.Name, vm.Namespace, "memory", "bytes", "allocated"},
		})
	}

	// Also report allocated memory limits if they exist
	if memoryLimits := vm.Spec.Template.Spec.Domain.Resources.Limits.Memory(); !memoryLimits.IsZero() {
		cr = append(cr, operatormetrics.CollectorResult{
			Metric: vmResourceLimits,
			Value:  float64(memoryLimits.Value()),
			Labels: []string{vm.Name, vm.Namespace, "memory", "bytes", "allocated"},
		})
	}
...
func collectAllocatedCpuValues(vm *k6tv1.VirtualMachine) []operatormetrics.CollectorResult {
	...
	if vm.Spec.Template.Spec.Domain.CPU != nil {
		...
		cr = append(cr, operatormetrics.CollectorResult{
			...
			Labels: []string{vm.Name, vm.Namespace, "cpu", "cores", "allocated"},
		})
	}

	// Also report allocated CPU limits if they exist
	if cpuLimits := vm.Spec.Template.Spec.Domain.Resources.Limits.Cpu(); !cpuLimits.IsZero() {
		cr = append(cr, operatormetrics.CollectorResult{
			Metric: vmResourceLimits,
			Value:  float64(cpuLimits.MilliValue()) / 1000,
			Labels: []string{vm.Name, vm.Namespace, "cpu", "cores", "allocated"},
		})
	}
