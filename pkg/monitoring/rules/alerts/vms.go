...
var vmsAlerts = []promv1.Rule{
	...
	},
	{
		...
		Expr:  intstr.FromString("kubevirt_vmi_non_evictable * on(name, namespace) group_left() topk by(name, namespace) (1, kubevirt_vmi_info{phase='running'}) == 1"),
	...
	},
	{
		...
		Annotations: map[string]string{
			...
			descriptionAnnotationKey: "Virtual machine {{ $labels.name }} in namespace {{ $labels.namespace }} has been in " +
	...
	},
	{
		...
		Expr: intstr.FromString(
			"sum by (name, namespace, status, node)((kubevirt_vm_info{status='starting'} == 1 " +
				...
				"and on(name, namespace) kubevirt_vmi_info{phase='running'}) ) " +
				"* on(name, namespace) group_left(node) topk by(name, namespace) (1, kubevirt_vmi_info{phase='running'}))",
		),
		For: ptr.To(promv1.Duration("5m")),
		Annotations: map[string]string{
			summaryAnnotationKey: "Virtual machine stuck in unhealthy state for more than 5 minutes",
			descriptionAnnotationKey: "Virtual machine {{ $labels.name }} in namespace {{ $labels.namespace }} on node {{ $labels.node }} " +
				"has been in {{ $labels.status }} state for more than 5 minutes. This may indicate issues with the VM lifecycle on the target " +
				"node.",
		},
		Labels: map[string]string{
			severityAlertLabelKey:        "warning",
	...
	},
	{
		...
		For: ptr.To(promv1.Duration("5m")),
		Annotations: map[string]string{
			descriptionAnnotationKey: "VirtualMachine {{ $labels.vm }} in namespace {{ $labels.namespace }} is under memory pressure: " +
				"low usable memory with elevated major faults and/or swap IO.",
	...
	},
	{
		...
		Annotations: map[string]string{
			...
			descriptionAnnotationKey: "VirtualMachineInstance {{ $labels.name }} in namespace {{ $labels.namespace }} has " +
				"filesystem {{ $labels.disk_name }} ({{ $labels.mount_point }}) usage above 85% (current: {{ $value }}%).",
		},
		Labels: map[string]string{
			severityAlertLabelKey:        "warning",
			operatorHealthImpactLabelKey: "none",
	...
	},
	{
		...
		Annotations: map[string]string{
			descriptionAnnotationKey: "VirtualMachine {{ $labels.vm }} in namespace {{ $labels.namespace }} has very low available " +
				"memory (<3% headroom) for 30 minutes with no meaningful swap IO (likely no swap).",
			summaryAnnotationKey: "The VirtualMachine has low available memory without swap",
		},
		Labels: map[string]string{
