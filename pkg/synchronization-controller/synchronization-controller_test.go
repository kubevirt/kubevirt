...
var _ = Describe("VMI status synchronization controller", func() {
	...
	Context("target controller different than source", func() {
		...
		It("should cancel remote migration", func() {
			...
			// _, err = remoteController.client.VirtualMachineInstanceMigration(targetMigration.Namespace).Update(context.Background(), targetMigration, metav1.UpdateOptions{})
			// Expect(err).ToNot(HaveOccurred())
