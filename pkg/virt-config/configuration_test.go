...
var _ = Describe("test configuration", func() {
	...
	DescribeTable("when TDX configuration", func(confidentialCompute *v1.ConfidentialComputeConfiguration, expectedEnforced bool, expectedSocketPath string) {
		...
		tdxConfig := clusterConfig.GetTDXConfiguration()
		Expect(tdxConfig.Attestation).ToNot(BeNil())
		Expect(*tdxConfig.Attestation.Enforced).To(Equal(expectedEnforced))
		Expect(*tdxConfig.Attestation.QgsSocketPath).To(Equal(expectedSocketPath))
		...
		Entry("is nil, should return defaults",
			nil,
			false, virtconfig.DefaultQGSSocketPath,
		),
