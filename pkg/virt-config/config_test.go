...
"bandwidthPerMigration": "110Mi"`},
	...
	table.DescribeTable("mdev configuration", func(nodeLabels map[string]string, expectedResult []string) {
		node := &kubev1.Node{
			ObjectMeta: metav1.ObjectMeta{
				Name: "testNode",
			},
		...
		table.Entry("expect default config when no selecors matched",
			map[string]string{},
			[]string{"nvidia-222", "nvidia-228", "i915-GVTg_V5_4"}),
		table.Entry("specific label match",
			map[string]string{"testLabel1": "true"},
			[]string{"nvidia-223"}),
		table.Entry("should match node by multiple selectors",
			map[string]string{"testLabel3": "true", "testLabel4": "true"},
			[]string{"nvidia-230"}),
		table.Entry("expect a merged result when several selectors match the same node",
