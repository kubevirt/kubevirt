...
func (c *ClusterConfig) GetSupportPodTolerations(typeName v1.SupportContainerType) []k8sv1.Toleration {
	for _, tolerations := range c.GetConfig().SupportPodTolerations {
		if tolerations.Type == typeName {
