...
func validateInstancetypeMatcher(vm *virtv1.VirtualMachine) []metav1.StatusCause {
	...
	if vm.Spec.Instancetype.InferFromVolume != "" {
		if vm.Spec.Instancetype.Name != "" || vm.Spec.Instancetype.Kind != "" {
			causes = append(causes, metav1.StatusCause{
				Type:    metav1.CauseTypeFieldValueNotFound,
				Message: "Name or kind already set, should be cleared before setting inferFromVolume",
				Field:   k8sfield.NewPath("spec", "instancetype", "inferFromVolume").String(),
			})
		}
	}
...
func validatePreferenceMatcher(vm *virtv1.VirtualMachine) []metav1.StatusCause {
	...
	if vm.Spec.Preference.InferFromVolume != "" {
		if vm.Spec.Preference.Name != "" || vm.Spec.Preference.Kind != "" {
			causes = append(causes, metav1.StatusCause{
				Type:    metav1.CauseTypeFieldValueNotFound,
				Message: "Name or kind already set, should be cleared before setting inferFromVolume",
				Field:   k8sfield.NewPath("spec", "preference", "inferFromVolume").String(),
			})
		}
	}
