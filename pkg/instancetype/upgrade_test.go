...
var _ = Describe("instancetype and preference Upgrades", func() {
	...
	Context("ControllerRevisionUpgrade should", func() {
		DescribeTable("skip upgrade of ControllerRevision labeled with latest object version of", func(createControllerRevision func() (*appsv1.ControllerRevision, error)) {
		...
		expectControllerRevisionCreationWithName := func(expectedCRName string) {
			k8sClient.Fake.PrependReactor("create", "controllerrevisions", func(action testing.Action) (handled bool, obj runtime.Object, err error) {
				...
				Expect(ok).To(BeTrue())
				if expectedCRName != "" {
					Expect(createdCR.GetName()).To(Equal(expectedCRName))
				}
		...
		expectControllerRevisionCreation := func() {
			expectControllerRevisionCreationWithName("")
		}
		...
		updateInstancetypeMatcher := func(vm *virtv1.VirtualMachine, revisionName string) {
			vm.Spec.Instancetype = &virtv1.InstancetypeMatcher{
				RevisionName: revisionName,
			}
		}

		updatePreferenceMatcher := func(vm *virtv1.VirtualMachine, revisionName string) {
			vm.Spec.Preference = &virtv1.PreferenceMatcher{
				RevisionName: revisionName,
			}
		}

		DescribeTable("upgrade ControllerRevision containing", func(createOriginalCR func(*virtv1.VirtualMachine) (*appsv1.ControllerRevision, error), updateMatcher func(*virtv1.VirtualMachine, string)) {
			originalCR, err := createOriginalCR(vm)
			Expect(err).ToNot(HaveOccurred())

			updateMatcher(vm, originalCR.Name)
		...
		DescribeTable("upgrade ControllerRevision without object version labels containing", func(createOriginalCR func(*virtv1.VirtualMachine) (*appsv1.ControllerRevision, error), updateMatcher func(*virtv1.VirtualMachine, string)) {
			originalCR, err := createOriginalCR(vm)
			Expect(err).ToNot(HaveOccurred())
			...
			expectControllerRevisionCreation()
			expectControllerRevisionDeletion(originalCR.Name)
			expectControllerRevisionCreationWithName(originalCR.Name)
			...
			Entry("VirtualMachineInstancetype v1beta1",
				func(vm *virtv1.VirtualMachine) (*appsv1.ControllerRevision, error) {
			...
			Entry("VirtualMachineClusterInstancetype v1beta1",
				func(vm *virtv1.VirtualMachine) (*appsv1.ControllerRevision, error) {
					cr, err := CreateControllerRevision(vm,
						&instancetypev1beta1.VirtualMachineClusterInstancetype{
							ObjectMeta: metav1.ObjectMeta{
			...
			Entry("VirtualMachinePreference v1beta1",
				func(vm *virtv1.VirtualMachine) (*appsv1.ControllerRevision, error) {
					cpuPreference := instancetypev1beta1.PreferSockets
					cr, err := CreateControllerRevision(vm,
						&instancetypev1beta1.VirtualMachinePreference{
			...
			Entry("VirtualMachineClusterPreference v1beta1",
				func(vm *virtv1.VirtualMachine) (*appsv1.ControllerRevision, error) {
					cpuPreference := instancetypev1beta1.PreferSockets
					cr, err := CreateControllerRevision(vm,
						&instancetypev1beta1.VirtualMachineClusterPreference{
