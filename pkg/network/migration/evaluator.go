...
func (e Evaluator) Evaluate(vmi *v1.VirtualMachineInstance) k8scorev1.ConditionStatus {
	...
	case pendingMigration:
		existingCondition := getMigrationRequiredCondition(vmi.Status.Conditions)
...
func getMigrationRequiredCondition(conditions []v1.VirtualMachineInstanceCondition) *v1.VirtualMachineInstanceCondition {
	for _, condition := range conditions {
		if condition.Type == v1.VirtualMachineInstanceMigrationRequired {
			return &condition
		}
	}
	return nil
