...
var _ = Describe("Evaluator", func() {
	...
	Context("NAD name change", func() {
		const (
			...
			newSecondaryNetworkName = "new-secondary-network"
		)
		...
		DescribeTable("should trigger",
				...
				&k8scorev1.Pod{
					ObjectMeta: metav1.ObjectMeta{
						Annotations: map[string]string{
							"k8s.v1.cni.cncf.io/network-status": multusStatusAnnotation(
			...
			Entry("no migration when both NAD name and network name in spec differs from that in pod annotation and FG is enabled",
				libvmi.New(
					libvmi.WithInterface(libvmi.InterfaceDeviceWithBridgeBinding(secondaryNetworkName)),
					libvmi.WithNetwork(libvmi.MultusNetwork(newSecondaryNetworkName, newSecondaryNadName)),
					libvmistatus.WithStatus(libvmistatus.New(
						libvmistatus.WithInterfaceStatus(v1.VirtualMachineInstanceNetworkInterface{
							Name:             secondaryNetworkName,
							PodInterfaceName: secondaryPodIfaceName,
...
func multusStatusAnnotation(ifaceName, nadName, namespace string) string {
