...
func validateBlockDiscontinuedBindings(field *k8sfield.Path, spec *v1.VirtualMachineInstanceSpec) []metav1.StatusCause {
	...
	for idx, ifaceSpec := range spec.Domain.Devices.Interfaces {
		...
		if ifaceSpec.InterfaceBindingMethod.DeprecatedPasst != nil {
			causes = append(causes, metav1.StatusCause{
				Type:    metav1.CauseTypeFieldValueInvalid,
				Message: "Passt network binding has been discontinued since v1.3. Use PasstBinding instead",
