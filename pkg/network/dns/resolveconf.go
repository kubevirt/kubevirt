...
func GetResolvConfDetailsFromPod() (*Nameservers, []string, error) {
	...
	log.Log.Infof("Found IPv4 nameservers in %s: %s", resolvConf, strings.Join(toIPStrings(ipv4Nameservers), " "))
	log.Log.Infof("Found IPv6 nameservers in %s: %s", resolvConf, strings.Join(toIPStrings(ipv6Nameservers), " "))
