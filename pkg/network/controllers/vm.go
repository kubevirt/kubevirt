...
func syncVMIInterfaces(
	...
	updatedVmiSpec := applyDynamicIfaceAndNetworkRequestOnVMI(vm, vmiCopy, hasOrdinalIfaces, isLiveUpdateNADRefEnabled)
...
func applyDynamicIfaceAndNetworkRequestOnVMI(
	vm *v1.VirtualMachine,
	vmi *v1.VirtualMachineInstance,
...
func syncNetworkNADReference(
	...
	if vmiNetExists && vmNetExists && !reflect.DeepEqual(vmNet, vmiNet) {
		if vmispec.AreNetsEqualIgnoringMultusNetName(vmNet, vmiNet) {
			for i, n := range vmiSpec.Networks {
				if n.Name == vmNet.Name && n.Multus != nil && vmNet.Multus != nil {
					vmiSpec.Networks[i].Multus.NetworkName = vmNet.Multus.NetworkName
					break
				}
			}
		}
