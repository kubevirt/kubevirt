...
func restorePodIfaceNames(
	...
	updatedStatuses := slices.Clone(interfacesStatus)

	prevIfaceStatusesByName := netvmispec.IndexInterfaceStatusByName(prevIfaceStatuses, netvmispec.HasIfaceStatusOriginatedFromSpec)

	for i, ifaceStatus := range updatedStatuses {
		if !netvmispec.HasIfaceStatusOriginatedFromSpec(ifaceStatus) {
			continue
		}
		updatedStatuses[i].PodInterfaceName = prevIfaceStatusesByName[ifaceStatus.Name].PodInterfaceName
	}
