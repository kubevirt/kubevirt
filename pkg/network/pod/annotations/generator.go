...
func (g Generator) generateMultusAnnotation(vmi *v1.VirtualMachineInstance, pod *k8scorev1.Pod) (string, bool) {
	var currentMultusAnnotationArr, multusAnnotationItemsToModifyArr, updatedMultusAnnotationArr []map[string]interface{}
	...
	for index, currentItem := range currentMultusAnnotationArr {
	...
	for _, remainingItem := range itemsToModifyMap {
		updatedMultusAnnotationArr = append(updatedMultusAnnotationArr, remainingItem)
	}
	if len(currentMultusAnnotation) == 0 {
		updatedMultusAnnotationArr = multusAnnotationItemsToModifyArr
	}

	// Marshal the array to updatedMultusAnnotation
	...
	if reflect.DeepEqual(currentMultusAnnotationArr, updatedMultusAnnotationArr) {
		...
		log.Log.Object(pod).V(logLevel).Infof(
			...
			string(updatedMultusAnnotation),
		)
