			...
			Context("with CloudInitNoCloud volume source", func() {
				...
				Context("with secretRefs", func() {
					...
					It("should resolve no-cloud data from volume", func() {
						...
						Expect(vmi.Spec.Volumes[0].CloudInitNoCloud.UserData).To(Equal("secret-userdata"))
						Expect(vmi.Spec.Volumes[0].CloudInitNoCloud.NetworkData).To(Equal("secret-networkdata"))
					...
					It("should resolve MetaDataSecretRef from volume", func() {
						...
						_, err := resolveNoCloudSecrets(vmi, tmpDir)
						Expect(err).To(Not(HaveOccurred()), "could not resolve secret volume")
						Expect(vmi.Spec.Volumes[0].CloudInitNoCloud.MetaData).To(Equal(map[string]string{
							customMetaAppKey:         customMetaAppVal,
							customMetaEnvironmentKey: customMetaEnvironmentVal,
						}))
					})
			...
			Context("with CloudInitConfigDrive volume source", func() {
				...
				Context("with secretRefs", func() {
					...
					It("should resolve config-drive data from volume", func() {
						...
						Expect(err).To(Not(HaveOccurred()), "could not resolve secret volume")
						Expect(vmi.Spec.Volumes[0].CloudInitConfigDrive.UserData).To(Equal("secret-userdata"))
						Expect(vmi.Spec.Volumes[0].CloudInitConfigDrive.NetworkData).To(Equal("secret-networkdata"))
						Expect(keys).To(HaveLen(2))
					})
					...
					It("should resolve MetaDataSecretRef from volume", func() {
						...
						Expect(err).To(Not(HaveOccurred()), "could not resolve secret volume")
						Expect(vmi.Spec.Volumes[0].CloudInitConfigDrive.MetaData).To(Equal(map[string]string{
							customMetaAppKey:         customMetaAppVal,
							customMetaEnvironmentKey: customMetaEnvironmentVal,
						}))
					})
