...
func (app *SubresourceAPIApp) evacuateCancel(ctx context.Context, namespace, name string, body io.ReadCloser, fetcher vmiFetcher) *errors.StatusError {
	...
	if _, err = app.virtCli.VirtualMachineInstance(namespace).Patch(ctx, vmi.GetName(), types.JSONPatchType, patchBytes, k8smetav1.PatchOptions{DryRun: opts.DryRun}); err != nil {
