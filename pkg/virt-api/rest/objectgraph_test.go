...
var _ = Describe("Backup Graph", func() {
	var (
	...
	It("should generate the correct backup graph for VirtualMachine", func() {
		vm = &kubevirtv1.VirtualMachine{
		...
		kubeClient.Fake.PrependReactor("list", "pods", func(action testing.Action) (handled bool, obj runtime.Object, err error) {
			return true, &corev1.PodList{Items: []corev1.Pod{corev1.Pod{
				ObjectMeta: metav1.ObjectMeta{
		...
		Expect(graphNodes.Items).To(HaveLen(6))
		Expect(graphNodes.Items[0].ObjectReference.Name).To(Equal("test-vm"))
		Expect(graphNodes.Items[1].ObjectReference.Name).To(Equal("virt-launcher-test-vmi"))
		Expect(graphNodes.Items[2].ObjectReference.Name).To(Equal("test-ssh-secret"))
		Expect(graphNodes.Items[3].ObjectReference.Name).To(Equal("test-root-disk-pvc"))
		Expect(graphNodes.Items[4].ObjectReference.Name).To(Equal("test-datavolume"))
		Expect(graphNodes.Items[4].ObjectReference.Kind).To(Equal("datavolumes"))
		Expect(graphNodes.Items[5].ObjectReference.Name).To(Equal("test-datavolume"))
		Expect(graphNodes.Items[5].ObjectReference.Kind).To(Equal("persistentvolumeclaims"))
	})
	...
	It("should handle error when listing pods", func() {
		...
		Expect(graphNodes.Items).To(HaveLen(1)) // Should still return the VMI
	})
	...
	It("should include backend storage PVC in the graph", func() {
		...
		graphNodes, err := graph.GetObjectGraph(vm)
		Expect(err).NotTo(HaveOccurred())
		Expect(graphNodes.Items).To(HaveLen(1))
		Expect(graphNodes.Items[0].ObjectReference.Name).To(Equal("backend-storage-pvc"))
	})
	...
	It("should return empty graph for unrelated objects", func() {
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(graphNodes.Items).To(BeEmpty())
	})
})
