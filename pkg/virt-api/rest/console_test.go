...
var _ = Describe("Console Subresource api", func() {
	...
	BeforeEach(func() {
		...
		app = NewSubresourceAPIApp(mockVirtClient, nil, 8080, &tls.Config{InsecureSkipVerify: true}, config)
	})
	...
	It("should proxy websocket data from client to handler", func() {
		...
		Expect(err).ToNot(HaveOccurred())
		app = NewSubresourceAPIApp(mockVirtClient, nil, handlerPortInt, &tls.Config{InsecureSkipVerify: true}, config)
