...
func (app *SubresourceAPIApp) UnpauseVMIRequestHandler(request *restful.Request, response *restful.Response) {
	...
	validate := func(vmi *v1.VirtualMachineInstance) *errors.StatusError {
		...
		if migrations.IsMigrating(vmi) {
			return errors.NewConflict(v1.Resource("virtualmachineinstance"), vmi.Name, fmt.Errorf("Cannot unpause VM during migration"))
		}
		condManager := controller.NewVirtualMachineInstanceConditionManager()
		if !condManager.HasCondition(vmi, v1.VirtualMachineInstancePaused) {
...
func (app *SubresourceAPIApp) ResetVMIRequestHandler(request *restful.Request, response *restful.Response) {
	...
	validate := func(vmi *v1.VirtualMachineInstance) *errors.StatusError {
		...
		// Check if VM is currently being migrated
		if migrations.IsMigrating(vmi) {
			return errors.NewConflict(v1.Resource("virtualmachineinstance"), vmi.Name, fmt.Errorf("Cannot reset VM during migration"))
		}
		return nil
	}
