...
func (app *SubresourceAPIApp) StartVM(ctx context.Context, namespace, name string, startOptions *v1.StartOptions) *errors.StatusError {
	vm, err := app.virtClient.VirtualMachine(namespace).Get(ctx, name, metav1.GetOptions{})
	if err != nil {
	...
	vmi, err := app.virtClient.VirtualMachineInstance(namespace).Get(ctx, name, metav1.GetOptions{})
	...
	case v1.RunStrategyHalted:
		...
		if startPaused && (vm.Spec.Template == nil || vm.Spec.Template.Spec.StartStrategy != &pausedStartStrategy) {
			...
			_, patchErr = app.virtClient.VirtualMachine(vm.Namespace).PatchStatus(ctx, vm.Name, types.JSONPatchType, patchBytes, metav1.PatchOptions{DryRun: startOptions.DryRun})
		} else {
			...
			_, patchErr = app.virtClient.VirtualMachine(namespace).Patch(ctx, vm.GetName(), types.JSONPatchType, patchBytes, metav1.PatchOptions{DryRun: startOptions.DryRun})
		}
...
func (app *SubresourceAPIApp) StopVM(ctx context.Context, namespace, name string, stopOptions *v1.StopOptions) *errors.StatusError {
	...
	case v1.RunStrategyAlways, v1.RunStrategyOnce:
		...
		log.Log.Object(vm).V(4).Infof(patchingVMFmt, string(patchBytes))
		_, patchErr = app.virtClient.VirtualMachine(namespace).Patch(ctx, vm.GetName(), types.JSONPatchType, patchBytes, metav1.PatchOptions{DryRun: stopOptions.DryRun})
	}
...
func (app *SubresourceAPIApp) RestartVM(ctx context.Context, namespace, name string, restartOptions *v1.RestartOptions) *errors.StatusError {
	...
	log.Log.Object(vm).V(4).Infof(patchingVMFmt, string(patchBytes))
	_, err = app.virtClient.VirtualMachine(vm.Namespace).PatchStatus(ctx, vm.Name, types.JSONPatchType, patchBytes, metav1.PatchOptions{DryRun: restartOptions.DryRun})
	if err != nil {
		if forceRestart {
...
func (app *SubresourceAPIApp) MigrateVM(ctx context.Context, namespace, name string, migrateOptions *v1.MigrateOptions) *errors.StatusError {
	...
	_, err := app.virtClient.VirtualMachineInstanceMigration(namespace).Create(ctx, &v1.VirtualMachineInstanceMigration{
...
func (app *SubresourceAPIApp) patchVMITerminationGracePeriod(ctx context.Context, vmi *v1.VirtualMachineInstance, namespace string, gracePeriod int64, dryRun []string) (int64, error) {
	...
	_, err = app.virtClient.VirtualMachineInstance(namespace).Patch(ctx, vmi.GetName(), types.JSONPatchType, patchBytes, metav1.PatchOptions{DryRun: dryRun})
...
func (app *SubresourceAPIApp) patchVMStatusStopped(ctx context.Context, vmi *v1.VirtualMachineInstance, vm *v1.VirtualMachine, stopOptions *v1.StopOptions) error {
	...
	log.Log.Object(vm).V(4).Infof(patchingVMStatusFmt, string(patchBytes))
	_, err = app.virtClient.VirtualMachine(vm.Namespace).PatchStatus(ctx, vm.Name, types.JSONPatchType, patchBytes, metav1.PatchOptions{DryRun: stopOptions.DryRun})
	return err
}
