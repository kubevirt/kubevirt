...
var _ = Describe("Validating VMICreate Admitter", func() {
	...
	It("should allow Clock without Timer", func() {
		...
		vmi := newBaseVmi(libvmi.WithClock(
			v1.Clock{
	...
	Context("with VirtualMachineInstance spec", func() {
			...
			vmi.Spec.Networks = []v1.Network{
				{
					...
					NetworkSource: v1.NetworkSource{
						ResourceClaim: &v1.ClaimRequest{
							ClaimName:   pointer.P("nonexistent-claim"),
							RequestName: pointer.P("vf"),
						},
					},
				},
		...
		It("should reject duplicate claimName/requestName between DRA network and DRA GPU", func() {
			...
			Expect(causes).To(HaveLen(1))
			Expect(causes[0].Message).To(ContainSubstring("duplicate claimName/requestName"))
			Expect(causes[0].Message).To(ContainSubstring("already used by GPU[0]"))
		})
		...
		It("should reject duplicate claimName/requestName between DRA network and DRA HostDevice", func() {
			...
			Expect(causes).To(HaveLen(1))
			Expect(causes[0].Message).To(ContainSubstring("duplicate claimName/requestName"))
			Expect(causes[0].Message).To(ContainSubstring("already used by HostDevice[0]"))
		})

		It("should reject mixing Multus and DRA resourceClaim networks in the same VMI", func() {
			enableFeatureGates(featuregate.NetworkDevicesWithDRAGate)
			defer disableFeatureGates()
			vmi := api.NewMinimalVMI("testvm")
			vmi.Spec.Networks = []v1.Network{
		...
		It("should reject mixing non-SRIOV Multus and DRA resourceClaim networks in the same VMI", func() {
			enableFeatureGates(featuregate.NetworkDevicesWithDRAGate)
			defer disableFeatureGates()
			vmi := api.NewMinimalVMI("testvm")
			vmi.Spec.Networks = []v1.Network{
				{
					Name:          "multus-net",
					NetworkSource: v1.NetworkSource{Multus: &v1.MultusNetwork{NetworkName: "bridge-nad"}},
				},
				{
					Name: "dra-net",
					NetworkSource: v1.NetworkSource{
			...
			vmi.Spec.Domain.Devices.Interfaces = []v1.Interface{
				{Name: "multus-net", InterfaceBindingMethod: v1.InterfaceBindingMethod{Bridge: &v1.InterfaceBridge{}}},
				{Name: "dra-net", InterfaceBindingMethod: v1.InterfaceBindingMethod{SRIOV: &v1.InterfaceSRIOV{}}},
			}
			vmi.Spec.ResourceClaims = []k8sv1.PodResourceClaim{
				{Name: "claim1"},
