...
var _ = Describe("Validating VMICreate Admitter", func() {
	...
	disableSEVFeatureGate := func() {
		kvConfig := kv.DeepCopy()
		kvConfig.Spec.Configuration.DeveloperConfiguration.DisabledFeatureGates = []string{legacy.WorkloadEncryptionSEV}
		testutils.UpdateFakeKubeVirtClusterConfig(kvStore, kvConfig)
	}
	...
	Context("with DRA GPUs", func() {
		...
		It("should reject a DRA-GPU when the feature-gate is NOT enabled", func() {
			kvConfig := kv.DeepCopy()
			kvConfig.Spec.Configuration.DeveloperConfiguration.DisabledFeatureGates = []string{legacy.GPUsWithDRAGate}
			testutils.UpdateFakeKubeVirtClusterConfig(kvStore, kvConfig)
			defer disableFeatureGates()
