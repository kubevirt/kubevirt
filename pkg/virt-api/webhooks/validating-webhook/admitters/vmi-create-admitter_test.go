...
var _ = Describe("Validating VMICreate Admitter", func() {
	...
	Context("with virtiofs subPath", func() {
		...
		It("should accept an empty subPath on a service account volume", func() {
			vmi.Spec.Volumes = []v1.Volume{{
				Name: "sharedvol",
				VolumeSource: v1.VolumeSource{
		...
		It("should reject subPath when the referenced volume uses a service account", func() {
			vmi.Spec.Volumes = []v1.Volume{{
				Name: "sharedvol",
				VolumeSource: v1.VolumeSource{
					ServiceAccount: &v1.ServiceAccountVolumeSource{
			...
			Expect(causes).To(HaveLen(1))
			Expect(causes[0].Message).To(ContainSubstring("not supported when volume"))
			Expect(causes[0].Message).To(ContainSubstring("service account"))
			Expect(causes[0].Field).To(Equal("fake.domain.devices.filesystems[0].virtiofs.subPath"))
		})
	})
