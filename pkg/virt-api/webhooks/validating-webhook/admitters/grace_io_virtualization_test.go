...
var _ = Describe("GraceIOVirtualization admission", func() {
	...
	It("accepts a Grace GPU request that satisfies the static baseline", func() {
		config := newGraceConfig("10DE:2342", featuregate.GraceIOVirtualization, pciNUMAAwareTopologyFeatureGate, featuregate.IOMMUFDGate)
	...
	It("accepts a Grace host device request that satisfies the static baseline", func() {
		config := newGraceConfig("10DE:2941", featuregate.GraceIOVirtualization, pciNUMAAwareTopologyFeatureGate, featuregate.IOMMUFDGate)
		spec := newGraceSpec()
	...
	It("requires the GraceIOVirtualization feature gate for Grace GPU resources", func() {
		config := newGraceConfig("10DE:2342", pciNUMAAwareTopologyFeatureGate)
	...
	It("requires the PCI NUMA-aware topology feature gate", func() {
		...
		Expect(causes[0].Message).To(ContainSubstring(pciNUMAAwareTopologyFeatureGate))
	})
	...
	It("requires the IOMMUFD feature gate", func() {
		config := newGraceConfig("10DE:2342", featuregate.GraceIOVirtualization, pciNUMAAwareTopologyFeatureGate)
	...
	It("rejects ambiguous NVIDIA wildcard PCI selectors when GraceIOVirtualization is enabled", func() {
		config := newGraceConfig("10DE:*", featuregate.GraceIOVirtualization, pciNUMAAwareTopologyFeatureGate, featuregate.IOMMUFDGate)
	...
	It("does not classify non-Grace NVIDIA PCI selectors", func() {
		config := newGraceConfig("10DE:2330", featuregate.GraceIOVirtualization, pciNUMAAwareTopologyFeatureGate, featuregate.IOMMUFDGate)
