...
var _ = Describe("Validating MigrationPolicy Admitter", func() {
	...
	DescribeTable("should reject migration policy with", func(policySpec migrationsv1.MigrationPolicySpec) {
		...
		Entry("non-positive MaxDowntime",
			migrationsv1.MigrationPolicySpec{MaxDowntime: pointer.P(int64(0))},
		),
		Entry("too large MaxDowntime",
			migrationsv1.MigrationPolicySpec{MaxDowntime: pointer.P(int64(migrationutils.QEMUMaxMigrationDowntimeMS + 1))},
		),
	)
	...
	DescribeTable("should accept migration policy with", func(policySpec migrationsv1.MigrationPolicySpec) {
		...
		Entry("valid MaxDowntime",
			migrationsv1.MigrationPolicySpec{MaxDowntime: pointer.P(int64(900))},
		),
