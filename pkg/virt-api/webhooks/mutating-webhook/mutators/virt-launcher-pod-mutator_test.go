...
var _ = Describe("VirtLauncherPodMutator", func() {
	...
	Context("when virt-launcher pod has containerPath volumes", func() {
		It("should inject virtiofs containers", func() {
			...
			// Verify the patch adds the expected container
			var patches []map[string]interface{}
			err := json.Unmarshal(response.Patch, &patches)
			Expect(err).ToNot(HaveOccurred())
			Expect(patches).To(HaveLen(1))
			Expect(patches[0]["op"]).To(Equal("add"))
			Expect(patches[0]["path"]).To(Equal("/spec/containers/-"))

			container := patches[0]["value"].(map[string]interface{})
