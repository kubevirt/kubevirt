...
func ServeVMs(resp http.ResponseWriter, req *http.Request, clusterConfig *virtconfig.ClusterConfig, virtCli kubecli.KubevirtClient, informers *webhooks.Informers) {
	serve(resp, req, &mutators.VMsMutator{ClusterConfig: clusterConfig, FlavorMethods: flavor.NewMethods(virtCli, informers.FlavorInformer.GetStore(), informers.ClusterFlavorInformer.GetStore())})
}
