...
func NewOptions(req *http.Request) (*Options, error) {
	tty := req.FormValue(v1.ExecTTYParam) == "1"
	stdin := req.FormValue(v1.ExecStdinParam) == "1"
	stdout := req.FormValue(v1.ExecStdoutParam) == "1"
	stderr := req.FormValue(v1.ExecStderrParam) == "1"
	if tty && stderr {
