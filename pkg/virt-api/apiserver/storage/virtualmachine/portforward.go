...
// PortForwardREST serves the virtualmachines/portforward endpoint (GET) of the
// subresources.kubevirt.io API group as a streaming rest.Connecter.
...
func (r *PortForwardREST) Connect(ctx context.Context, name string, _ runtime.Object, responder rest.Responder) (http.Handler, error) {
	...
	if !ok {
		return nil, apierrors.NewBadRequest("namespace is required to port-forward to a VirtualMachine")
	}

	// [resource, name, subresource, extra...] i.e.
	// [virtualmachines, <name>, portforward, <port>, <protocol>].
