...
type vmiObjectGraphGetter interface {
	GetVMIObjectGraph(namespace, name string, body io.Reader) (v1.ObjectGraphNode, *apierrors.StatusError)
}

// ObjectGraphREST serves the virtualmachineinstances/objectgraph subresource of
// the subresources.kubevirt.io API group as a rest.Connecter storage
type ObjectGraphREST struct {
	getter vmiObjectGraphGetter
}

func NewObjectGraphREST(getter vmiObjectGraphGetter) *ObjectGraphREST {
...
func (r *ObjectGraphREST) Connect(ctx context.Context, name string, _ runtime.Object, responder rest.Responder) (http.Handler, error) {
	...
	if !ok {
		return nil, apierrors.NewBadRequest("namespace is required to get the object graph of a VirtualMachineInstance")
	}

	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		graph, statErr := r.getter.GetVMIObjectGraph(namespace, name, req.Body)
		if statErr != nil {
