...
func (h *Handler) getAllComponentPods() ([]k8sv1.Pod, error) {
	...
	podList, err := h.virtCli.CoreV1().Pods(namespace).List(context.Background(), metav1.ListOptions{LabelSelector: "kubevirt.io"})
	if err != nil {
		return nil, err
	}
...
func (h *Handler) getPodsNextPage(cpRequest *v1.ClusterProfilerRequest) (pods []k8sv1.Pod, cont string, err error) {
	...
	if podList, err = h.virtCli.CoreV1().Pods(namespace).List(context.Background(), listOptions); err != nil {
		return nil, "", err
	}
