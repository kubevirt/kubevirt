...
func (app *virtAPIApp) composeSubresources() {
	...
	for _, version := range v1.SubresourceGroupVersions {
		...
		subresourceApp := rest.NewSubresourceAPIApp(app.virtClient, app.consoleServerPort, app.handlerTLSConfiguration, app.clusterConfig)
...
func (app *virtAPIApp) composeLegacy() {
...
func (app *virtAPIApp) readRequestHeader() error {
	authConfigMap, err := app.virtClient.CoreV1().ConfigMaps(metav1.NamespaceSystem).Get(context.Background(), util.ExtensionAPIServerAuthenticationConfigMap, metav1.GetOptions{})
	if err != nil {
		return err
	}
...
func (app *virtAPIApp) setupLegacyTLS(k8sCAManager kvtls.KubernetesCAManager, kubevirtCAManager kvtls.ClientCAManager) {
...
// runLegacyServer wires up and serves the legacy go-restful based virt-api.
// It is the default (non GenericAPIServer) code path and mirrors main.
func (app *virtAPIApp) runLegacyServer(informerFactory controller.KubeInformerFactory, webhookInformers *webhooks.Informers) error {
	// The legacy server relies on the serving certificate managed on disk,
	// reusing the same --tls-cert-file/--tls-private-key-file the
	// GenericAPIServer options expose.
	app.certmanager = bootstrap.NewFileCertificateManager(app.apiServer.CertFile(), app.apiServer.KeyFile())
	go app.certmanager.Start()

	// Configure the authorizor request headers from the cluster's
	// extension-apiserver-authentication ConfigMap.
	if err := app.readRequestHeader(); err != nil {
		return err
	}

	// The auth ConfigMap informer is only needed by the legacy TLS setup.
	...
	app.setupLegacyTLS(k8sCAManager, kubevirtCAManager)

	// Compose the subresource web services (needs handlerTLSConfiguration).
	app.composeLegacy()

	// Register the admission webhooks and metrics on the default mux. The
	// webhook handlers are shared with the GenericAPIServer path to avoid
	// signature drift.
	for _, mh := range app.webhookMuxHandlers(webhookInformers) {
		http.Handle(mh.Path, mh.Handler)
	}
	...
	server := &http.Server{
		Addr:      fmt.Sprintf("0.0.0.0:%d", app.apiServer.BindPort()),
