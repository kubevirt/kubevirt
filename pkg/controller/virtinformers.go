...
func (f *kubeInformerFactory) VirtualMachineExport() cache.SharedIndexInformer {
	return f.getInformer("vmExportInformer", func() cache.SharedIndexInformer {
		lw := cache.NewListWatchFromClient(f.virtClient.GeneratedKubeVirtClient().ExportV1beta1().RESTClient(), "virtualmachineexports", k8sv1.NamespaceAll, fields.Everything())
