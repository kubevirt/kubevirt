...
func (f *kubeInformerFactory) VirtualMachineExport() cache.SharedIndexInformer {
	return f.getInformer("vmExportInformer", func() cache.SharedIndexInformer {
		lw := cache.NewListWatchFromClient(f.virtClientSet.GeneratedKubeVirtClient().ExportV1().RESTClient(), "virtualmachineexports", k8sv1.NamespaceAll, fields.Everything())
...
func (f *kubeInformerFactory) Plugin() cache.SharedIndexInformer {
	return f.getInformer("pluginInformer", func() cache.SharedIndexInformer {
		lw := cache.NewListWatchFromClient(f.virtClientSet.GeneratedKubeVirtClient().PluginV1alpha1().RESTClient(), plugin.ResourcePluginPlural, k8sv1.NamespaceAll, fields.Everything())
