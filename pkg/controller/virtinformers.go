...
func (f *kubeInformerFactory) VirtualMachineTemplate() cache.SharedIndexInformer {
	return f.getInformer("vmTemplateInformer", func() cache.SharedIndexInformer {
		tc, err := templateclient.NewForConfig(f.clientSet.Config())
		if err != nil {
			panic(fmt.Sprintf("failed to create virt-template client: %v", err))
		}
		lw := cache.NewListWatchFromClient(tc.TemplateV1alpha1().RESTClient(), templateapi.PluralResourceName, k8sv1.NamespaceAll, fields.Everything())
...
func (f *kubeInformerFactory) Plugin() cache.SharedIndexInformer {
	return f.getInformer("pluginInformer", func() cache.SharedIndexInformer {
		lw := cache.NewListWatchFromClient(f.clientSet.GeneratedKubeVirtClient().PluginV1alpha1().RESTClient(), plugin.ResourcePluginPlural, k8sv1.NamespaceAll, fields.Everything())
