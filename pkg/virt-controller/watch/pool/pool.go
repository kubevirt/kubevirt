...
func (c *Controller) proactiveUpdate(pool *poolv1.VirtualMachinePool, vmUpdatedList []*virtv1.VirtualMachine) error {
	...
	for i := range vmUpdatedList {
		...
		vm, err := c.clientset.VirtualMachine(pool.Namespace).Get(context.Background(), vmUpdatedList[i].Name, metav1.GetOptions{})
		if err != nil {
			return err
		}
...
func hasSortingPolicy(pool *poolv1.VirtualMachinePool) bool {
	if pool.Spec.UpdateStrategy == nil || pool.Spec.UpdateStrategy.Proactive == nil || pool.Spec.UpdateStrategy.Proactive.SelectionPolicy == nil || pool.Spec.UpdateStrategy.Proactive.SelectionPolicy.SortPolicy == nil {
		return false
	}
	return true
}
