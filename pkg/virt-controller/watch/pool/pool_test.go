		...
		It("should update VMIs and VMs using label selector filtering for proactive update strategy", func() {
			...
			for i, vm := range vmList.Items {
				if i == 0 || i == 1 {
					...
					Expect(vm.Spec.Template.ObjectMeta.Labels).NotTo(HaveKeyWithValue("newkey", "newval"))
				}
			}
			...
			Expect(testing.FilterActions(&fakeVirtClient.Fake, "update", "virtualmachines")).To(HaveLen(2))
		})
