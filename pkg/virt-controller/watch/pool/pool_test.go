...
var _ = Describe("Pool", func() {
	...
	Context("One valid Pool controller given", func() {
		...
		It("should delete VMs with highest ordinals first when using ordered downscale strategy", func() {
			pool, vm := DefaultPool(2)
			pool.Spec.DownscalePolicy = &poolv1.VirtualMachinePoolDownscalePolicy{
				Strategy: poolv1.VirtualMachinePoolDownscaleStrategyOrdered,
			}
