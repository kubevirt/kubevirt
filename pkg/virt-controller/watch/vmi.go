...
func (c *VMIController) updateStatus(vmi *virtv1.VirtualMachineInstance, pod *k8sv1.Pod, dataVolumes []*cdiv1.DataVolume, syncErr common.SyncError) error {
	key := controller.VirtualMachineInstanceKey(vmi)
		...
		if vmiPodExists {
			...
			if vmi.Status.TopologyHints == nil {
				if topologyHints, tscRequirement, err := c.topologyHinter.TopologyHintsForVMI(vmi); err != nil && tscRequirement == topology.RequiredForBoot {
					c.recorder.Eventf(vmi, k8sv1.EventTypeWarning, common.FailedGatherhingClusterTopologyHints, err.Error())
					return common.NewSyncError(err, common.FailedGatherhingClusterTopologyHints)
			...
			if syncErr != nil &&
				(syncErr.Reason() == common.FailedPvcNotFoundReason) {
				condition := virtv1.VirtualMachineInstanceCondition{
		...
		case vmiPodExists:
			...
			if common.IsPodReady(pod) && vmi.DeletionTimestamp == nil {
				...
				if podQosClass != k8sv1.PodQOSGuaranteed && vmi.IsCPUDedicated() {
					c.recorder.Eventf(vmi, k8sv1.EventTypeWarning, common.FailedGuaranteePodResourcesReason, "failed to guarantee pod resources")
					syncErr = common.NewSyncError(fmt.Errorf("failed to guarantee pod resources"), common.FailedGuaranteePodResourcesReason)
					break
				}
...
func checkForContainerImageError(pod *k8sv1.Pod) common.SyncError {
	...
	for _, containerStatus := range containerStatuses {
		...
		if reason == common.ErrImagePullReason || reason == common.ImagePullBackOffReason {
...
func (c *VMIController) sync(vmi *virtv1.VirtualMachineInstance, pod *k8sv1.Pod, dataVolumes []*cdiv1.DataVolume) common.SyncError {
	key := controller.VirtualMachineInstanceKey(vmi)
	...
	if vmi.DeletionTimestamp != nil {
		...
		if err != nil {
			return common.NewSyncError(fmt.Errorf("failed to delete pod: %v", err), common.FailedDeletePodReason)
		}
		return nil
	}
	...
	if vmi.IsFinal() {
		...
		if err != nil {
			return common.NewSyncError(fmt.Errorf("failed to delete attachment pods: %v", err), common.FailedHotplugSyncReason)
		}
		return nil
	}
	...
	if !common.PodExists(pod) {
		...
		if _, ok := err.(storagetypes.PvcNotFoundError); ok {
			...
			return common.NewSyncError(fmt.Errorf(common.FailedToRenderLaunchManifestErrFormat, err), common.FailedCreatePodReason)
		}
		...
		if k8serrors.IsForbidden(err) && strings.Contains(err.Error(), "violates PodSecurity") {
			...
			return common.NewSyncError(psaErr, common.FailedCreatePodReason)
		}
		if err != nil {
			...
			return common.NewSyncError(fmt.Errorf("failed to create virtual machine pod: %v", err), common.FailedCreatePodReason)
		}
		c.recorder.Eventf(vmi, k8sv1.EventTypeNormal, common.SuccessfulCreatePodReason, "Created virtual machine pod %s", pod.Name)
		return nil
	}
	...
	if !isWaitForFirstConsumer {
		...
		if err != nil {
			return common.NewSyncError(fmt.Errorf("failed to clean up temporary pods: %v", err), common.FailedHotplugSyncReason)
		}
	}
	...
	if !isTempPod(pod) && common.IsPodReady(pod) {
		if vmispec.SRIOVInterfaceExist(vmi.Spec.Domain.Devices.Interfaces) {
			...
			if err != nil {
				return common.NewSyncError(err, common.FailedPodPatchReason)
			}
			*pod = *patchedPod
		}
		...
		if pod.DeletionTimestamp == nil && c.needsHandleHotplug(hotplugVolumes, hotplugAttachmentPods) {
			...
			if hotplugSyncErr != nil {
				if hotplugSyncErr.Reason() == common.MissingAttachmentPodReason {
		...
		if len(vmispec.NetworksToHotplug(vmi.Spec.Networks, vmi.Status.Interfaces)) > 0 {
			if err := c.handleDynamicInterfaceRequests(vmi, pod); err != nil {
				return common.NewSyncError(
					fmt.Errorf("failed to hotplug network interfaces for vmi [%s/%s]: %w", vmi.GetNamespace(), vmi.GetName(), err),
					common.FailedHotplugSyncReason,
				)
			}
		}
	}
...
func (c *VMIController) handleSyncDataVolumes(vmi *virtv1.VirtualMachineInstance, dataVolumes []*cdiv1.DataVolume) (bool, bool, common.SyncError) {
		...
		if volume.VolumeSource.DataVolume != nil || volume.VolumeSource.PersistentVolumeClaim != nil {
			...
			if err != nil {
				if _, ok := err.(storagetypes.PvcNotFoundError); ok {
					...
					c.recorder.Eventf(vmi, k8sv1.EventTypeNormal, common.FailedPvcNotFoundReason, "PVC %s/%s does not exist, waiting for it to appear", vmi.Namespace, storagetypes.PVCNameFromVirtVolume(&volume))
					return false, false, &informalSyncError{err: fmt.Errorf("PVC %s/%s does not exist, waiting for it to appear", vmi.Namespace, storagetypes.PVCNameFromVirtVolume(&volume)), reason: common.FailedPvcNotFoundReason}
				} else {
					c.recorder.Eventf(vmi, k8sv1.EventTypeWarning, common.FailedPvcNotFoundReason, "Error determining if volume is ready: %v", err)
					return false, false, common.NewSyncError(
						fmt.Errorf("Error determining if volume is ready %v", err),
						common.FailedDataVolumeImportReason,
					)
				}
			}
...
func (c *VMIController) deleteAllMatchingPods(vmi *virtv1.VirtualMachineInstance) error {
	...
	for _, pod := range pods {
		...
		if err != nil {
			c.podExpectations.DeletionObserved(vmiKey, controller.PodKey(pod))
			c.recorder.Eventf(vmi, k8sv1.EventTypeWarning, common.FailedDeletePodReason, "Failed to delete virtual machine pod %s", pod.Name)
			return err
		}
		c.recorder.Eventf(vmi, k8sv1.EventTypeNormal, common.SuccessfulDeletePodReason, "Deleted virtual machine pod %s", pod.Name)
	}
...
func (c *VMIController) deleteRunningOrFinishedWFFCPods(vmi *virtv1.VirtualMachineInstance, pods ...*k8sv1.Pod) error {
	for _, pod := range pods {
		...
		if err != nil && !k8serrors.IsNotFound(err) {
			c.recorder.Eventf(vmi, k8sv1.EventTypeWarning, common.FailedDeletePodReason, "Failed to delete WaitForFirstConsumer temporary pod %s", pod.Name)
			return err
		}
		c.recorder.Eventf(vmi, k8sv1.EventTypeNormal, common.SuccessfulDeletePodReason, "Deleted WaitForFirstConsumer temporary pod %s", pod.Name)
	}
	return nil
}
...
func (c *VMIController) handleHotplugVolumes(hotplugVolumes []*virtv1.Volume, hotplugAttachmentPods []*k8sv1.Pod, vmi *virtv1.VirtualMachineInstance, virtLauncherPod *k8sv1.Pod, dataVolumes []*cdiv1.DataVolume) common.SyncError {
	logger := log.Log.Object(vmi)
	...
	for _, volume := range hotplugVolumes {
		...
		if err != nil {
			return common.NewSyncError(fmt.Errorf("Error determining volume status %v", err), common.PVCNotReadyReason)
		}
		if wffc {
	...
	for _, attachmentPod := range oldPods {
		if err := c.deleteAttachmentPodForVolume(vmi, attachmentPod); err != nil {
			return common.NewSyncError(fmt.Errorf("Error deleting attachment pod %v", err), common.FailedDeletePodReason)
		}
	}
	return nil
}
...
func (c *VMIController) createAttachmentPod(vmi *virtv1.VirtualMachineInstance, virtLauncherPod *k8sv1.Pod, volumes []*virtv1.Volume) common.SyncError {
	...
	if err != nil {
		c.podExpectations.CreationObserved(vmiKey)
		c.recorder.Eventf(vmi, k8sv1.EventTypeWarning, common.FailedCreatePodReason, "Error creating attachment pod: %v", err)
		return common.NewSyncError(fmt.Errorf("Error creating attachment pod %v", err), common.FailedCreatePodReason)
	}
	c.recorder.Eventf(vmi, k8sv1.EventTypeNormal, common.SuccessfulCreatePodReason, "Created attachment pod %s", pod.Name)
	return nil
}
...
func (c *VMIController) triggerHotplugPopulation(volume *virtv1.Volume, vmi *virtv1.VirtualMachineInstance, virtLauncherPod *k8sv1.Pod) common.SyncError {
	...
	if err != nil {
		return common.NewSyncError(fmt.Errorf("Error creating trigger pod template %v", err), common.FailedCreatePodReason)
	}
	if populateHotplugPodTemplate != nil { // nil means the PVC is not populated yet.
		...
		if err != nil {
			c.podExpectations.CreationObserved(vmiKey)
			c.recorder.Eventf(vmi, k8sv1.EventTypeWarning, common.FailedCreatePodReason, "Error creating hotplug population trigger pod for volume %s: %v", volume.Name, err)
			return common.NewSyncError(fmt.Errorf("Error creating hotplug population trigger pod %v", err), common.FailedCreatePodReason)
		}
		c.recorder.Eventf(vmi, k8sv1.EventTypeNormal, common.SuccessfulCreatePodReason, "Created hotplug trigger pod for volume %s", volume.Name)
	}
	return nil
}
...
func (c *VMIController) deleteAttachmentPodForVolume(vmi *virtv1.VirtualMachineInstance, attachmentPod *k8sv1.Pod) error {
	...
	if err != nil {
		...
		c.recorder.Eventf(vmi, k8sv1.EventTypeWarning, common.FailedDeletePodReason, "Failed to delete attachment pod %s", attachmentPod.Name)
		return err
	}
	c.recorder.Eventf(vmi, k8sv1.EventTypeNormal, common.SuccessfulDeletePodReason, "Deleted attachment pod %s", attachmentPod.Name)
	return nil
}
		...
		if _, ok := hotplugVolumesMap[volume.Name]; ok {
			...
			if attachmentPod == nil {
				...
				if c.canMoveToAttachedPhase(status.Phase) {
					...
					status.Reason = common.SuccessfulCreatePodReason
	...
	for k, v := range oldStatusMap {
		...
		if attachmentPod != nil {
			...
			if attachmentPod.DeletionTimestamp != nil {
				...
				v.Reason = common.SuccessfulDeletePodReason
...
func (c *VMIController) getVolumePhaseMessageReason(volume *virtv1.Volume, namespace string) (virtv1.VolumePhase, string, string) {
	...
	if !pvcExists {
		return virtv1.VolumePending, common.FailedPvcNotFoundReason, "Unable to determine PVC name"
	}
	pvc := pvcInterface.(*k8sv1.PersistentVolumeClaim)
	if pvc.Status.Phase == k8sv1.ClaimPending {
		return virtv1.VolumePending, common.PVCNotReadyReason, "PVC is in phase ClaimPending"
