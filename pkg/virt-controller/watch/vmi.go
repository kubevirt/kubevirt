		...
		if vmiPodExists {
			...
			if vmi.Status.TopologyHints == nil {
				if topologyHints, tscRequirement, err := c.topologyHinter.TopologyHintsForVMI(vmi); err != nil && tscRequirement == topology.RequiredForBoot {
					...
					return &syncErrorImpl{err, common.FailedGatherhingClusterTopologyHints}
		...
		case vmiPodExists:
			...
			if isPodReady(pod) && vmi.DeletionTimestamp == nil {
				...
				if podQosClass != k8sv1.PodQOSGuaranteed && vmi.IsCPUDedicated() {
					...
					syncErr = &syncErrorImpl{fmt.Errorf("failed to guarantee pod resources"), common.FailedGuaranteePodResourcesReason}
					break
				}
...
func (c *VMIController) hotplugPodsReady(vmi *virtv1.VirtualMachineInstance, virtLauncherPod *k8sv1.Pod) (bool, syncError) {
	if controller.VMIHasHotplugVolumes(vmi) {
		...
		if err != nil {
			return false, &syncErrorImpl{fmt.Errorf("failed to get attachment pods: %v", err), common.FailedHotplugSyncReason}
		}
...
func (c *VMIController) sync(vmi *virtv1.VirtualMachineInstance, pod *k8sv1.Pod, dataVolumes []*cdiv1.DataVolume) syncError {
	...
	if vmi.DeletionTimestamp != nil {
		...
		if err != nil {
			return &syncErrorImpl{fmt.Errorf("failed to delete pod: %v", err), common.FailedDeletePodReason}
		}
		return nil
	}
	...
	if vmi.IsFinal() {
		...
		if err != nil {
			return &syncErrorImpl{fmt.Errorf("failed to delete attachment pods: %v", err), common.FailedHotplugSyncReason}
		}
		return nil
	}
	...
	if err != nil {
		return &syncErrorImpl{
			err:    err,
			reason: common.FailedBackendStorageCreateReason,
		}
	}
	...
	if !podExists(pod) {
		...
		if err != nil {
			...
			return &syncErrorImpl{fmt.Errorf("failed to create virtual machine pod: %v", err), common.FailedCreatePodReason}
		}
	...
	if !isWaitForFirstConsumer {
		...
		if err != nil {
			return &syncErrorImpl{fmt.Errorf("failed to clean up temporary pods: %v", err), common.FailedHotplugSyncReason}
		}
	}
	...
	if !isTempPod(pod) && isPodReady(pod) {
		if vmispec.SRIOVInterfaceExist(vmi.Spec.Domain.Devices.Interfaces) {
			...
			if err != nil {
				return &syncErrorImpl{err, common.FailedPodPatchReason}
			}
			*pod = *patchedPod
		}
		...
		if err != nil {
			return &syncErrorImpl{fmt.Errorf("failed to get attachment pods: %v", err), common.FailedHotplugSyncReason}
		}
		...
		if len(vmispec.NetworksToHotplug(vmi.Spec.Networks, vmi.Status.Interfaces)) > 0 {
			if err := c.handleDynamicInterfaceRequests(vmi, pod); err != nil {
				return &syncErrorImpl{
					...
					reason: common.FailedHotplugSyncReason,
				}
			}
		}
	}
		...
		if volume.VolumeSource.DataVolume != nil || volume.VolumeSource.PersistentVolumeClaim != nil {
			...
			if err != nil {
				if _, ok := err.(storagetypes.PvcNotFoundError); ok {
					...
					return false, false, &syncErrorImpl{err: fmt.Errorf("Error determining if volume is ready %v", err), reason: common.FailedDataVolumeImportReason}
				}
			}
...
func (c *VMIController) handleHotplugVolumes(hotplugVolumes []*virtv1.Volume, hotplugAttachmentPods []*k8sv1.Pod, vmi *virtv1.VirtualMachineInstance, virtLauncherPod *k8sv1.Pod, dataVolumes []*cdiv1.DataVolume) syncError {
	...
	for _, volume := range hotplugVolumes {
		...
		if err != nil {
			return &syncErrorImpl{fmt.Errorf("Error determining volume status %v", err), common.PVCNotReadyReason}
		}
		if wffc {
	...
	for _, attachmentPod := range oldPods {
		if err := c.deleteAttachmentPodForVolume(vmi, attachmentPod); err != nil {
			return &syncErrorImpl{fmt.Errorf("Error deleting attachment pod %v", err), common.FailedDeletePodReason}
		}
	}
	return nil
}
...
func (c *VMIController) createAttachmentPod(vmi *virtv1.VirtualMachineInstance, virtLauncherPod *k8sv1.Pod, volumes []*virtv1.Volume) syncError {
	...
	if err != nil {
		...
		return &syncErrorImpl{fmt.Errorf("Error creating attachment pod %v", err), common.FailedCreatePodReason}
	}
...
func (c *VMIController) triggerHotplugPopulation(volume *virtv1.Volume, vmi *virtv1.VirtualMachineInstance, virtLauncherPod *k8sv1.Pod) syncError {
	...
	if err != nil {
		return &syncErrorImpl{fmt.Errorf("Error creating trigger pod template %v", err), common.FailedCreatePodReason}
	}
	if populateHotplugPodTemplate != nil { // nil means the PVC is not populated yet.
		...
		if err != nil {
			...
			return &syncErrorImpl{fmt.Errorf("Error creating hotplug population trigger pod %v", err), common.FailedCreatePodReason}
		}
