...
var _ = Describe("Application", func() {
	...
	It("Reports leader prometheus metric when onStartedLeading is called ", func() {
		...
		app.nodeController = node.NewNodeController(virtClient, nodeInformer, vmiInformer, recorder)
		app.vmiController = vmi.NewVMIController(services.NewTemplateService("a", 240, "b", "c", "d", "e", "f", "g", pvcInformer.GetStore(), virtClient, config, qemuGid, "h"),
			vmiInformer,
			vmInformer,
			podInformer,
			pvcInformer,
		...
		app.vmController = vm.NewVMController(vmiInformer,
			...
			virtClient,
			config)
		app.migrationController = migration.NewMigrationController(services.NewTemplateService("a", 240, "b", "c", "d", "e", "f", "g", pvcInformer.GetStore(), virtClient, config, qemuGid, "h"),
			vmiInformer,
			podInformer,
			migrationInformer,
			nodeInformer,
