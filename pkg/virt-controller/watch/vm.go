...
func (c *VMController) handleMemoryHotplugRequest(vm *virtv1.VirtualMachine, vmi *virtv1.VirtualMachineInstance) error {
	...
	patches := []string{
		...
		fmt.Sprintf(`{ "op": "replace", "path": "/spec/domain/memory/guest", "value": "%s"}`, vmCopyWithInstancetype.Spec.Template.Spec.Domain.Memory.Guest.String()),
	...
	log.Log.Object(vmi).V(4).Infof("hotplugging memory to %s", vmCopyWithInstancetype.Spec.Template.Spec.Domain.Memory.Guest.String())
