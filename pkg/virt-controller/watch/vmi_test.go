...
var _ = Describe("VirtualMachineInstance watcher", func() {
	...
	BeforeEach(func() {
		...
		controller = NewVMIController(services.NewTemplateService("a", "b", "c", config.NewClusterConfig(configMapInformer.GetStore())), vmiInformer, podInformer, recorder, virtClient)
