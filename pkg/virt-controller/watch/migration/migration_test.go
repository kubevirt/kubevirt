...
var _ = Describe("Migration watcher", func() {
	...
	Context("CPU vendor label constraints", func() {
		...
		DescribeTable("should preserve existing CPU vendor label", func(source, expectedVendorLabel string) {
			vmi := newVirtualMachine("testvmi", v1.Running)
			addNodeNameToVMI(vmi, nodeName)
			migration := newMigration("testmigration", vmi.Name, v1.MigrationPending)

			node := newNode(nodeName)
			node.Labels = map[string]string{intelVendorLabel: "true"}

			switch source {
			case "AddedNodeSelector":
				migration.Spec.AddedNodeSelector = map[string]string{amdVendorLabel: "true"}
			case "VMI NodeSelector":
				vmi.Spec.NodeSelector = map[string]string{amdVendorLabel: "true"}
			}
			...
			Expect(targetPod.Spec.NodeSelector).To(HaveKeyWithValue(expectedVendorLabel, "true"))
			Expect(targetPod.Spec.NodeSelector).ToNot(HaveKey(intelVendorLabel))
		},

			Entry("from AddedNodeSelector", "AddedNodeSelector", amdVendorLabel),
			Entry("from VMI NodeSelector", "VMI NodeSelector", amdVendorLabel),
		)
