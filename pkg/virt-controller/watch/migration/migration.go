...
func (c *Controller) listBackoffEligibleMigrations(namespace string, name string) ([]*virtv1.VirtualMachineInstanceMigration, error) {
	...
	evacObjs, err := c.migrationIndexer.ByIndex(controller.ByMigrationTypeIndex, fmt.Sprintf("%s/%s", namespace, controller.EvacuationKeySuffix))
	if err != nil {
		return nil, err
	}

	// get workload migrations
	workObjs, err := c.migrationIndexer.ByIndex(controller.ByMigrationTypeIndex, fmt.Sprintf("%s/%s", namespace, controller.WorkloadKeySuffix))
	if err != nil {
		return nil, err
	}
