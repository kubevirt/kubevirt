...
func (c *Controller) enqueueMigration(obj interface{}) {
	...
	// If the key is already in the queue at active priority, it will keep that priority.
	// If the key is already in the queue at pending priority, it will be bumped to 0 (still below active).
	// If the key is not present in the queue, it will default to a priority of 0.
	c.Queue.Add(key)
}
