...
var _ = Describe("VirtualMachine", func() {
	...
	Context("One valid VirtualMachine controller given", func() {
		...
		It("should create missing DataVolume for VirtualMachineInstance", func() {
			...
			existingDataVolume, _ := createDataVolumeManifest(&vm.Spec.Template.Spec.Volumes[1],
				&vm.ObjectMeta,
				v1.VirtualMachineGroupVersionKind.GroupVersion().String(),
		...
		It("should only start VMI once DataVolumes are complete", func() {
			...
			existingDataVolume, _ := createDataVolumeManifest(&vm.Spec.Template.Spec.Volumes[0],
				&vmi.ObjectMeta,
				v1.VirtualMachineGroupVersionKind.GroupVersion().String(),
				v1.VirtualMachineGroupVersionKind.Kind)
			existingDataVolume.Namespace = "default"
