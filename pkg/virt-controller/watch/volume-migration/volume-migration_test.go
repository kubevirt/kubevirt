...
var _ = Describe("Volume Migration", func() {
	...
	Context("PatchVMIStatusWithMigratedVolumes", func() {
		...
		It("should return VMI migratedVolumes when volumes match but VM status was not updated", func() {
			// Setup: both VMI and VM use the same volume "pvc-b" (volumes already match)
			shouldAddPVCsIntoTheStore([]string{"pvc-b"}, []string{"pvc-b"})
			vmi := libvmi.New(append(addVMIOptionsForVolumes([]string{"pvc-b"}), libvmi.WithNamespace(ns))...)
			vm := libvmi.NewVirtualMachine(libvmi.New(append(addVMIOptionsForVolumes([]string{"pvc-b"}), libvmi.WithNamespace(ns))...))

			// Simulate: VMI has migratedVolumes from a previous successful patch
			// (PatchVMIStatusWithMigratedVolumes succeeded on a prior sync)
			vmi.Status.MigratedVolumes = []v1.StorageMigratedVolumeInfo{
				{
					VolumeName: generateDiskNameFromIndex(0),
			...
			// Simulate: VM status update failed - VM has NO VolumeUpdateState
			// (vm.Status.VolumeUpdateState is nil by default)

			// Act: call GenerateMigratedVolumes
			migVols, err := volumemigration.GenerateMigratedVolumes(pvcStore, vmi, vm)
			Expect(err).ToNot(HaveOccurred())

			// Assert: should return the VMI's migratedVolumes, not an empty list
			Expect(migVols).To(HaveLen(1))
			Expect(migVols[0].SourcePVCInfo.ClaimName).To(Equal("pvc-a"))
			Expect(migVols[0].DestinationPVCInfo.ClaimName).To(Equal("pvc-b"))
		})
	})
