...
func (c *Controller) syncDynamicAnnotationsAndLabelsToPod(vmi *virtv1.VirtualMachineInstance, pod *k8sv1.Pod) (*k8sv1.Pod, error) {
	...
	syncMap := func(keys []string, vmiMap, podMap, podOrigMap map[string]string, subPath string) {
		if podMap == nil {
		...
		if podOrigMap == nil {
			patchSet.AddOption(patch.WithAdd("/metadata/"+subPath, podMap))
		} else {
			patchSet.AddOption(
				patch.WithTest("/metadata/"+subPath, podOrigMap),
				patch.WithReplace("/metadata/"+subPath, podMap),
			)
		}
	}
	...
	syncMap(
		...
		vmi.Labels, podMeta.Labels, pod.ObjectMeta.Labels, "labels",
	)

	syncMap(
		[]string{descheduler.EvictPodAnnotationKeyAlpha, descheduler.EvictPodAnnotationKeyBeta},
		vmi.Annotations, podMeta.Annotations, pod.ObjectMeta.Annotations, "annotations",
	)
	...
	if pod, err = c.clientset.CoreV1().Pods(pod.Namespace).Patch(
