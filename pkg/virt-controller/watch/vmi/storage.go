...
func (c *Controller) updateVolumeStatus(vmi *virtv1.VirtualMachineInstance, virtlauncherPod *k8sv1.Pod) error {
	...
	for i, volume := range vmi.Spec.Volumes {
		...
		if hotplugVolume, ok := hotplugVolumesMap[volume.Name]; ok {
			...
			if hotplugVolume.MemoryDump != nil {
				pvcName := storagetypes.PVCNameFromVirtVolume(hotplugVolume)
				pvc, err := c.clientset.CoreV1().PersistentVolumeClaims(vmi.Namespace).Get(context.Background(), pvcName, v1.GetOptions{})
				if err != nil && !k8serrors.IsNotFound(err) {
					return err
				}
				if pvc != nil {
					status.MemoryDumpVolume.ClaimName = pvc.Name
				}
			}
		}
...
func (c *Controller) getVolumePhaseMessageReason(volume *virtv1.Volume, namespace string) (virtv1.VolumePhase, string, string) {
	...
	if !pvcExists {
		return virtv1.VolumePending, controller.PVCNotFoundReason, fmt.Sprintf("PVC %s not found", claimName)
	}
	pvc := pvcInterface.(*k8sv1.PersistentVolumeClaim)
	if pvc.Status.Phase == k8sv1.ClaimBound {
		return virtv1.VolumeBound, controller.PVCBoundReason, fmt.Sprintf("PVC %s is bound", claimName)
	}
	return virtv1.VolumePending, controller.PVCPendingReason, fmt.Sprintf("PVC %s is pending", claimName)
}
