...
func (c *Controller) processHotplugVolumeStatus(
	...
	if attachmentPod == nil {
		...
		if canMoveToAttachedPhase(status.Phase) {
			...
			status.Message = fmt.Sprintf("Created hotplug attachment pod %s, for volume %s", attachmentPod.Name, volumeName)
...
func (c *Controller) processPVCInfo(status virtv1.VolumeStatus, pvcName string, namespace string, isUtilityVolume bool) (virtv1.VolumeStatus, error) {
	...
	if pvcExists {
		...
		if isUtilityVolume && storagetypes.IsPVCBlock(pvc.Spec.VolumeMode) {
			...
			status.Reason = controller.PVCNotReadyReason
			status.Message = fmt.Sprintf("Utility volume PVC %s must be filesystem mode, not block mode", pvcName)
			log.Log.Errorf("Utility volume %s references block mode PVC %s, but utility volumes require filesystem mode", status.Name, pvcName)
			return status, nil
		}
...
func (c *Controller) updateVolumeStatus(vmi *virtv1.VirtualMachineInstance, virtlauncherPod *k8sv1.Pod) error {
	...
	for _, volume := range vmi.Spec.Volumes {
		...
		if _, ok := hotplugVolumesMap[volume.Name]; ok {
			status = c.processHotplugVolumeStatus(vmi, volume.Name, pvcName, status, attachmentPod)
		}
		if volume.VolumeSource.PersistentVolumeClaim != nil || volume.VolumeSource.DataVolume != nil || volume.VolumeSource.MemoryDump != nil {
			status, err = c.processPVCInfo(status, pvcName, vmi.Namespace, false)
			if err != nil {
				return err
			}
		}
	...
	for _, utilityVolume := range vmi.Spec.UtilityVolumes {
		...
		status = c.processHotplugVolumeStatus(vmi, utilityVolume.Name, utilityVolume.ClaimName, status, attachmentPod)
		status, err = c.processPVCInfo(status, utilityVolume.ClaimName, vmi.Namespace, true)
		if err != nil {
			return err
		}
		newStatus = append(newStatus, status)
