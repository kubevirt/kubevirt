...
var _ = Describe("VirtualMachineInstance watcher", func() {
	...
	Context("On valid VirtualMachineInstance given", func() {
		...
		DescribeTable("should remove the fore ground finalizer if no pod is present and the vmi is in ", func(phase virtv1.VirtualMachineInstancePhase, finalizer string) {
			...
			Entry("succeeded state", virtv1.Succeeded, virtv1.DeprecatedVirtualMachineInstanceFinalizer),
			Entry("failed state", virtv1.Failed, virtv1.DeprecatedVirtualMachineInstanceFinalizer),
			Entry("succeeded state", virtv1.Succeeded, virtv1.VirtualMachineInstanceFinalizer),
			Entry("failed state", virtv1.Failed, virtv1.VirtualMachineInstanceFinalizer),
		)

		DescribeTable("VM controller finalizer", func(hasOwner, vmExists, expectFinalizer bool) {
			vmi := newPendingVirtualMachine("testvmi")
			vmi.Finalizers = append(vmi.Finalizers, virtv1.VirtualMachineControllerFinalizer, virtv1.DeprecatedVirtualMachineInstanceFinalizer)
