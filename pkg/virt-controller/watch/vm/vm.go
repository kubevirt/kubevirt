...
// from the latest revision and the DesiredGeneration from the vm current generation.
...
func (c *Controller) syncVMAnnotationsToVMI(vm *virtv1.VirtualMachine, vmi *virtv1.VirtualMachineInstance) (*virtv1.VirtualMachineInstance, error) {
	...
	for k, v := range vmi.Annotations {
		// Remove deprecated annotation from VMI
		// For more context, see https://github.com/kubevirt/kubevirt/pull/15298
		if k == virtv1.DeprecatedVirtualMachineGenerationAnnotation {
...
func shouldCreateRevision(vmSpec, revisionSpec virtv1.VirtualMachineSpec) bool {
	...
	if vmSpec.Running != nil && *vmSpec.Running == false {
		return false
	}
