...
var _ = Describe("ResourceClaim management", func() {
	...
	Context("rewriteResourceClaimTemplateRefs", func() {
		It("should not be applied by SetupVMIFromVM on its own (rewrite is gated at the call site)", func() {
			...
			Expect(*rc.ResourceClaimTemplateName).To(Equal(templateName))
	...
	Context("hasVMInReservedFor", func() {
		vm := &virtv1.VirtualMachine{
			ObjectMeta: metav1.ObjectMeta{Name: "test-vm", Namespace: "default", UID: "vm-uid"},
		}

		It("should return false when reservedFor is empty", func() {
			claim := &resourcev1.ResourceClaim{}
			Expect(hasVMInReservedFor(claim, vm)).To(BeFalse())
		})

		It("should return true when the VM UID is present", func() {
			claim := &resourcev1.ResourceClaim{
				Status: resourcev1.ResourceClaimStatus{
					ReservedFor: []resourcev1.ResourceClaimConsumerReference{
						{Resource: "pods", Name: "some-pod", UID: "pod-uid"},
						{Resource: "virtualmachines", Name: "test-vm", UID: "vm-uid"},
					},
				},
			}
			Expect(hasVMInReservedFor(claim, vm)).To(BeTrue())
		})

		It("should return false when only other consumers are present", func() {
			claim := &resourcev1.ResourceClaim{
				Status: resourcev1.ResourceClaimStatus{
					ReservedFor: []resourcev1.ResourceClaimConsumerReference{
						{Resource: "pods", Name: "some-pod", UID: "pod-uid"},
