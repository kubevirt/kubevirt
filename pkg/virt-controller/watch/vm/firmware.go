...
func (c *FirmwareController) execute(key string) error {
	...
	patchOps, err := c.generateFirmwarePatch(vm)
	if err != nil {
		return err
	}
...
func (c *FirmwareController) generateFirmwarePatch(vm *virtv1.VirtualMachine) (patch.PatchSet, error) {
	...
	switch {
	case firmware == nil:
		firmware = &virtv1.Firmware{UUID: legacyUUID}
		patchOps.AddOption(patch.WithAdd(firmwarePatchPath, firmware))

	case firmware.UUID == "":
		updatedFirmware := firmware.DeepCopy()
		updatedFirmware.UUID = legacyUUID

		patchOps.AddOption(patch.WithTest(firmwareUUIDPatchPath, ""))
		patchOps.AddOption(patch.WithReplace(firmwarePatchPath, firmware))
	}
