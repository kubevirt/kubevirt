...
var _ = Describe("VirtualMachine", func() {
	...
	Context("One valid VirtualMachine controller given", func() {
		...
		It("should create VMI with ChangedBlockTrackingState when VM matches cbt selector", func() {
			...
			vm, err = virtFakeClient.KubevirtV1().VirtualMachines(vm.Namespace).Get(context.TODO(), vm.Name, metav1.GetOptions{})
			Expect(err).To(Succeed())
			Expect(cbt.GetCBTState(vm.Status.ChangedBlockTracking)).To(Equal(v1.ChangedBlockTrackingInitializing))

			vmi, err := virtFakeClient.KubevirtV1().VirtualMachineInstances(vm.Namespace).Get(context.TODO(), vm.Name, metav1.GetOptions{})
			Expect(err).To(Succeed())
			Expect(cbt.GetCBTState(vmi.Status.ChangedBlockTracking)).To(Equal(v1.ChangedBlockTrackingInitializing))
		})
