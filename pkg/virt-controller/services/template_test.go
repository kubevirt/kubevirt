...
`,
		...
		Context("with ImageVolume", func() {
			...
			DescribeTable("should use image digest extracted from source pod during migration", func(vmi *v1.VirtualMachineInstance, sourcePod *k8sv1.Pod, expectedVolumes []k8sv1.Volume) {
				...
				// Verify that expected image volumes with digests are present
				for _, expectedVolume := range expectedVolumes {
					Expect(targetPod.Spec.Volumes).To(ContainElement(expectedVolume))
				}
			},
				Entry("with container disk using digest",
					libvmi.New(
						libvmi.WithNamespace("default"),
						libvmi.WithContainerDisk("r0", "someImage"),
					),
				...
				Entry("with kernel boot using digest",
					libvmi.New(
						libvmi.WithNamespace("default"),
						libvmi.WithKernelBootContainer("kernelImage"),
					),
				...
				Entry("with both container disk and kernel boot using digests",
					libvmi.New(
						libvmi.WithNamespace("default"),
						libvmi.WithContainerDisk("r0", "someImage"),
						libvmi.WithKernelBootContainer("kernelImage"),
