		...
		Context("launch template with correct parameters", func() {
			...
			DescribeTable("should work", func(arch string, ovmfPath string) {
				...
				Expect(pod.ObjectMeta.Labels).To(Equal(map[string]string{
					...
					v1.VirtualMachineInstanceSelectorLabel: "testvmi",
				}))
			...
			It("Should have a stable label to select virt-launcher pods", func() {
				...
				const expectedVMISelectorLabelValue = "a-very-very-long-virtual-machine-instance-name-used-fo-88ed080c"
				Expect(pod.ObjectMeta.Labels).To(HaveKeyWithValue(v1.VirtualMachineInstanceSelectorLabel, expectedVMISelectorLabelValue))
			})
		})
