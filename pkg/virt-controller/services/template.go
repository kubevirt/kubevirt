...
func (t *TemplateService) renderLaunchManifest(vmi *v1.VirtualMachineInstance, imageIDs map[string]string, backendStoragePVCName string, tempPod bool, memoryOverhead resource.Quantity) (*k8sv1.Pod, error) {
	...
	if pool := poolmatcher.MatchVMIToWorkerPool(pools, vmi); pool != nil {
		if pool.Spec.VirtLauncherImage != "" {
			overrideLauncherImage(&pod, t.launcherImage, pool.Spec.VirtLauncherImage)
		}
		mergePoolNodeSelector(&pod, pool.Spec.NodeSelector)
		pod.Annotations[workerv1.WorkerPoolLabel] = pool.Name
	}
