...
func (t *TemplateService) renderLaunchManifest(vmi *v1.VirtualMachineInstance, imageIDs map[string]string, backendStoragePVCName string, tempPod bool) (*k8sv1.Pod, error) {
	...
	resourceRenderer, err := t.newResourceRenderer(vmi, networkToResourceMap, t.clusterConfig.GetHypervisor().Name)
	if err != nil {
		return nil, err
	}
...
func (t *TemplateService) newResourceRenderer(vmi *v1.VirtualMachineInstance, networkToResourceMap map[string]string, hypervisorName string) (*ResourceRenderer, error) {
	vmiResources := vmi.Spec.Domain.Resources
	hypervisorResource := k8sv1.ResourceName(K8sDevicePrefix + "/" + hypervisor.NewLauncherResourceRenderer(hypervisorName).GetHypervisorDevice())
...
func CalculateMemoryOverhead(clusterConfig *virtconfig.ClusterConfig, netBindingPluginMemoryCalculator netBindingPluginMemoryCalculator, vmi *v1.VirtualMachineInstance) resource.Quantity {
	...
	memoryOverhead := hypervisor.NewLauncherResourceRenderer(clusterConfig.GetHypervisor().Name).GetMemoryOverhead(vmi, vmiCPUArch, clusterConfig.GetConfig().AdditionalGuestMemoryOverheadRatio)
