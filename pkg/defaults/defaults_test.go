...
var _ = Describe("Defaults", func() {
	Context("Architecture", func() {
		Context("VirtualMachine", func() {
			...
			DescribeTable("should default to", func(createVM func() *v1.VirtualMachine, match OmegaMatcher) {
				vm := createVM()
				defaults.SetVirtualMachineDefaults(vm, clusterConfig, nil, virtClient)
				Expect(vm.Spec.Template.Spec.Architecture).To(match)
			},
				Entry("user provided value when provided", func() *v1.VirtualMachine {
					return libvmi.NewVirtualMachine(libvmi.New(libvmi.WithArchitecture(userProvidedArch)))
				...
				}, Equal(configProvidedArch)),
				Entry("complied arch when not provided by user or config", func() *v1.VirtualMachine {
					clusterConfig, _, _ = testutils.NewFakeClusterConfigUsingKVConfig(&v1.KubeVirtConfiguration{})
					return libvmi.NewVirtualMachine(libvmi.New())
				}, Equal(runtime.GOARCH)),
			)
		})
	})
})
