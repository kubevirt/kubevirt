...
func setupCPUHotplug(clusterConfig *virtconfig.ClusterConfig, vmi *v1.VirtualMachineInstance) {
	...
	if vmi.Spec.Domain.CPU.MaxSockets == 0 {
		// Define an implicit upper limit based on hotplug ratio (default: 128 * 4 = 512)
		defaultMaxSocketsLimit := 128 * clusterConfig.GetMaxHotplugRatio()

		// Calculate the max sockets based on hotplug ratio
		...
		// Ensure total VCPUs do not exceed the default limit
		if totalVCPUs > defaultMaxSocketsLimit {
			adjustedSockets := defaultMaxSocketsLimit / (vmi.Spec.Domain.CPU.Cores * vmi.Spec.Domain.CPU.Threads)
