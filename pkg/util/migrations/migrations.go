...
func ListWorkloadUpdateMigrations(indexer cache.Indexer, vmiName, ns string) []v1.VirtualMachineInstanceMigration {
	objs, err := indexer.ByIndex(controller.ByMigrationTypeIndex, fmt.Sprintf("%s/%s", ns, controller.WorkloadKeySuffix))
	if err != nil {
		log.Log.Reason(err).Errorf("Failed to use byMigrationType index for workload")
		return nil
	}
	...
	for _, obj := range objs {
		migration := obj.(*v1.VirtualMachineInstanceMigration)
		if migration.IsFinal() ||
			migration.Spec.VMIName != vmiName ||
			!metav1.HasAnnotation(migration.ObjectMeta, v1.WorkloadUpdateMigrationAnnotation) {
			continue
		}
		migrations = append(migrations, *migration)
	}
