...
var _ = Describe("Hardware utils test", func() {
	...
	Context("devices NUMA affinity", func() {
		...
		It("should prefer NUMATune memnode mapping over vCPU pinning", func() {
			domainSpec := &api.DomainSpec{
				CPU: api.CPU{
					NUMA: &api.NUMA{
						Cells: []api.NUMACell{
							{ID: "0", CPUs: "0", Memory: 2048, Unit: "MiB"},
							{ID: "1", CPUs: "1", Memory: 2048, Unit: "MiB"},
						},
					},
				},
				CPUTune: &api.CPUTune{
		...
		It("should report warnings for ambiguous NUMATune memnode mapping", func() {
			...
			lookupResult := LookupDevicesNumaNodesWithWarnings([]string{testPCIAddress}, domainSpec)
			Expect(lookupResult.NUMANodes).To(Equal(map[string]uint32{testPCIAddress: 0}))
		...
		It("should report warnings for devices without host NUMA affinity", func() {
			domainSpec := &api.DomainSpec{
				CPU: api.CPU{
					NUMA: &api.NUMA{
						Cells: []api.NUMACell{{ID: "0", CPUs: "0", Memory: 2048, Unit: "MiB"}},
					},
				},
				CPUTune: &api.CPUTune{
					VCPUPin: []api.CPUTuneVCPUPin{{VCPU: 0, CPUSet: "0"}},
			...
			lookupResult := LookupDevicesNumaNodesWithWarnings([]string{"0000:ff:00.0"}, domainSpec)
			Expect(lookupResult.NUMANodes).To(BeEmpty())
		...
		It("should not map a device through a pinned vCPU outside guest NUMA cells", func() {
			domainSpec := &api.DomainSpec{
				CPU: api.CPU{
					NUMA: &api.NUMA{
						Cells: []api.NUMACell{{ID: "0", CPUs: "1", Memory: 2048, Unit: "MiB"}},
					},
				},
				CPUTune: &api.CPUTune{
					VCPUPin: []api.CPUTuneVCPUPin{{VCPU: 0, CPUSet: "0"}},
