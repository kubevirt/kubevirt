...
var _ = Describe("PVC utils test", func() {
	...
	Context("Hostpath PV with client", func() {
		pvcCache := cache.NewIndexer(cache.DeletionHandlingMetaNamespaceKeyFunc, nil)
		pvCache := cache.NewIndexer(cache.DeletionHandlingMetaNamespaceKeyFunc, nil)

		boundPvc1 := kubev1.PersistentVolumeClaim{
			TypeMeta:   metav1.TypeMeta{Kind: "PersistentVolumeClaim", APIVersion: "v1"},
			ObjectMeta: metav1.ObjectMeta{Namespace: namespace, Name: "boundPvc1"},
			Spec: kubev1.PersistentVolumeClaimSpec{
				VolumeMode: &modeFile,
				VolumeName: "invalid",
			},
		...
		boundPvc2 := kubev1.PersistentVolumeClaim{
			TypeMeta:   metav1.TypeMeta{Kind: "PersistentVolumeClaim", APIVersion: "v1"},
			ObjectMeta: metav1.ObjectMeta{Namespace: namespace, Name: "boundPvc2"},
			Spec: kubev1.PersistentVolumeClaimSpec{
				VolumeMode: &modeFile,
				VolumeName: "testpv",
			},
		...
		testpv := kubev1.PersistentVolume{
		...
		pvcCache.Add(&boundPvc1)
		pvcCache.Add(&boundPvc2)
		pvcCache.Add(&filePvc1)
		pvCache.Add(&testpv)

		It("Should error on non existing PVC", func() {
			hostpath, err := GetPVCHostPathFromStore(pvcCache, pvCache, namespace, "doesNotExist")
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("Unable to find PVC"))
			Expect(hostpath).To(BeEmpty())
		})

		It("Should error on bound PVC and no matching PV", func() {
			hostpath, err := GetPVCHostPathFromStore(pvcCache, pvCache, namespace, boundPvc1.Name)
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("Unable to find PV invalid"))
			Expect(hostpath).To(BeEmpty())
		})

		It("should return blank on unbound PVC", func() {
			hostpath, err := GetPVCHostPathFromStore(pvcCache, pvCache, namespace, filePvc1.Name)
			Expect(err).ToNot(HaveOccurred())
			Expect(hostpath).To(BeEmpty())
		})

		It("should return path from PV", func() {
			hostpath, err := GetPVCHostPathFromStore(pvcCache, pvCache, namespace, boundPvc2.Name)
			Expect(err).ToNot(HaveOccurred())
			Expect(hostpath).To(Equal("/result"))
		})
	})
})
