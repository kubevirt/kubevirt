...
func (hdc DiskImgCreator) Create(vmi *v1.VirtualMachineInstance) error {
	for _, volume := range vmi.Spec.Volumes {
		if hostDisk := volume.VolumeSource.HostDisk; shouldMountHostDisk(hostDisk) {
			if err := mountHostDiskAndSetOwnership(vmi, hdc, volume.Name, hostDisk); err != nil {
				return err
			}
		}
	}
...
func mountHostDiskAndSetOwnership(vmi *v1.VirtualMachineInstance, hdc DiskImgCreator, volumeName string, hostDisk *v1.HostDisk) error {
	...
	if !fileExists {
		if err := handleRequestedSizeAndCreateSparseRaw(vmi, hdc, diskDir, diskPath, hostDisk); err != nil {
			return err
		}
	} else if err != nil {
		return err
...
func handleRequestedSizeAndCreateSparseRaw(vmi *v1.VirtualMachineInstance, hdc DiskImgCreator, diskDir string, diskPath string, hostDisk *v1.HostDisk) error {
	...
	if requestedSize > availableSize {
		requestedSize, err = shrinkRequestedSize(vmi, hdc, requestedSize, availableSize, hostDisk)
		if err != nil {
			return err
		}
	}
...
func shrinkRequestedSize(vmi *v1.VirtualMachineInstance, hdc DiskImgCreator, requestedSize int64, availableSize int64, hostDisk *v1.HostDisk) (int64, error) {
