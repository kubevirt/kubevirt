...
var _ = Describe("DownwardAPIAttributes", func() {
	...
	// Direct claims:   {base}/{claimName}/{requestName}/metadata.json
	// Template claims: {base}/templates/{podClaimName}/{requestName}/metadata.json
	...
	createTemplateMetadataFile := func(podClaimName, requestName string, md *metadata.DeviceMetadata) {
		dir := filepath.Join(tempDir, templatesSubdir, podClaimName, requestName)
		Expect(os.MkdirAll(dir, 0755)).To(Succeed())
