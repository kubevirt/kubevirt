...
var _ = Describe("DownwardAPIAttributes", func() {
	...
	Context("multiple driver files per request", func() {
		It("should merge metadata from two driver files in the same request directory", func() {
			...
			addr, err := GetPCIAddressForClaim(tempDir, resourceClaims, "my-claim", "gpu-req")
			Expect(err).ToNot(HaveOccurred())
			Expect(addr).To(BeElementOf(pciAddr1, pciAddr2))
		})
	})
