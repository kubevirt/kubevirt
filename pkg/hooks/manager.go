...
func (m *hookManager) collectSideCarSockets(numberOfRequestedHookSidecars uint, timeout time.Duration) (map[string][]*callBackClient, error) {
	...
	for uint(len(processedSockets)) < numberOfRequestedHookSidecars {
		...
		for _, socket := range sockets {
			...
			err := handleSidecarSocket(filepath.Join(m.hookSocketSharedDirectory, socket.Name()), callbacksPerHookPoint)
			if err != nil {
				return nil, err
			}
...
func handleSidecarSocket(filePath string, callbacksPerHookPoint map[string][]*callBackClient) error {
