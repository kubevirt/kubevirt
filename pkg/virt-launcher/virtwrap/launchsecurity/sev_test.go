...
var _ = Describe("LaunchSecurity: AMD Secure Encrypted Virtualization (SEV)", func() {
	Context("SEV policy conversion", func() {
		...
		It("should not set any SNP bits when policy is nil", func() {
			bits := launchsecurity.SEVSNPPolicyToBits(nil)
			expectedMask := launchsecurity.SNPPolicySmt | launchsecurity.SNPPolicyReserved
			Expect(bits & expectedMask).To(Equal(uint(0)))
		})
	})
})
