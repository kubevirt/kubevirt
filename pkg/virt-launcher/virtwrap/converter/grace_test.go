...
var _ = Describe("Grace domain conversion", func() {
	...
	It("adds SMMUv3, hostdev ACPI, GI NUMA cells, distances, and pcihole64 for one Grace GPU", func() {
		...
		fakeRuntime.capabilities["0000:81:00.0"] = gracePCICapabilities{SSIDSize: &ssidSize, OAS: &oas}
		...
		Expect(domainSpec.Devices.IOMMU[0].Driver.PCIBus).To(Equal(graceExpanderBusNumbers(domainSpec)[0]))
		...
		Expect(domainSpec.Devices.IOMMU[0].Driver.SSIDSize).To(Equal(&ssidSize))
		Expect(domainSpec.Devices.IOMMU[0].Driver.OAS).To(Equal(&oas))
		...
		Expect(findGraceSibling(domainSpec.CPU.NUMA.Cells[0].Distances.Siblings, 1).Value).To(Equal(uint(80)))
		Expect(findGraceSibling(domainSpec.CPU.NUMA.Cells[1].Distances.Siblings, 0).Value).To(Equal(uint(80)))
	})
	...
	It("uses one pcie-expander-bus and SMMUv3 IOMMU per Grace GPU", func() {
		...
		Expect([]string{domainSpec.Devices.IOMMU[0].Driver.PCIBus, domainSpec.Devices.IOMMU[1].Driver.PCIBus}).To(ConsistOf(graceExpanderBusNumbers(domainSpec)))
		...
		Expect(rootPCIController(domainSpec).PCIHole64).To(Equal(&api.PCIHole64{Value: 67108864, Unit: "KiB"}))
	})
...
func graceExpanderBusNumbers(domainSpec *api.DomainSpec) []string {
	var busNumbers []string
	for _, controller := range domainSpec.Devices.Controllers {
		if controller.Model != api.ControllerModelPCIeExpanderBus || controller.Target == nil || controller.Target.BusNr == nil {
			continue
		}
		busNumbers = append(busNumbers, strconv.FormatUint(uint64(*controller.Target.BusNr), 10))
	}
	return busNumbers
}
...
func findGraceSibling(siblings []api.NUMACellSibling, targetID uint) *api.NUMACellSibling {
