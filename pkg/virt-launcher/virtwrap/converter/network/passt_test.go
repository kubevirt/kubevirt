	...
	Context("generate domain spec interface", func() {
		DescribeTable("should fail to create configurator given",
			...
			Entry("no pod network",
				libvmi.New(
					libvmi.WithInterface(interfaceWithPasstBinding("default")),
					libvmi.WithNetwork(libvmi.MultusNetwork("default", "test-nad")),
				),
			),
			Entry("no corresponding iface",
				libvmi.New(
					libvmi.WithInterface(interfaceWithPasstBinding("not-default")),
		...
		DescribeTable("should add interface to domain spec given iface with",
			...
			Entry("passt binding plugin",
				interfaceWithPasstBinding("default"),
...
func interfaceWithPasstBinding(name string) v1.Interface {
	return v1.Interface{
		Name: name,
		InterfaceBindingMethod: v1.InterfaceBindingMethod{
			PasstBinding: &v1.InterfacePasstBinding{},
		},
	}
}

func interfaceWithPasstBindingAndPCI(name, pciAddress string) v1.Interface {
	iface := interfaceWithPasstBinding(name)
...
func withSourceDevice(device string) passtOption {
	return func(iface *api.Interface) {
		iface.Source.Device = device
	}
}
