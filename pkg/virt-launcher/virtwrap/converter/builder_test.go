...
var _ = Describe("Domain Builder", func() {
	It("Should succeed when all configurators succeed", func() {
		...
		builder := converter.NewDomainBuilderWithConfigurators(configurator1, configurator2)

		vmi := libvmi.New()
		var domain api.Domain

		Expect(builder.BuildDomainFromVMI(vmi, &domain)).To(Succeed())
	})
	...
	It("Should fail when a single configurator fails", func() {
		...
		Expect(builder.BuildDomainFromVMI(vmi, &domain)).To(MatchError(expectedError))
	})
