  ...
  <shareable></shareable>
	...
	Context("with Intel TDX LaunchSecurity", func() {
		...
		BeforeEach(func() {
			...
			vmi.Spec.Domain.LaunchSecurity = &v1.LaunchSecurity{
				TDX: &v1.TDX{
					Policy: lsec.TDXDisableVEConversion | lsec.TDXPolicyNoDebug,
				},
			}
			vmi.Spec.Domain.Features = &v1.Features{
				SMM: &v1.FeatureState{
		...
		It("should set LaunchSecurity domain element with 'tdx' type and 'NoDebug' policy", func() {
			...
			Expect(domain.Spec.LaunchSecurity.Policy).To(Equal("0x" + strconv.FormatUint(lsec.TDXPolicyNoDebug|lsec.TDXDisableVEConversion, 16)))
		})
	})
