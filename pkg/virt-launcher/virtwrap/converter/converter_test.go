  ...
  <shareable></shareable>
	...
	Context("with v1.VirtualMachineInstance", func() {
		...
		It("should generate the block backingstore disk within the domain", func() {
			...
			domain := vmiToDomain(vmi, &converterTypes.ConverterContext{Architecture: archconverter.NewConverter(runtime.GOARCH), AllowEmulation: true, HypervisorAvailable: true, EphemeraldiskCreator: EphemeralDiskImageCreator, IsBlockPVC: isBlockPVCMap, IsBlockDV: isBlockDVMap})
	...
	Context("graphics and video device", func() {
		...
		DescribeTable("should check autoAttachGraphicsDevices", func(arch string, autoAttach *bool, devices int) {
			...
			domain := vmiToDomain(&vmi, &converterTypes.ConverterContext{AllowEmulation: true, HypervisorAvailable: true, Architecture: archconverter.NewConverter(arch)})
		...
		DescribeTable("Should have one vnc", func(arch string) {
			...
			domain := vmiToDomain(&vmi, &converterTypes.ConverterContext{Architecture: archconverter.NewConverter(arch), AllowEmulation: true, HypervisorAvailable: true})
	...
	Context("HyperV", func() {
		...
		It("should convert hyperv passthrough", func() {
			...
			domain := vmiToDomain(&vmi, &converterTypes.ConverterContext{Architecture: archconverter.NewConverter(runtime.GOARCH), AllowEmulation: true, HypervisorAvailable: true})
