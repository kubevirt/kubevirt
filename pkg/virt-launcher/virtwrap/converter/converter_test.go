  ...
  <shareable></shareable>
	...
	Context("with v1.VirtualMachineInstance", func() {
		...
		BeforeEach(func() {
			...
			vmi.Spec.Domain.Features = &v1.Features{
				...
				Pvspinlock: &v1.FeatureState{Enabled: new(false)},
				Hyperv: &v1.FeatureHyperv{
					Relaxed:   &v1.FeatureState{Enabled: new(false)},
					VAPIC:     &v1.FeatureState{Enabled: new(true)},
					Spinlocks: &v1.FeatureSpinlocks{FeatureState: v1.FeatureState{Enabled: new(true)}},
					VPIndex:   &v1.FeatureState{Enabled: new(true)},
					Runtime:   &v1.FeatureState{Enabled: new(false)},
					...
					VendorID: &v1.FeatureVendorID{
						FeatureState: v1.FeatureState{Enabled: new(false)},
						VendorID:     "myvendor"},
					Frequencies:     &v1.FeatureState{Enabled: new(false)},
					Reenlightenment: &v1.FeatureState{Enabled: new(false)},
					TLBFlush:        &v1.TLBFlush{FeatureState: v1.FeatureState{Enabled: new(true)}},
					IPI:             &v1.FeatureState{Enabled: new(true)},
					EVMCS:           &v1.FeatureState{Enabled: new(false)},
				},
			}
			...
			vmi.Spec.TerminationGracePeriodSeconds = new(int64(5))
		...
		It("should generate the block backingstore disk within the domain", func() {
			...
			domain := vmiToDomain(vmi, &convertertypes.ConverterContext{Architecture: archconverter.NewConverter(runtime.GOARCH), AllowEmulation: true, HypervisorDeviceAvailable: true, EphemeraldiskCreator: EphemeralDiskImageCreator, IsBlockPVC: isBlockPVCMap, IsBlockDV: isBlockDVMap})
		...
		DescribeTable("should add VSOCK section when present",
			func(useVirtioTransitional bool) {
				...
				vmi.Status.VSOCKCID = new(uint32(100))
		...
		DescribeTable("Should set the error policy", func(epolicy *v1.DiskErrorPolicy, expected string) {
			...
			Entry("ErrorPolicy equal to stop", new(v1.DiskErrorPolicyStop), "stop"),
			Entry("ErrorPolicy equal to ignore", new(v1.DiskErrorPolicyIgnore), "ignore"),
			Entry("ErrorPolicy equal to report", new(v1.DiskErrorPolicyReport), "report"),
			Entry("ErrorPolicy equal to enospace", new(v1.DiskErrorPolicyEnospace), "enospace"),
		)
	...
	Context("graphics and video device", func() {
		...
		DescribeTable("should check autoAttachGraphicsDevices", func(arch string, autoAttach *bool, devices int) {
			...
			domain := vmiToDomain(&vmi, &convertertypes.ConverterContext{AllowEmulation: true, HypervisorDeviceAvailable: true, Architecture: archconverter.NewConverter(arch)})
			...
			MultiArchEntry("and add the graphics and video device if it is set to true", new(true), 1),
			MultiArchEntry("and not add the graphics and video device if it is set to false", new(false), 0),
		)
	...
	Context("serial console", func() {
		...
		DescribeTable("should check autoAttachSerialConsole", func(autoAttach *bool, devices int) {
			...
			Entry("and add the serial console if it is set to true", new(true), 1),
			Entry("and not add the serial console if it is set to false", new(false), 0),
		)
	})
	...
	Context("IOThreads", func() {
		...
		It("Should set the iothread pool with the supplementalPool policy", func() {
			...
			vmi := libvmi.New(
				...
				libvmi.WithIOThreads(v1.DiskIOThreads{SupplementalPoolThreadCount: new(count)}),
	...
	Context("virtio block multi-queue", func() {
		...
		It("should assign correct number of queues with CPU hotplug topology", func() {
			...
			domain := vmiToDomain(vmi, &convertertypes.ConverterContext{Architecture: archconverter.NewConverter(runtime.GOARCH), AllowEmulation: true, SMBios: &cmdv1.SMBios{}})
			Expect(domain.Spec.Devices.Disks).To(HaveLen(1))
		...
		DescribeTable("should succeed assigning CPUs to supplementalPool",
			func(vmiAnnotations map[string]string, isolateEmulator bool, vCPUs, iothreadCount uint32, expectedLayout api.CPUTune) {
				...
				vmi := libvmi.New(
					...
					libvmi.WithIOThreadsPolicy(v1.IOThreadsPolicySupplementalPool),
					libvmi.WithIOThreads(v1.DiskIOThreads{SupplementalPoolThreadCount: new(iothreadCount)}),
					libvmi.WithDedicatedCPUPlacement(),
				)
	...
	Context("Correctly handle IsolateEmulatorThread with dedicated cpus", func() {
		DescribeTable("should succeed assigning CPUs to emulatorThread",
			func(cpu v1.CPU, converterContext *convertertypes.ConverterContext, vmiAnnotations map[string]string, expectedEmulatorThreads int) {
				var err error
				domain := &api.Domain{}
			...
			Entry("when EmulatorThreadCompleteToEvenParity is disabled and there is one extra CPU assigned for emulatorThread",
				...
				&convertertypes.ConverterContext{CPUSet: []int{5, 6, 7},
					Topology: &cmdv1.Topology{
						NumaCells: []*cmdv1.Cell{{
							Cpus: []*cmdv1.CPU{
								{Id: 5}, {Id: 6},
			...
			Entry("when EmulatorThreadCompleteToEvenParity is enabled and there is one extra CPU assigned for emulatorThread (odd CPUs)",
				...
				&convertertypes.ConverterContext{CPUSet: []int{5, 6, 7, 8, 9, 10},
					Topology: &cmdv1.Topology{
						NumaCells: []*cmdv1.Cell{{
							Cpus: []*cmdv1.CPU{
			...
			Entry("when EmulatorThreadCompleteToEvenParity is enabled and there are two extra CPUs assigned for emulatorThread (even CPUs)",
				...
				&convertertypes.ConverterContext{CPUSet: []int{5, 6, 7, 8, 9, 10, 11, 12},
					Topology: &cmdv1.Topology{
						NumaCells: []*cmdv1.Cell{{
							Cpus: []*cmdv1.CPU{
								{Id: 5}, {Id: 6}, {Id: 7}, {Id: 8}, {Id: 9}, {Id: 10},
		...
		DescribeTable("should fail assigning CPUs to emulatorThread",
			func(cpu v1.CPU, converterContext *convertertypes.ConverterContext, vmiAnnotations map[string]string, expectedErrorString string) {
				var err error
				domain := &api.Domain{}
			...
			Entry("when EmulatorThreadCompleteToEvenParity is disabled and there are not enough CPUs to allocate emulator threads",
				v1.CPU{Sockets: 1, Cores: 2, Threads: 1},
				&convertertypes.ConverterContext{CPUSet: []int{5, 6},
					Topology: &cmdv1.Topology{
						NumaCells: []*cmdv1.Cell{{
							Cpus: []*cmdv1.CPU{
								{Id: 5}, {Id: 6},
	...
	Context("virtio-net multi-queue", func() {
		...
		BeforeEach(func() {
			...
			vmi.Spec.Domain.Devices.NetworkInterfaceMultiQueue = new(true)
			vmi.Spec.Domain.Resources.Requests = k8sv1.ResourceList{
				k8sv1.ResourceMemory: resource.MustParse("8192Ki"),
				k8sv1.ResourceCPU:    resource.MustParse("2"),
			}
	...
	Context("Realtime", func() {
		var vmi *v1.VirtualMachineInstance
		var rtContext *convertertypes.ConverterContext
		BeforeEach(func() {
			rtContext = &convertertypes.ConverterContext{
				Architecture:   archconverter.NewConverter(runtime.GOARCH),
				AllowEmulation: true,
	...
	Context("Bootloader", func() {
		...
		DescribeTable("EFI bootloader", func(secureBoot *bool, efiCode, efiVars string) {
			...
			Entry("should use SecureBoot", new(true), "OVMF_CODE.secboot.fd", "OVMF_VARS.secboot.fd"),
			Entry("should use SecureBoot when SB not defined", nil, "OVMF_CODE.secboot.fd", "OVMF_VARS.secboot.fd"),
			Entry("should not use SecureBoot", new(false), "OVMF_CODE.fd", "OVMF_VARS.fd"),
			Entry("should not use SecureBoot when OVMF_CODE.fd not present", new(true), "OVMF_CODE.secboot.fd", "OVMF_VARS.fd"),
		)
	...
	Context("with AMD SEV LaunchSecurity", func() {
		...
		BeforeEach(func() {
			...
			v1.SetObjectDefaults_VirtualMachineInstance(vmi)
			vmi.Spec.Domain.Devices.Rng = &v1.Rng{}
			vmi.Spec.Domain.Devices.AutoattachMemBalloon = new(true)
	...
	Context("when TSC Frequency", func() {
		...
		BeforeEach(func() {
			vmi = kvapi.NewMinimalVMI("testvmi")
			v1.SetObjectDefaults_VirtualMachineInstance(vmi)
			vmi.Status.TopologyHints = &v1.TopologyHints{TSCFrequency: new(int64(fakeFrequency))}
			c = &convertertypes.ConverterContext{
				Architecture:   archconverter.NewConverter(runtime.GOARCH),
				AllowEmulation: true,
			}
	...
	Context("with Paused strategy", func() {
		...
		DescribeTable("bootmenu should be", func(startPaused bool) {
			...
			if startPaused {
				vmi.Spec.StartStrategy = new(v1.StartStrategyPaused)
			}
			domain := vmiToDomain(vmi, c)
			Expect(domain).ToNot(BeNil())
	...
	Context("TPM", func() {
		DescribeTable("should", func(vmiTPM *v1.TPMDevice, matcher types.GomegaMatcher) {
			...
			domain := vmiToDomain(
				vmi,
				&convertertypes.ConverterContext{
					Architecture:   archconverter.NewConverter(runtime.GOARCH),
					AllowEmulation: true,
				},
			)
			...
			Entry("be enabled within domain when device provided in VMI with persistent=true",
				&v1.TPMDevice{Persistent: new(true)},
				ContainElement(api.TPM{
					Model: "tpm-crb",
					Backend: api.TPMBackend{
						Type:            "emulator",
			...
			Entry("not be present within domain when explicitly disabled in VMI",
				&v1.TPMDevice{Enabled: new(false), Persistent: new(true)},
				BeEmpty(),
			),
		)
	})
...
func vmiToDomain(vmi *v1.VirtualMachineInstance, c *convertertypes.ConverterContext) *api.Domain {
	domain := &api.Domain{}
...
func vmiToDomainXMLToDomainSpec(vmi *v1.VirtualMachineInstance, c *convertertypes.ConverterContext) *api.DomainSpec {
...
func vmiArchMutate(arch string, vmi *v1.VirtualMachineInstance, c *convertertypes.ConverterContext) {
	switch arch {
	case arm64:
