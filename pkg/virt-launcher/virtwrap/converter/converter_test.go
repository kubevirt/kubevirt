...
import (
	...
	ls "kubevirt.io/kubevirt/pkg/virt-launcher/virtwrap/launchsecurity"
)
  ...
  <shareable></shareable>
	...
	Context("with AMD SEV LaunchSecurity", func() {
		...
		It("should set LaunchSecurity domain element with 'sev' type and 'NoDebug' policy", func() {
			...
			Expect(domain.Spec.LaunchSecurity.Policy).To(Equal("0x" + strconv.FormatUint(uint64(ls.SEVPolicyNoDebug), 16)))
		})
		...
		It("should set LaunchSecurity domain element with 'sev' type with 'NoDebug' and 'EncryptedState' policy bits", func() {
			...
			Expect(domain.Spec.LaunchSecurity).ToNot(BeNil())
			Expect(domain.Spec.LaunchSecurity.Type).To(Equal("sev"))
			Expect(domain.Spec.LaunchSecurity.Policy).To(Equal("0x" + strconv.FormatUint(uint64(ls.SEVPolicyNoDebug|ls.SEVPolicyEncryptedState), 16)))
		})
	...
	Context("with Intel TDX LaunchSecurity", func() {
		...
		BeforeEach(func() {
			...
			vmi.Spec.Domain.LaunchSecurity = &v1.LaunchSecurity{
				TDX: &v1.TDX{
					Policy: ls.TDXDisableVEConversion | ls.TDXPolicyNoDebug,
				},
			}
			vmi.Spec.Domain.Features = &v1.Features{
				SMM: &v1.FeatureState{
		...
		It("should set LaunchSecurity domain element with 'tdx' type and 'NoDebug' policy", func() {
			...
			Expect(domain.Spec.LaunchSecurity.Policy).To(Equal("0x" + strconv.FormatUint(ls.TDXPolicyNoDebug|ls.TDXDisableVEConversion, 16)))
		})
	})
