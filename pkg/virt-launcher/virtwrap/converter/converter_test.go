...
var _ = Describe("Converter", func() {
	...
	Context("with v1.Disk", func() {
		DescribeTable("Should define disk capacity as the minimum of capacity and request", func(arch string, requests, capacity, expected int64) {
			context := &ConverterContext{Architecture: archconverter.NewArchConverter(arch)}
			v1Disk := v1.Disk{
				Name: "myvolume",
  ...
  <shareable></shareable>
	...
	Context("with v1.VirtualMachineInstance", func() {
		...
		DescribeTable("should use virtio-transitional models if requested", func(arch string) {
			c.Architecture = archconverter.NewArchConverter(arch)
			v1.SetObjectDefaults_VirtualMachineInstance(vmi)
		...
		Context("when all addresses should be places at the root complex", func() {
			It("should be converted to a libvirt Domain with vmi defaults set", func() {
				v1.SetObjectDefaults_VirtualMachineInstance(vmi)
				vmi.Spec.Domain.Devices.Rng = &v1.Rng{}
				c.Architecture = archconverter.NewArchConverter(amd64)
		...
		Context("when CPU spec defined", func() {
			...
			It("should not define hotplugable topology for ARM64", func() {
				...
				c.Architecture = archconverter.NewArchConverter(arm64)
				domainSpec := vmiToDomainXMLToDomainSpec(vmi, c)
				Expect(domainSpec.CPU.Topology.Cores).To(Equal(uint32(2)), "Expect cores")
				Expect(domainSpec.CPU.Topology.Sockets).To(Equal(uint32(2)), "Expect sockets")
				Expect(domainSpec.CPU.Topology.Threads).To(Equal(uint32(1)), "Expect threads")
		...
		It("should generate the block backingstore disk within the domain", func() {
			...
			domain := vmiToDomain(vmi, &ConverterContext{Architecture: archconverter.NewArchConverter(runtime.GOARCH), AllowEmulation: true, EphemeraldiskCreator: EphemeralDiskImageCreator, IsBlockPVC: isBlockPVCMap, IsBlockDV: isBlockDVMap})
	...
	Context("graphics and video device", func() {
		...
		DescribeTable("should check autoAttachGraphicsDevices", func(arch string, autoAttach *bool, devices int) {
			...
			domain := vmiToDomain(&vmi, &ConverterContext{AllowEmulation: true, Architecture: archconverter.NewArchConverter(arch)})
		...
		DescribeTable("Should have one vnc", func(arch string) {
			...
			domain := vmiToDomain(&vmi, &ConverterContext{Architecture: archconverter.NewArchConverter(arch), AllowEmulation: true})
	...
	Context("HyperV", func() {
		DescribeTable("should convert hyperv features", func(hyperV *v1.FeatureHyperv, result *api.FeatureHyperv) {
			...
			domain := vmiToDomain(&vmi, &ConverterContext{Architecture: archconverter.NewArchConverter(runtime.GOARCH), AllowEmulation: true})
	...
	Context("IOThreads", func() {
		...
		DescribeTable("Should use correct IOThreads policies", func(policy v1.IOThreadsPolicy, cpuCores int, threadCount int, threadIDs []int) {
			...
			domain := vmiToDomain(&vmi, &ConverterContext{Architecture: archconverter.NewArchConverter(runtime.GOARCH), AllowEmulation: true, EphemeraldiskCreator: EphemeralDiskImageCreator})
	...
	Context("virtio block multi-queue", func() {
		...
		BeforeEach(func() {
			context = &ConverterContext{Architecture: archconverter.NewArchConverter(runtime.GOARCH), UseVirtioTransitional: false}
			vmi = &v1.VirtualMachineInstance{
				ObjectMeta: k8smeta.ObjectMeta{
					Name:      "testvmi",
					Namespace: "mynamespace",
		...
		It("should assign correct number of queues with CPU hotplug topology", func() {
			...
			domain := vmiToDomain(vmi, &ConverterContext{Architecture: archconverter.NewArchConverter(runtime.GOARCH), AllowEmulation: true, SMBios: &cmdv1.SMBios{}})
	...
	Context("Correctly handle iothreads with dedicated cpus", func() {
		...
		It("assigns a set of cpus per iothread, if there are more vcpus than iothreads", func() {
			...
			v1.SetObjectDefaults_VirtualMachineInstance(vmi)
			c := &ConverterContext{Architecture: archconverter.NewArchConverter(runtime.GOARCH),
	...
	Context("Bootloader", func() {
		...
		DescribeTable("display device should be set to", func(arch string, bootloader v1.Bootloader, enableFG bool, expectedDevice string) {
			...
			c = &ConverterContext{
				Architecture:      archconverter.NewArchConverter(arch),
...
func diskToDiskXML(arch string, disk *v1.Disk) string {
	...
	libvirtDisk := &api.Disk{}
	Expect(Convert_v1_Disk_To_api_Disk(&ConverterContext{Architecture: archconverter.NewArchConverter(arch), UseVirtioTransitional: false}, disk, libvirtDisk, devicePerBus, nil, make(map[string]v1.VolumeStatus))).To(Succeed())
	data, err := xml.MarshalIndent(libvirtDisk, "", "  ")
	Expect(err).ToNot(HaveOccurred())
	return string(data)
}
