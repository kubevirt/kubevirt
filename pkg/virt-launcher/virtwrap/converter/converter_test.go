...
var _ = Describe("Converter", func() {
	...
	Context("with v1.Disk", func() {
		DescribeTable("Should define disk capacity as the minimum of capacity and request", func(arch string, requests, capacity, expected int64) {
			context := &converter_types.ConverterContext{Architecture: archconverter.NewConverter(arch)}
			v1Disk := v1.Disk{
				Name: "myvolume",
		...
		DescribeTable("Should assign scsi controller to", func(diskDevice v1.DiskDevice) {
			context := &converter_types.ConverterContext{}
			v1Disk := v1.Disk{
				Name:       "myvolume",
  ...
  <shareable></shareable>
	...
	Context("with v1.VirtualMachineInstance", func() {
		...
		var c *converter_types.ConverterContext
		...
		It("should generate the block backingstore disk within the domain", func() {
			...
			domain := vmiToDomain(vmi, &converter_types.ConverterContext{Architecture: archconverter.NewConverter(runtime.GOARCH), AllowEmulation: true, HypervisorDeviceAvailable: true, EphemeraldiskCreator: EphemeralDiskImageCreator, IsBlockPVC: isBlockPVCMap, IsBlockDV: isBlockDVMap})
		...
		Context("with CBT volumes", func() {
			...
			DescribeTable("should create domain disk with datastore for block volumes with CBT enabled",
				func(volumeName string, createVolumeSource func(string) v1.VolumeSource, setupContext func(*converter_types.ConverterContext, string)) {
					cbtPath := "/var/lib/libvirt/qemu/cbt/" + volumeName + ".qcow2"
				...
				Entry("PVC", "test-block-pvc",
					...
					func(c *converter_types.ConverterContext, name string) {
	...
	Context("graphics and video device", func() {
		...
		DescribeTable("should check autoAttachGraphicsDevices", func(arch string, autoAttach *bool, devices int) {
			...
			domain := vmiToDomain(&vmi, &converter_types.ConverterContext{AllowEmulation: true, HypervisorDeviceAvailable: true, Architecture: archconverter.NewConverter(arch)})
		...
		DescribeTable("Should have one vnc", func(arch string) {
			...
			domain := vmiToDomain(&vmi, &converter_types.ConverterContext{Architecture: archconverter.NewConverter(arch), AllowEmulation: true, HypervisorDeviceAvailable: true})
	...
	Context("HyperV", func() {
		DescribeTable("should convert hyperv features", func(hyperV *v1.FeatureHyperv, result *api.FeatureHyperv) {
			...
			domain := vmiToDomain(&vmi, &converter_types.ConverterContext{Architecture: archconverter.NewConverter(runtime.GOARCH), AllowEmulation: true})
		...
		It("should convert hyperv passthrough", func() {
			...
			domain := vmiToDomain(&vmi, &converter_types.ConverterContext{Architecture: archconverter.NewConverter(runtime.GOARCH), AllowEmulation: true, HypervisorDeviceAvailable: true})
	...
	Context("IOThreads", func() {
		...
		DescribeTable("Should use correct IOThreads policies", func(policy v1.IOThreadsPolicy, cpuCores int, threadCount int, threadIDs []int) {
			...
			domain := vmiToDomain(&vmi, &converter_types.ConverterContext{Architecture: archconverter.NewConverter(runtime.GOARCH), AllowEmulation: true, EphemeraldiskCreator: EphemeralDiskImageCreator})
	...
	Context("virtio block multi-queue", func() {
		var vmi *v1.VirtualMachineInstance
		var context *converter_types.ConverterContext

		BeforeEach(func() {
			context = &converter_types.ConverterContext{Architecture: archconverter.NewConverter(runtime.GOARCH), UseVirtioTransitional: false}
			vmi = &v1.VirtualMachineInstance{
				ObjectMeta: k8smeta.ObjectMeta{
					Name:      "testvmi",
					Namespace: "mynamespace",
		...
		It("should assign correct number of queues with CPU hotplug topology", func() {
			...
			domain := vmiToDomain(vmi, &converter_types.ConverterContext{Architecture: archconverter.NewConverter(runtime.GOARCH), AllowEmulation: true, SMBios: &cmdv1.SMBios{}})
			Expect(domain.Spec.Devices.Disks).To(HaveLen(1))
	...
	Context("Correctly handle IsolateEmulatorThread with dedicated cpus", func() {
		DescribeTable("should succeed assigning CPUs to emulatorThread",
			func(cpu v1.CPU, converterContext *converter_types.ConverterContext, vmiAnnotations map[string]string, expectedEmulatorThreads int) {
				var err error
				domain := &api.Domain{}
			...
			Entry("when EmulatorThreadCompleteToEvenParity is disabled and there is one extra CPU assigned for emulatorThread",
				...
				&converter_types.ConverterContext{CPUSet: []int{5, 6, 7},
					Topology: &cmdv1.Topology{
						NumaCells: []*cmdv1.Cell{{
							Cpus: []*cmdv1.CPU{
								{Id: 5}, {Id: 6},
			...
			Entry("when EmulatorThreadCompleteToEvenParity is enabled and there is one extra CPU assigned for emulatorThread (odd CPUs)",
				...
				&converter_types.ConverterContext{CPUSet: []int{5, 6, 7, 8, 9, 10},
					Topology: &cmdv1.Topology{
						NumaCells: []*cmdv1.Cell{{
							Cpus: []*cmdv1.CPU{
			...
			Entry("when EmulatorThreadCompleteToEvenParity is enabled and there are two extra CPUs assigned for emulatorThread (even CPUs)",
				...
				&converter_types.ConverterContext{CPUSet: []int{5, 6, 7, 8, 9, 10, 11, 12},
					Topology: &cmdv1.Topology{
						NumaCells: []*cmdv1.Cell{{
							Cpus: []*cmdv1.CPU{
		...
		DescribeTable("should fail assigning CPUs to emulatorThread",
			func(cpu v1.CPU, converterContext *converter_types.ConverterContext, vmiAnnotations map[string]string, expectedErrorString string) {
				var err error
				domain := &api.Domain{}
			...
			Entry("when EmulatorThreadCompleteToEvenParity is disabled and there are not enough CPUs to allocate emulator threads",
				v1.CPU{Sockets: 1, Cores: 2, Threads: 1},
				&converter_types.ConverterContext{CPUSet: []int{5, 6},
					Topology: &cmdv1.Topology{
						NumaCells: []*cmdv1.Cell{{
							Cpus: []*cmdv1.CPU{
								{Id: 5}, {Id: 6},
	...
	Context("Correctly handle iothreads with dedicated cpus", func() {
		...
		It("assigns a set of cpus per iothread, if there are more vcpus than iothreads", func() {
			...
			v1.SetObjectDefaults_VirtualMachineInstance(vmi)
			c := &converter_types.ConverterContext{Architecture: archconverter.NewConverter(runtime.GOARCH),
	...
	Context("Realtime", func() {
		var vmi *v1.VirtualMachineInstance
		var rtContext *converter_types.ConverterContext
		BeforeEach(func() {
			rtContext = &converter_types.ConverterContext{
				Architecture:   archconverter.NewConverter(runtime.GOARCH),
				AllowEmulation: true,
	...
	Context("Bootloader", func() {
		...
		DescribeTable("EFI bootloader", func(secureBoot *bool, efiCode, efiVars string) {
			c.EFIConfiguration = &converter_types.EFIConfiguration{
				EFICode:      efiCode,
				EFIVars:      efiVars,
		...
		DescribeTable("display device should be set to", func(arch string, bootloader v1.Bootloader, enableFG bool, expectedDevice string) {
			...
			c = &converter_types.ConverterContext{
				...
				VirtualMachine:    vmi,
				AllowEmulation:    true,
				EFIConfiguration:  &converter_types.EFIConfiguration{},
			}
			domainSpec := vmiToDomainXMLToDomainSpec(vmi, c)
	...
	Context("hotplug", func() {
		...
		Context("disk", func() {
			...
			type ConverterFunc = func(name string, disk *api.Disk, c *converter_types.ConverterContext) error
		...
		Context("memory", func() {
			...
			DescribeTable("should correctly convert memory configuration from VMI spec to domain",
				func(expectedMemoryMiB int64, opts ...libvmi.Option) {
					...
					testContext := &converter_types.ConverterContext{
						VirtualMachine: vmi,
						AllowEmulation: true,
						Architecture:   archconverter.NewConverter(runtime.GOARCH),
					}
	...
	Context("TPM", func() {
		DescribeTable("should", func(vmiTPM *v1.TPMDevice, matcher types.GomegaMatcher) {
			...
			domain := vmiToDomain(
				vmi,
				&converter_types.ConverterContext{
					Architecture:   archconverter.NewConverter(runtime.GOARCH),
					AllowEmulation: true,
				},
			)
...
func diskToDiskXML(arch string, disk *v1.Disk) string {
	...
	libvirtDisk := &api.Disk{}
	Expect(Convert_v1_Disk_To_api_Disk(&converter_types.ConverterContext{Architecture: archconverter.NewConverter(arch), UseVirtioTransitional: false}, disk, libvirtDisk, devicePerBus, nil, make(map[string]v1.VolumeStatus))).To(Succeed())
	data, err := xml.MarshalIndent(libvirtDisk, "", "  ")
	Expect(err).ToNot(HaveOccurred())
	return string(data)
}

func vmiToDomainXML(vmi *v1.VirtualMachineInstance, c *converter_types.ConverterContext) string {
	domain := vmiToDomain(vmi, c)
...
func vmiToDomain(vmi *v1.VirtualMachineInstance, c *converter_types.ConverterContext) *api.Domain {
	domain := &api.Domain{}
...
func vmiToDomainXMLToDomainSpec(vmi *v1.VirtualMachineInstance, c *converter_types.ConverterContext) *api.DomainSpec {
...
func vmiArchMutate(arch string, vmi *v1.VirtualMachineInstance, c *converter_types.ConverterContext) {
	switch arch {
	case arm64:
