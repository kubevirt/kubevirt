...
var _ = Describe("Converter", func() {
  ...
  <alias name="ua-mydisk"></alias>
		...
		DescribeTable("Should handle custom block sizes correctly per architecture", func(arch string, logical, physical uint, shouldSucceed bool) {
			...
			err := Convert_v1_BlockSize_To_api_BlockIO(kubevirtDisk, libvirtDisk, archconverter.NewConverter(arch))
			if shouldSucceed {
				Expect(err).ToNot(HaveOccurred())
  ...
  <blockio logical_block_size="%d" physical_block_size="%d" discard_granularity="%d"></blockio>
			...
			Entry("on amd64", amd64, uint(1234), uint(1234), true),
			Entry("on arm64", arm64, uint(1234), uint(1234), true),
	...
	Context("with v1.VirtualMachineInstance", func() {
		...
		Context("BlockIO", func() {
			It("Should detect disk block sizes for a file DiskSource", func() {
				...
				Expect(Convert_v1_BlockSize_To_api_BlockIO(&v1Disk, &apiDisk, archconverter.NewConverter(amd64))).To(Succeed())
			...
			It("Should fail for non-file or non-block devices", func() {
				...
				Expect(Convert_v1_BlockSize_To_api_BlockIO(&v1Disk, &apiDisk, archconverter.NewConverter(amd64))).To(MatchError(ContainSubstring(blockIoConfigErrorMessage)))
			})
		})
	})
