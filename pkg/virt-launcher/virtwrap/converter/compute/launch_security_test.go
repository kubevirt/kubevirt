...
var _ = Describe("configureSEVLaunchSecurity", func() {
	It("should configure basic SEV with default policy", func() {
		...
		result := launchsecurity.ConfigureSEVLaunchSecurity(sevConfig)
...
var _ = Describe("configureSEVSNPLaunchSecurity", func() {
	It("should configure basic SEV-SNP with default policy", func() {
		...
		result := launchsecurity.ConfigureSEVSNPLaunchSecurity(snpConfig)
	...
	It("should configure SEV-SNP with all fields", func() {
		snpConfig := &v1.SEVSNP{
			VCEK:            "test-vcek",
			AuthorKey:       pointer.P(true),
			KernelHashes:    pointer.P(false),
			Cbitpos:         "51",
		...
		Expect(result.LaunchSecuritySNP.AuthorKey).To(HaveValue(BeTrue()))
		Expect(result.LaunchSecuritySEVCommon.KernelHashes).To(Equal("no"))
