...
var _ = Describe("Memory Domain Configurator", func() {
	...
	Context("configure multiple memory fields", func() {
		...
		DescribeTable("maxGuest and guest memory settings",
			func(guestMemory *resource.Quantity, maxGuestMemory *resource.Quantity, expectedGuestMemory *resource.Quantity, expectedMaxMemory *resource.Quantity) {
				vmiOptions := []libvmi.Option{
					libvmi.WithGuestMemory(guestMemory.String()),
				}
				if maxGuestMemory != nil {
					vmiOptions = append(vmiOptions, libvmi.WithMaxGuest(maxGuestMemory.String()))
				}
				vmi := libvmi.New(
					vmiOptions...,
				)
			...
			Entry("maxGuest is missing", &guestMemory, nil, &guestMemory, nil),
			Entry("maxGuest equal to guest memory", &guestMemory, &guestMemory, &guestMemory, nil),
			Entry("maxGuest greater than guest memory", &guestMemory, &maxGuestMemory, &guestMemory, &maxGuestMemory),
		)
