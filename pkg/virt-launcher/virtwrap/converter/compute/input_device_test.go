...
var _ = Describe("Input Device Configurator", func() {
	Context("User-specified input devices", func() {
		DescribeTable("should configure user-specified input devices", func(bus v1.InputBus, expectedModel string) {
			vmi := libvmi.New(libvmi.WithTablet("my-tablet", bus))
			...
			Entry("with USB bus", v1.InputBusUSB, ""),
			Entry("with virtio bus", v1.InputBusVirtio, v1.VirtIO),
		)
	...
	Context("Architecture-specific input devices", func() {
		DescribeTable("should add architecture-specific input devices when AutoattachGraphicsDevice is nil or true",
			func(arch string, autoattachGraphicsDevice *bool, expectedInputs []api.Input) {
				vmi := libvmi.New()
				...
				expectedDomain := api.Domain{Spec: api.DomainSpec{Devices: api.Devices{Inputs: expectedInputs}}}
				Expect(domain).To(Equal(expectedDomain))
			},
		...
		DescribeTable("should not add architecture-specific input devices when AutoattachGraphicsDevice is false",
			func(arch string) {
				...
				expectedDomain := api.Domain{Spec: api.DomainSpec{Devices: api.Devices{Inputs: nil}}}
				Expect(domain).To(Equal(expectedDomain))
			},
			Entry("amd64", "amd64"),
			Entry("arm64", "arm64"),
