...
func (i InputDeviceDomainConfigurator) Configure(vmi *v1.VirtualMachineInstance, domain *api.Domain) error {
	if vmi.Spec.Domain.Devices.Inputs != nil {
		for idx := range vmi.Spec.Domain.Devices.Inputs {
			inputDevice := api.Input{}
			if err := convertInputDevice(&vmi.Spec.Domain.Devices.Inputs[idx], &inputDevice); err != nil {
				return err
			}
			domain.Spec.Devices.Inputs = append(domain.Spec.Devices.Inputs, inputDevice)
		}
	}
...
func (i InputDeviceDomainConfigurator) addArchitectureSpecificInputDevices(vmi *v1.VirtualMachineInstance, domain *api.Domain) error {
	...
	default:
		return fmt.Errorf("unsupported architecture: %s", i.architecture)
	}
	return nil
}
...
func convertInputDevice(input *v1.Input, inputDevice *api.Input) error {
