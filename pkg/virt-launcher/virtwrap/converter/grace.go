...
func applyGraceNUMADistances(domainSpec *api.DomainSpec, guestToHostNUMA map[uint32]uint32) error {
	...
	guestNodeIDs := sortedUint32MapKeys(guestToHostNUMA)
	for cellIndex := range domainSpec.CPU.NUMA.Cells {
		...
		for _, guestSiblingID := range guestNodeIDs {
			...
			if distance > uint64(^uint(0)) {
				return fmt.Errorf("host NUMA distance from node %d to node %d overflows uint: %d", hostNode, hostSiblingID, distance)
			}
			siblings = append(siblings, api.NUMACellSibling{
				ID:    uint(guestSiblingID),
				Value: uint(distance),
			})
		}
...
func applyGracePCIHole64(domainSpec *api.DomainSpec, pciHoleBytes uint64) error {
	...
	if pciHoleKiB == 0 || pciHoleKiB > uint64(math.MaxUint) {
	...
	return fmt.Errorf("GraceIOVirtualization requires a PCIe root controller for pcihole64 sizing")
}
...
func sortedUint32MapKeys[T any](values map[uint32]T) []uint32 {
