	...
	if options.StallDetectionEnabled {
		...
		monitor.logger.V(3).Infof(
			"initialized migration monitor: stallDetection=%t progressTimeout=%ds completionTimeoutPerGiB=%d maxDowntimeMs=%d allowPostCopy=%t allowWorkloadDisruption=%t "+
				"stallMargin=%.2f stallProgressTimeout=%ds switchoverTimeout=%ds preCopyPossibleFactor=%s patienceWindowDecayFactor=%s bandwidthEWMAAlpha=%s searchLocalMinima=%t completionTimeoutFactor=%s",
			...
			options.StallDetectorOptions.PrecopyPossibleFactor.String(),
			options.StallDetectorOptions.PatienceWindowDecayFactor.String(),
			options.StallDetectorOptions.EwmaAlpha.String(),
			options.StallDetectorOptions.SearchLocalMinima,
			options.StallDetectorOptions.CompletionTimeoutFactor.String(),
		)
...
func (m *migrationMonitor) scaledCompletionDeadlineSeconds(baseSeconds int64) int64 {
	...
	m.logger.V(4).Infof("scaledCompletionDeadlineSeconds: baseSeconds=%ds, completionTimeoutFactor=%s", baseSeconds, m.options.StallDetectorOptions.CompletionTimeoutFactor.String())
