	...
	for i, disk := range domSpec.Devices.Disks {
		...
		if disk.Source.File != "" {
			if domcfgDisk.Source == nil || domcfgDisk.Source.File == nil {
				return fmt.Errorf("disk %s: spec has file source but domcfgDisk %+v is missing the File structure, skipping file update", disk.Alias.GetName(), domcfgDisk)
			}
			log.Log.Infof("Updating disk %s source file from %s to %s", disk.Alias.GetName(), domcfgDisk.Source.File.File, disk.Source.File)
		...
		if disk.Source.DataStore != nil && disk.Source.DataStore.Source != nil && disk.Source.DataStore.Source.File != "" {
			if domcfgDisk.Source == nil || domcfgDisk.Source.DataStore == nil || domcfgDisk.Source.DataStore.Source == nil || domcfgDisk.Source.DataStore.Source.File == nil {
				return fmt.Errorf("disk %s: spec has DataStore file source but domcfgDisk %+v is missing the DataStore structure, skipping backend update", disk.Alias.GetName(), domcfgDisk)
			}
			log.Log.Infof("Updating disk %s datastore backend from %s to %s", disk.Alias.GetName(), domcfgDisk.Source.DataStore.Source.File.File, disk.Source.DataStore.Source.File)
