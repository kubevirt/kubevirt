...
func (l *LibvirtDomainManager) GetScreenshot(vmi *v1.VirtualMachineInstance) (*v1.ScreenshotResponse, error) {
	...
	if err != nil {
		log.Log.Reason(err).Error("Failed to create stream")
		return nil, err
	}
	defer stream.Close()
	...
	if err != nil {
		log.Log.Reason(err).Error("Failed to call libvirt's Screenshot API")
		return nil, err
	}

	data, err := io.ReadAll(stream)
	if err != nil {
		log.Log.Reason(err).Error("ReadAll from stream failed")
		return nil, err
	}
	log.Log.Object(vmi).V(9).Infof("Screenshot successful: %d bytes, mime: %s", len(data), mime)
