...
func createQCOW2OverlayFunc(overlayPath, imagePath string, blockDev bool) error {
	...
	defer func(path string) {
		if err != nil {
			log.Log.Errorf("Deleting QCOW2 overlay %s due to failure", path)
			os.Remove(path)
		}
	}(overlayPath)
...
func applyChangedBlockTracking(vmi *v1.VirtualMachineInstance, c *converter.ConverterContext) error {
	...
	for _, volume := range vmi.Spec.Volumes {
		...
		logger.Infof("Creating QCOW2 overlay for %+v", volume)
		if volume.VolumeSource.PersistentVolumeClaim == nil &&
			volume.VolumeSource.DataVolume == nil &&
			volume.VolumeSource.HostDisk == nil {
			logger.Infof("SKIP Creating QCOW2 overlay for %s", volume.Name)
			continue
		}
		...
		overlayPath := backup.GetQCOW2OverlayPath(vmi, volumeName)
		logger.V(3).Infof("QCOW2 overlay path is %s", overlayPath)
		if !shouldCreateQCOW2Overlay(vmi) {
			logger.Infof("SKIP Creating QCOW2 overlay for %s, overlay is already enabled", volume.Name)
