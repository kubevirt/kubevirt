...
func (l *LibvirtDomainManager) generateConverterContext(vmi *v1.VirtualMachineInstance, allowEmulation bool, options *cmdv1.VirtualMachineOptions, isMigrationTarget bool) (*converter.ConverterContext, error) {
	...
	if vmi.IsBootloaderEFI() {
		...
		if !l.efiEnvironment.Bootable(secureBoot, sev, cca) {
			log.Log.Errorf("EFI OVMF roms missing for booting in EFI mode with SecureBoot=%v, SEV=%v, CCA=%v", secureBoot, sev, cca)
			return nil, fmt.Errorf("EFI OVMF roms missing for booting in EFI mode with SecureBoot=%v, SEV=%v, CCA=%v", secureBoot, sev, cca)
		}

		efiConf = &converter.EFIConfiguration{
			EFICode:      l.efiEnvironment.EFICode(secureBoot, sev, cca),
			EFIVars:      l.efiEnvironment.EFIVars(secureBoot, sev, cca),
