...
var _ = Describe("SetOptimalIOMode", func() {
	diskInfo := func(virtualSize, actualSize int64) func(string) (*disk.DiskInfo, error) {
		return func(_ string) (*disk.DiskInfo, error) {
			return &disk.DiskInfo{VirtualSize: virtualSize, ActualSize: actualSize}, nil
		}
	}
	// used where the decision must not depend on the image at all, so that
	// consulting it would change the outcome
	diskInfoUnavailable := func(_ string) (*disk.DiskInfo, error) {
		return nil, errors.New("qemu-img failed")
	}

	DescribeTable("should set appropriate IO modes",
		func(d *api.Disk, expectedIO v1.DriverIO, getDiskInfo func(string) (*disk.DiskInfo, error)) {
			configurator := &Configurator{getDiskInfo: getDiskInfo}
			configurator.SetOptimalIOMode(d)
			Expect(d.Driver.IO).To(Equal(expectedIO))
		},
		Entry("user-specified IO",
			&api.Disk{Driver: &api.DiskDriver{IO: v1.IOThreads}},
			v1.IOThreads, diskInfoUnavailable,
		),
		Entry("sparse image",
			&api.Disk{Source: api.DiskSource{File: "test.img"}, Driver: &api.DiskDriver{}},
			v1.DriverIO(""), diskInfo(100, 50),
		),
		...
		Entry("qcow2 image allocating slightly beyond its virtual size, with O_DIRECT",
			&api.Disk{Source: api.DiskSource{File: "test.qcow2"}, Driver: &api.DiskDriver{Cache: string(v1.CacheNone)}},
			v1.IONative, diskInfo(100, 110),
		),
		Entry("image whose allocation cannot be determined, with O_DIRECT",
			&api.Disk{Source: api.DiskSource{File: "test.img"}, Driver: &api.DiskDriver{Cache: string(v1.CacheNone)}},
			v1.DriverIO(""), diskInfoUnavailable,
		),
