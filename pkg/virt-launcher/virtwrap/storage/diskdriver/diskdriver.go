...
type Configurator struct {
	ioChecker   ioChecker
	getDiskInfo func(path string) (*disk.DiskInfo, error)
}

func New() *Configurator {
	return &Configurator{
		ioChecker:   &directIOChecker{},
		getDiskInfo: disk.GetDiskInfo,
	}
}
...
func (c *Configurator) SetDriverCacheMode(d *api.Disk) error {
	...
	switch mode {
	case v1.CacheNone, v1.CacheDirectSync:
...
func (c *Configurator) SetOptimalIOMode(d *api.Disk) {
	if d == nil {
		return
	}
	...
	cacheMode := v1.DriverCache(d.Driver.Cache)
	if cacheMode == v1.CacheNone || cacheMode == v1.CacheDirectSync {
		if ds.BackendIsBlock() || c.isFullyAllocated(ds.BackendPath()) {
...
// isFullyAllocated reports whether the image's allocated size already covers its
// virtual size, meaning guest writes will not have to allocate. That is the
// precondition for io=native: on a sparse image, allocating writes make io_submit
// block and defeat native AIO.
func (c *Configurator) isFullyAllocated(path string) bool {
	diskInf, err := c.getDiskInfo(path)
	if err != nil {
		log.Log.Reason(err).Warningf("Cannot determine allocation for %s, assuming sparse", path)
		return false
	}
