...
var _ = Describe("FSFreeze", func() {
	...
	It("should freeze a VirtualMachineInstance", func() {
		...
		mockDomain.EXPECT().AgentSetResponseTimeout(fsFreezeTimeoutSeconds, uint32(0)).Return(nil).Times(1)
		mockDomain.EXPECT().FSFreeze(nil, uint32(0)).Times(1)
		mockDomain.EXPECT().AgentSetResponseTimeout(-1, uint32(0)).Return(nil).Times(1)
	...
	It("should fail freeze when setting agent timeout fails", func() {
		...
		mockConn.EXPECT().LookupDomainByName(testDomainName).Return(mockDomain, nil).Times(1)
		mockDomain.EXPECT().Free().Times(1)
		mockDomain.EXPECT().AgentSetResponseTimeout(fsFreezeTimeoutSeconds, uint32(0)).Return(fmt.Errorf("agent not available")).Times(1)
