...
func (b *mapBuilder) coalesce(metacontext string, offset, length, flags uint64) error {
	...
	if last != nil && last.Type == flags && last.Offset+last.Length == offset {
		last.Length += length
		return nil
	}
...
func (c *NBDClient) Map(req *nbdv1.MapRequest, stream nbdv1.NBD_MapServer) error {
	...
	for currentOffset < endOffset {
		...
		if currentOffset <= prevOffset {
			log.Log.Warningf("BlockStatus64 returned no forward progress at offset %d, truncating map", prevOffset)
			break
		}
	}
