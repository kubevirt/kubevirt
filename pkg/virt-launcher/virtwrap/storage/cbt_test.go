...
var _ = Describe("Changed Block Tracking", func() {
	...
	Context("ApplyChangedBlockTracking", func() {
		...
		It("should process hotplug volumes with correct paths", func() {
			...
			Expect(capturedPaths).To(ContainElement(converterstorage.GetHotplugFilesystemVolumePath("hotplug-pvc-volume")))
			Expect(capturedPaths).To(ContainElement(converterstorage.GetHotplugFilesystemVolumePath("hotplug-dv-volume")))
		})
		...
		It("should process hotplug block volumes with correct paths", func() {
			...
			Expect(capturedPath).To(Equal(converterstorage.GetHotplugBlockDeviceVolumePath("hotplug-block-volume")))
		})
	...
	Context("ApplyChangedBlockTrackingForMigration", func() {
		...
		DescribeTable("should create overlays for RWO backend storage",
			...
			Entry("filesystem volumes", false, converterstorage.GetFilesystemVolumePath),
			Entry("block volumes", true, converterstorage.GetBlockDeviceVolumePath),
		)
		...
		DescribeTable("should create overlays for hotplug volumes with RWO backend",
			...
			Entry("filesystem volume", "hotplug-fs-volume", false, converterstorage.GetHotplugFilesystemVolumePath),
			Entry("block volume", "hotplug-block-volume", true, converterstorage.GetHotplugBlockDeviceVolumePath),
		)
