...
var _ = Describe("Backup", func() {
	...
	Describe("backup with freeze/thaw", func() {
		...
		Context("successful backup with freeze and thaw", func() {
			It("should freeze, start backup, and thaw", func() {
				...
				Expect(backupMetadata.CheckpointName).ToNot(BeEmpty())
				Expect(backupMetadata.CheckpointName).To(ContainSubstring("test-backup"))
				Expect(backupMetadata.Disks).ToNot(BeEmpty())
				Expect(backupMetadata.Disks).To(ContainSubstring("disk0"))
				Expect(backupMetadata.Disks).To(ContainSubstring("vda"))
			})
		})
	...
	Describe("generateDomainBackup", func() {
		It("should generate backup XML for disks with DataStore", func() {
			...
			domainBackup, domainCheckpoint, diskInfos := generateDomainBackup(disks, backupOptions, tempDir)
			...
			Expect(diskInfos).To(HaveLen(1))
			Expect(diskInfos[0].VolumeName).To(Equal("disk0"))
			Expect(diskInfos[0].DiskTarget).To(Equal("vda"))
		})
		...
		It("should return disk info for multiple disks with DataStore", func() {
			disks := []api.Disk{
				{
					Target: api.DiskTarget{Device: "vda"},
					Source: api.DiskSource{DataStore: &api.DataStore{}},
			...
			_, _, diskInfos := generateDomainBackup(disks, backupOptions, tempDir)

			Expect(diskInfos).To(HaveLen(2))
			Expect(diskInfos[0].VolumeName).To(Equal("rootdisk"))
			Expect(diskInfos[0].DiskTarget).To(Equal("vda"))
			Expect(diskInfos[1].VolumeName).To(Equal("datadisk"))
			Expect(diskInfos[1].DiskTarget).To(Equal("vdb"))
		})
	})
