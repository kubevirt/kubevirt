...
var _ = ginkgo.Describe("LaunchSecurity SEV-SNP", func() {
	ginkgo.Context("XML marshalling and unmarshalling", func() {
		ginkgo.It("should marshal and unmarshal SEV-SNP launch security with all fields", func() {
			...
			Expect(unmarshalled.Type).To(Equal("sev-snp"))
			Expect(unmarshalled.Policy).To(Equal("0x30000"))
			Expect(unmarshalled.Cbitpos).To(Equal("51"))
			Expect(unmarshalled.ReducedPhysBits).To(Equal("1"))
		})

		ginkgo.It("should marshal and unmarshal SEV-SNP launch security with minimal fields", func() {
			launchSecurity := &LaunchSecurity{
				Type:   "sev-snp",
				Policy: "0x30000",
			}
