...
var _ = Describe("Sidecar readiness deadline", func() {
	...
	BeforeEach(func() {
		originalTimeout = sidecarReadinessTimeout
		sidecarReadinessTimeout = testReadinessTimeout
	...
	AfterEach(func() {
		sidecarReadinessTimeout = originalTimeout
	...
	//
	// The assertion is deliberately not a wall-clock measurement. Plugin "a" has a socket that
	// never appears and so burns the whole budget; plugin "b" has a socket that already exists.
	// With one shared deadline, "b" is rejected without ever looking at its socket ("not ready
	// after"). Were the deadline recomputed per hook, "b" would get a fresh window, find its
	// socket immediately and fail later, while dialing it. The two outcomes are distinguishable
	// by error alone.
	It("should be shared by all sidecar hooks rather than recomputed per hook", func() {
		...
		presentSocketDir := filepath.Join(pluginSocketBaseDir, "b")
		Expect(os.MkdirAll(presentSocketDir, 0755)).To(Succeed())
		presentSocket := filepath.Join(presentSocketDir, "hook.sock")
		Expect(os.WriteFile(presentSocket, nil, 0600)).To(Succeed())

		plugins := []pluginv1alpha1.Plugin{
			sidecarPlugin("a", missingSocket, pluginv1alpha1.FailureStrategyIgnore),
			sidecarPlugin("b", presentSocket, pluginv1alpha1.FailureStrategyFail),
		}

		_, _, err := ApplyGuestDefinitionHooks(plugins, vmi, spec, pluginv1alpha1.InvocationContextBoot)
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring(presentSocket))
