...
var _ = Describe("selectHooks", func() {
	...
	Context("hook point filtering", func() {
		...
		It("skips a CEL hook whose hook point does not match", func() {
			plugin := pluginv1alpha1.Plugin{
				ObjectMeta: metav1.ObjectMeta{Name: "plugin"},
				Spec: pluginv1alpha1.PluginSpec{
					LauncherHooks: []pluginv1alpha1.LauncherHook{
						{CEL: &pluginv1alpha1.CELLauncherHook{HookPoint: pluginv1alpha1.LauncherHookPreBoot, Expression: "Domain{}"}},
					},
				},
			}
		...
		It("selects a sidecar hook whose permittedHooks include the hook point", func() {
			plugin := pluginv1alpha1.Plugin{
				...
				Spec: pluginv1alpha1.PluginSpec{
					LauncherHooks: []pluginv1alpha1.LauncherHook{
						{Sidecar: &pluginv1alpha1.SidecarLauncherHook{SocketPath: "/tmp/hook.sock", PermittedHooks: []pluginv1alpha1.LauncherHookPoint{pluginv1alpha1.LauncherHookPreBoot, pluginv1alpha1.LauncherHookGuestDefinition}}},
					},
				},
			}
		...
		It("skips a sidecar hook whose permittedHooks do not include the hook point", func() {
			plugin := pluginv1alpha1.Plugin{
				ObjectMeta: metav1.ObjectMeta{Name: "plugin"},
				Spec: pluginv1alpha1.PluginSpec{
					LauncherHooks: []pluginv1alpha1.LauncherHook{
						{Sidecar: &pluginv1alpha1.SidecarLauncherHook{SocketPath: "/tmp/hook.sock", PermittedHooks: []pluginv1alpha1.LauncherHookPoint{pluginv1alpha1.LauncherHookPreBoot}}},
					},
				},
			}
