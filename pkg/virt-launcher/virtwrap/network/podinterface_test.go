...
var _ = Describe("Pod Network", func() {
	...
	TestPodInterfaceIPBinding := func(vm *v1.VirtualMachineInstance, domain *api.Domain) {
		...
		mockNetwork.EXPECT().GenerateRandomMac().Return(fakeMac)
		...
		mockNetwork.EXPECT().StartDHCP(proxyTestNic, proxyGwAddr, api.DefaultBridgeName)
		mockNetwork.EXPECT().GetAvailableAddrsFromCIDR(api.DefaultVMCIDR).Return([]string{"10.0.2.1/30", "10.0.2.2/30"}, nil)
