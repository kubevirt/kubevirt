...
type NetworkHandler interface {
	...
	GetAvailableAddrsFromCIDR(s string) ([]string, error)
	SetRandomMac(iface string) (net.HardwareAddr, error)
	GenerateRandomMac() net.HardwareAddr
...
func (h *NetworkUtilsHandler) GetAvailableAddrsFromCIDR(s string) ([]string, error) {
	...
	if len(ips) < 4 {
		return nil, fmt.Errorf("less than 4 addresses on network")
	}
...
func (h *NetworkUtilsHandler) GenerateRandomMac() net.HardwareAddr {
