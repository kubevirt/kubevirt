...
func (p *ProxyPodInterface) discoverPodNetworkInterface() error {
	...
	defaultGateway, vm, err := p.getVMNetwork()
	if err != nil {
		return err
	}
...
func (p *ProxyPodInterface) preparePodNetworkInterfaces() error {
	...
	if p.iface.MacAddress == "" {
		p.vif.MAC = Handler.GenerateRandomMac()
	}
...
func (p *ProxyPodInterface) startDHCPServer() {
	// Start DHCP Server
	Handler.StartDHCP(p.vif, p.gatewayAddr, p.bridgeInterfaceName)
}
...
func (p *ProxyPodInterface) getVMNetwork() (string, string, error) {
	...
	if err != nil {
		log.Log.Errorf("failed to get avilable addresses from CIDR %s", p.vmNetworkCIDR)
		return "", "", err
	}
