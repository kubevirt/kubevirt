...
func (p *PollerWorker) Poll(execAgentCommands agentCommandsExecutor, closeChan chan struct{}, initialInterval time.Duration) {
	log.Log.Infof("Agent poller started with commands: %v", p.AgentCommands)
...
// Poll is the call to the guest agent using libvirt API.
func pollApiOperation(execApiOperation apiOperationsExecutor, closeChan chan struct{}, initialInterval time.Duration, callTick time.Duration) {
	log.Log.Infof("Polling API operation")
...
func (p *AgentPoller) Start() {
	...
	go pollApiOperation(func() {
		fetchAndStoreGuestInfo(p.Connection, p.agentStore, p.domainName)
	}, p.agentDone, pollInitialInterval, apiCallTick)
}
...
func fetchAndStoreGuestInfo(con cli.Connection, agentStore *AsyncAgentStore, domainName string) {
	domain, err := con.LookupDomainByName(domainName)
	if err != nil {
		// Skip execution on error, domain lookup failed
		return
	}
	defer domain.Free()

	guestInfo, err := domain.GetGuestInfo(INTERFACES|OSINFO|HOSTNAME|TIMEZONE|USERS, 0)
	if err != nil {
		// If fetching guest info fails, skip processing
		return
	}
