	...
	if guestInfo.FileSystems != nil {
		for _, fs := range guestInfo.FileSystems {
			filesystems = append(filesystems, api.Filesystem{
				...
				Type:       fs.FSType,
				TotalBytes: int(fs.TotalBytes),
				UsedBytes:  int(fs.UsedBytes),
				Disk:       convertToFSDisks(fs.Disks),
			})
		}
	}
	return filesystems
...
func convertToFSDisks(disks []libvirt.DomainGuestInfoFileSystemDisk) []api.FSDisk {
	var fsDisks []api.FSDisk
	for _, disk := range disks {
		fsDisks = append(fsDisks, api.FSDisk{
			Serial:  disk.Serial,
			BusType: disk.Alias,
		})
	}
