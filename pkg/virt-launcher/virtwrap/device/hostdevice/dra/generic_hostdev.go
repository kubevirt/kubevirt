...
func CreateDRAHostDevices(vmi *v1.VirtualMachineInstance, metadataCache *drautil.DRAFileData) ([]api.HostDevice, error) {
	...
	for _, hd := range vmi.Spec.Domain.Devices.HostDevices {
		...
		hostDevice, err := createHostDeviceForHostDevice(hd, metadataCache)
		if err != nil {
...
func createHostDeviceForHostDevice(hd v1.HostDevice, metadataCache *drautil.DRAFileData) (*api.HostDevice, error) {
