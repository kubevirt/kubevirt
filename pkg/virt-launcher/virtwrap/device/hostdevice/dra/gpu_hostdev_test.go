...
var _ = Describe("CreateDRAGPUHostDevices", func() {
	var pciAddr0 = "0000:02:00.0"

	var mdevUUID0 = "123e4567-e89b-12d3-a456-426614174000"

	var pciAddr1 = "0000:01:01.0"
	var mdevUUID1 = "abcd1234-e89b-12d3-a456-426614174000"

	var pciAddr2 = "0000:00:01.0"
	var vgpuPCIAddr2 = "0000:01:01.0"
	var mdevUUID2 = "deadbeef-e89b-12d3-a456-426614174000"
	...
	DescribeTableSubtree("creation", func(viaIOMMUFD bool) {
		BeforeEach(func() {
			...
			mockVFIOSpec.EXPECT().IsPCIAssignableViaIOMMUFD(pciAddr1).Return(viaIOMMUFD).AnyTimes()
			mockVFIOSpec.EXPECT().IsPCIAssignableViaIOMMUFD(pciAddr2).Return(viaIOMMUFD).AnyTimes()
			...
			mockVFIOSpec.EXPECT().IsMDevAssignableViaIOMMUFD(mdevUUID1).Return(viaIOMMUFD).AnyTimes()
			mockVFIOSpec.EXPECT().IsMDevAssignableViaIOMMUFD(mdevUUID2).Return(viaIOMMUFD).AnyTimes()
		...
		Context("when the VMI has no GPUs with DRA", func() {
			It("should return an empty slice without error", func() {
				...
				hostDevices, err := CreateDRAGPUHostDevices(vmi, tempDir, vfioSpec)
				Expect(err).ToNot(HaveOccurred())
		...
		Context("when the device has both pciBusID and mdevUUID", func() {
			It("should prefer mdevUUID and create an mdev host device", func() {
					...
					Requests: []metadata.DeviceMetadataRequest{{
						...
						Devices: []metadata.Device{{
							...
							Attributes: map[resourcev1.QualifiedName]resourcev1.DeviceAttribute{
								...
								metadata.MDevUUIDAttribute: {StringValue: &mdevUUID1},
							},
						}},
					}},
				})
				...
				Expect(dev.Source.Address.UUID).To(Equal(mdevUUID1))
				Expect(dev.Alias.GetName()).To(Equal(AliasPrefix + "vgpu1"))
				Expect(dev.Display).To(Equal("on"))
				Expect(dev.RamFB).To(Equal("on"))
				if viaIOMMUFD {
		...
		Context("when VMI has both a pGPU and a vGPU", func() {
			It("should create one PCI and one mdev host device", func() {
				...
				createMetadataFile("vgpu-claim", "vgpu", "gpu.example.com", &metadata.DeviceMetadata{
					...
					Requests: []metadata.DeviceMetadataRequest{{
						...
						Devices: []metadata.Device{{
							...
							Name:   "gpu-0-vgpu-0",
							Attributes: map[resourcev1.QualifiedName]resourcev1.DeviceAttribute{
								metadata.PCIBusIDAttribute: {StringValue: &vgpuPCIAddr2},
								metadata.MDevUUIDAttribute: {StringValue: &mdevUUID2},
							},
						}},
					}},
				})
				...
				if viaIOMMUFD {
					Expect(pciDev.Driver).ToNot(BeNil())
					Expect(pciDev.Driver.IOMMUFD).To(Equal("yes"))
				} else {
					Expect(pciDev.Driver).To(BeNil())
				}
				...
				Expect(mdevDev.Source.Address.UUID).To(Equal(mdevUUID2))
				if viaIOMMUFD {
					Expect(mdevDev.Driver).ToNot(BeNil())
					Expect(mdevDev.Driver.IOMMUFD).To(Equal("yes"))
				} else {
					Expect(mdevDev.Driver).To(BeNil())
				}
			})
		})
