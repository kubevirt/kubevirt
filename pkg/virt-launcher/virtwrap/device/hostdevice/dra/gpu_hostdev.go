...
func CreateDRAGPUHostDevices(vmi *v1.VirtualMachineInstance, metadataCache *drautil.DRAFileData) ([]api.HostDevice, error) {
	...
	for _, gpu := range vmi.Spec.Domain.Devices.GPUs {
		...
		hostDevice, err := createHostDeviceForGPU(gpu, metadataCache)
		if err != nil {
...
func createHostDeviceForGPU(gpu v1.GPU, metadataCache *drautil.DRAFileData) (*api.HostDevice, error) {
	...
	if pciAddr, err := metadataCache.GetPCIAddressForClaim(claimName, requestName); err == nil {
	...
	if mdevUUID, err := metadataCache.GetMDevUUIDForClaim(claimName, requestName); err == nil {
	...
	return nil, fmt.Errorf("GPU %s has no pciBusID or mdevUUID in metadata for claim %s request %s", gpu.Name, claimName, requestName)
}
