...
func (l *AccessCredentialManager) agentWriteAuthorizedKeys(domName string, filePath string, authorizedKeys string) error {
	...
	// ######
	// Step 1. Validate if file exists on guest already or not.
	// ######
	...
	if err == nil {
		...
		log.Log.Infof("VOSSEL -DEBUG- OUPENOUTPUT: %s ERR: %s", output, err.Error())
		return err
	}

	if !fileExists {
		return fmt.Errorf("Unable to update authorized_keys file because file does not exist on the guest at path %s", filePath)
	...
	if readRes.Count > 0 {
		curAuthorizedKeysBytes, err := base64.StdEncoding.DecodeString(readRes.BufB64)
		if err != nil {
			return err
		}
		curAuthorizedKeys = string(curAuthorizedKeysBytes)
	}

	log.Log.Infof("VOSSEL -DEBUG- CUR AUTH KEYS: %s", curAuthorizedKeys)
	...
	log.Log.Infof("VOSSEL -DEBUG- STRIPPED CUR AUTH KEYS: %s", curAuthorizedKeys)
...
func (l *AccessCredentialManager) HandleQemuAgentAccessCredentials(vmi *v1.VirtualMachineInstance) {
	...
	go func() {
		...
		for {
			...
			// Step 2. Write Authorized keys file
