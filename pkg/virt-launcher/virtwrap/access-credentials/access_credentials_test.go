...
var _ = Describe("AccessCredentials", func() {
	...
	It("should handle dynamically updating user/password with qemu agent", func() {
		...
		mockDomain.EXPECT().SetUserPassword(user, password, libvirt.DomainSetUserPasswordFlags(0)).Times(1)
	...
	It("should trigger updating a credential when secret propagation change occurs.", func() {
		...
		matched := false
		mockDomain.EXPECT().SetUserPassword(user, password, libvirt.DomainSetUserPasswordFlags(0)).MinTimes(1).DoAndReturn(
			func(_, _ string, _ libvirt.DomainSetUserPasswordFlags) error {
				matched = true
				return nil
			})
		...
		mockDomain.EXPECT().SetUserPassword(user, password, libvirt.DomainSetUserPasswordFlags(0)).MinTimes(1).Return(nil)
