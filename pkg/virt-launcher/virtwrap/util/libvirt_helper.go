...
func configureQemuConf(qemuFilename string) (err error) {
	...
	if pathsStr, ok := os.LookupEnv(envvar.ENV_VAR_SHARED_FILESYSTEM_PATHS); ok {
...
func (l LibvirtWrapper) SetupLibvirt(customLogFilters *string) (err error) {
	...
	if envVarValue, envVarDefined := os.LookupEnv(envvar.ENV_VAR_VIRT_LAUNCHER_LOG_VERBOSITY); envVarDefined {
		libvirtLogVerbosityEnvVar = &envVarValue
	}
	_, libvirtDebugLogsEnvVarDefined := os.LookupEnv(envvar.ENV_VAR_LIBVIRT_DEBUG_LOGS)
...
func getLibvirtLogFilters(customLogFilters, libvirtLogVerbosityEnvVar *string, libvirtDebugLogsEnvVarDefined bool) (logFilters string, enableDebugLogs bool) {
	...
	if libvirtLogVerbosityEnvVar != nil {
		...
		if err != nil {
			log.Log.Infof("cannot apply %s value %s - must be a number", envvar.ENV_VAR_VIRT_LAUNCHER_LOG_VERBOSITY, *libvirtLogVerbosityEnvVar)
