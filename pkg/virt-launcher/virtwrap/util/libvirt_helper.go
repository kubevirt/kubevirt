...
func validateDomainSpecWithVirtLint(virtLintChannel chan VirtLintEvent, libvirtConnection *libvirt.Connect, vmi *v1.VirtualMachineInstance, wantedSpec string) error {
	...
	virtLintChannel <- VirtLintEvent{"Normal", "Starting", fmt.Sprintf("Using virt-lint version %v.%v.%v", major, minor, micro)}
	...
	if slices.Contains(tags, "kubevirt") != true {
		virtLintChannel <- VirtLintEvent{"Warning", "Starting", "No 'kubevirt' validation tag exists"}
		return nil
	}
	...
	for _, warn := range warnings {
		...
		message := warn.Msg
		virtLintChannel <- VirtLintEvent{severity, reason, message}
	}
