...
var _ = Describe("LibvirtHelper", func() {
	...
	Context("getLibvirtLogFilters()", func() {
		...
		DescribeTable("should return customLogFilters if defined and not empty with", func(libvirtLogVerbosityEnvVar *string, libvirtDebugLogsEnvVarDefined bool) {
			customLogFilters := ptr.To("3:remote 4:event 3:util.json 3:util.object 3:util.dbus 3:util.netlink 3:node_device 3:rpc 3:access")
			...
			Entry("libvirtLogVerbosityEnvVar defined, libvirtDebugLogsEnvVarDefined false", ptr.To("2"), false),
			Entry("libvirtLogVerbosityEnvVar not defined, libvirtDebugLogsEnvVarDefined true", nil, true),
			Entry("libvirtLogVerbosityEnvVar defined, libvirtDebugLogsEnvVarDefined true", ptr.To("1"), true),
		)
		...
		Context("with customLogFilters not defined", func() {
			...
			DescribeTable("logs should be enabled if debugLogs env var is defined when", func(libvirtLogVerbosityEnvVar *string) {
				...
				Entry("libvirtLogVerbosityEnvVar defined to 8", ptr.To("8")),
				Entry("libvirtLogVerbosityEnvVar defined to 3", ptr.To("3")),
			...
			DescribeTable("with debugLogs not defined logs should", func(libvirtLogVerbosity *int, expectedEnableDebugLogs bool) {
				...
				if libvirtLogVerbosity != nil {
					libvirtLogVerbosityEnvVar = ptr.To(fmt.Sprintf("%d", *libvirtLogVerbosity))
				}
				...
				Entry("be disabled when libvirt log verbosity is below threshold", ptr.To(verbosityThreshold-1), false),
				Entry("be disabled when libvirt log verbosity is equal to threshold", ptr.To(verbosityThreshold), true),
				Entry("be enabled when libvirt log verbosity is above threshold", ptr.To(verbosityThreshold+1), true),
