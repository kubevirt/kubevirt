...
var _ = Describe("LibvirtHelper", func() {
	...
	It("should parse libvirt logs", func() {
		...
		logger := funcr.NewJSON(
			func(obj string) { fmt.Fprintln(buffer, obj) },
			funcr.Options{LogTimestamp: true, Verbosity: 10},
		)
		...
		for scanner.Scan() {
			entry := stripFuncrFields(scanner.Bytes())
