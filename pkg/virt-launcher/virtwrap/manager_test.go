	...
	Context("on successful VirtualMachineInstance migrate", func() {
		...
		Context("vGPU live migration target", func() {
			...
			It("should add vGPUs to converter context when migration target has single vGPU and vGPU live migration is enabled", func() {
				...
				manager, err := NewLibvirtDomainManager(mockLibvirt.VirtConnection, testVirtShareDir, testEphemeralDiskDir, nil, "/usr/share/OVMF", ephemeralDiskCreatorMock, metadataCache, nil, virtconfig.DefaultDiskVerificationMemoryLimitBytes, fakeCpuSetGetter, false, nil, v1.KvmHypervisorName, true)
				Expect(err).ToNot(HaveOccurred())
