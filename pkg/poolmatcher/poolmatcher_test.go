...
var _ = Describe("PoolMatcher", func() {
	...
	Context("MatchVMIToWorkerPool", func() {
		...
		It("should return nil when no pool matches", func() {
			pools := []workerv1.WorkerPool{
				{
					ObjectMeta: metav1.ObjectMeta{Name: "gpu-pool"},
					Spec: workerv1.WorkerPoolSpec{
						Selector: workerv1.WorkerPoolSelector{DeviceNames: []string{"nvidia.com/A100"}},
					},
				},
			}
			vmi := &v1.VirtualMachineInstance{}
		...
		It("should match by hostDevice deviceName", func() {
			pools := []workerv1.WorkerPool{
				{
					ObjectMeta: metav1.ObjectMeta{Name: "fpga-pool"},
					Spec: workerv1.WorkerPoolSpec{
						Selector: workerv1.WorkerPoolSelector{DeviceNames: []string{"intel.com/fpga"}},
					},
				},
			}
			vmi := &v1.VirtualMachineInstance{
		...
		It("should match by VMLabels", func() {
			pools := []workerv1.WorkerPool{
				{
					ObjectMeta: metav1.ObjectMeta{Name: "label-pool"},
					Spec: workerv1.WorkerPoolSpec{
						Selector: workerv1.WorkerPoolSelector{
							VMLabels: &workerv1.WorkerPoolVMLabels{
		...
		It("should match with OR semantics between deviceNames and vmLabels", func() {
			pools := []workerv1.WorkerPool{
				{
					ObjectMeta: metav1.ObjectMeta{Name: "or-pool"},
					Spec: workerv1.WorkerPoolSpec{
						Selector: workerv1.WorkerPoolSelector{
