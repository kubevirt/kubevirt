...
type CheckpointManager interface {
	...
	Check(key string, value interface{}) error
	// os.ErrNotExist is returned if there is no checkpoint for given key.
...
func (cp *simpleCheckpointManager) Check(key string, value interface{}) error {
	path := path(cp.basePath, key)
...
func (cp *simpleCheckpointManager) Delete(key string) error {
	return os.Remove(path(cp.basePath, key))
}
