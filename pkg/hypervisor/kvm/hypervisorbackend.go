...
func (k *KvmHypervisorBackend) GetMemoryOverhead(vmi *v1.VirtualMachineInstance, cpuArch string, additionalOverheadRatio *string) resource.Quantity {
	...
	if vmipredicates.IsVFIOVMI(vmi) {
	...
	if vmipredicates.IsSEVVMI(vmi) || vmipredicates.IsSEVSNPVMI(vmi) || vmipredicates.IsSEVESVMI(vmi) {
	...
	if vmipredicates.RequiresMemoryOverheadReservation(vmi) {
